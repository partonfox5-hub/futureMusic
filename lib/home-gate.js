...
function ipIsHome(ip) {
  ...
  if (p && p[0] === 172 && p[1] >= 16 && p[1] <= 31) return true;
  const v6 = ipv6Hextets(ip);
  if (v6) {
    // Unique-local fd00::/8 (and fc00::/7) and link-local fe80::/10.
...
function isRestrictedPath(pathname) {
  ...
  if (starts(p, "/battle-sphere-arena")) return false;
  if (starts(p, "/arena")) return false;
