...
function mount(app, { storage, bucketName }) {
  ...
  async function writeGcs(id, obj) {
    ...
    try {
      ...
      if (obj.rev) {
        await storage.bucket(bucketName).file(prefix + id + ".r" + obj.rev + ".json").save(payload, opts);
      }
      return true;
    } catch (e) {
  ...
  async function readGcsRev(id, rev) {
    if (!storage || !bucketName || !rev) return null;
    try {
      const [buf] = await storage.bucket(bucketName).file(prefix + id + ".r" + rev + ".json").download();
      return JSON.parse(buf.toString("utf8"));
    } catch {
      return null;
    }
  ...
  app.get("/api/zoom/maps", async (req, res) => {
    ...
    try {
      ...
      for (const m of await listGcs()) if (m && m.id && !String(m.id).includes(".r")) byId.set(m.id, prefer(byId.get(m.id), m));
  ...
  app.get("/api/zoom/maps/:id", async (req, res) => {
    ...
    const wantRev = parseInt(req.query.rev, 10);
    const pinned = Number.isFinite(wantRev) && wantRev > 0 ? await readGcsRev(id, wantRev) : null;
    if (pinned) return res.json(pinned);
  ...
  app.post("/api/zoom/maps", async (req, res) => {
    ...
    try {
      ...
      writeLocal(id, obj);
      let remote = await writeGcs(id, obj);
      if (!remote) remote = await writeGcs(id, obj);
