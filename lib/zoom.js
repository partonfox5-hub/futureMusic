...
function slim(map) {
  return {
    v: 1,
    id: map.id,
    name: String(map.name || "Untitled").slice(0, 64),
    w: map.w,
    h: map.h,
    hallH: map.hallH,
    cells: map.cells,
    spheres: Array.isArray(map.spheres) ? map.spheres.slice(0, 400) : [],
    objects: Array.isArray(map.objects) ? map.objects.slice(0, 400) : [],
    spawners: Array.isArray(map.spawners) ? map.spawners.slice(0, 80) : [],
...
function mount(app, { storage, bucketName }) {
  ...
  async function writeGcs(id, obj) {
    ...
    try {
      await storage
        .bucket(bucketName)
        .file(prefix + id + ".json")
        .save(JSON.stringify(obj), { contentType: "application/json", resumable: false });
      return true;
    } catch (e) {
  ...
  app.get("/api/zoom/maps/:id", async (req, res) => {
    ...
    const m = (await readGcs(id)) || readLocal(id);
  ...
  app.post("/api/zoom/maps", async (req, res) => {
    try {
      ...
      if (body.cells.length > 80000) return res.status(413).json({ error: "too large" });
      const obj = slim({ ...body, id, updated: Date.now() });
      writeLocal(id, obj);
      const remote = await writeGcs(id, obj);
      res.json({ ok: true, id, remote });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
