...
function preloadAssets() {
    const types = ['dog', 'chimp', 'deer', 'wolf', 'gorilla', 'moose', 'rabbit', 'snake', 'crocodile', 'pig', 'rhino', 'anteater', 'elephant', 'cat', 'lion', 'sparrow', 'eagle', 'velociraptor', 't-rex'];
 ...
 // 0. Respawn Logic (5% every 30s)
    ...
    if(world.respawnTimer >= 30) {
        ...
        if(monos < 5) {
         ...
         // Maps (add 1 occasionally if low)
        ...
        // Birdshots (add 1 occasionally if low)
    ...
    if(player.type === 'velociraptor' || player.type === 't-rex') baseBreed = 0.065; // -35% reproduction
                ...
                if(getDist(h.center.x, h.center.y, ent.x, ent.y) < 150) {
                    ...
                    for(let m of h.members) {
                         if(getDist(m.x, m.y, ent.x, ent.y) < 40) {
                            ...
                            // AI Pickup Logic Fix
                            else if (ent.type === 'monolith') { aiAte = true; h.xp += 10; playSound('monolith'); } // AI just gets XP for monolith
                            else if (ent.type === 'birdshot') { aiAte = true; h.birdCooldown = 0; } // Instant reload
  ...
  // FIX: Check if actual members are touching, not just herd centers
    ...
    for(let i=world.projectiles.length-1; i>=0; i--) {
        ...
        if(checkHardCollision(p.x, p.y, 5)) {
         ...
         // --- NEW: Player Collision (AI shooting Player) ---
        if (p.ownerHerd) { // Only check if projectile has an owner (AI fired it)
            if (getDist(p.x, p.y, player.pos.x, player.pos.y) < 100) {
                // Create Fight Cloud
                const cloud = new FightCloud(p.x, p.y);

                // Create temporary Bird (Enemy Side)
                const bird = new Animal(p.type, p.x, p.y, false, null); // isPlayer=false
                bird.currentHp = p.isEagle ? p.hp * 2 : p.hp;
                // Override damage logic
                bird.getDamage = () => (p.isEagle ? p.damage * 2 : p.damage);

                cloud.addCombatant(bird, 'enemy'); // Bird fights AGAINST player

                // Add nearby Player animals (Player Side)
                player.herd.forEach(m => {
                    if (getDist(m.x, m.y, p.x, p.y) < 150 && !m.combatState) {
                        cloud.addCombatant(m, 'player');
                    }
                });
...
// Enemy Collision
    ...
    if(player.type === 't-rex') baseZoomTarget *= 0.85; // 15% extra zoom out for T-Rex
 ...
 // Update Enemy Herds
  ...
  // Passive XP Gain with Difficulty Multiplier
        ...
        // Reproduction
        let diffBreedMod = 0;
        if(GAME_STATE.difficulty === 'medium') diffBreedMod = 0.025; // +2.5%
        if(GAME_STATE.difficulty === 'hard') diffBreedMod = 0.075;   // +7.5%
...
// Fire at player
         ...
         // --- NEW: Stuck Prevention Logic ---
        herd.stuckTimer += dt;
        // Check every 2 seconds
        if(herd.stuckTimer >= 2.0) {
            // Calculate distance moved in last 2 seconds
            const distMoved = getDist(herd.center.x, herd.center.y, herd.lastPos.x, herd.lastPos.y);
            
            // If moved less than 30px, we are likely stuck on a tree or water edge
            if(distMoved < 30) {
                // Force a completely new random direction away from current heading
                herd.dir = Math.random() * 6.28; 
                // Force movement in this new direction for at least 1.5 seconds
                herd.timer = 1.5; 
            }
            
            // Reset Tracker
            herd.lastPos = { x: herd.center.x, y: herd.center.y };
            herd.stuckTimer = 0;
        }
        // -----------------------------------
 ...
 // AI: Behavior (Advanced Priority System)
        ...
        if(nearestHerd) {
            ...
            if(herd.members.length > nearestHerd.count) {
                ...
                // --- 1.5. SCAN FOR UPGRADES (High Priority) ---
        // If not fleeing/chasing, look for powerups before looking for food
        if(mode === 'wander') {
            let nearestUpg = null;
            let nearestUpgDist = 9999;

            for(let ent of world.entities) {
                // Check for Monolith, Birdshot, or Map
                if(ent.type === 'monolith' || ent.type === 'birdshot' || ent.type === 'map') {
                    // Fast bounding box check
                    if(Math.abs(ent.x - herd.center.x) > awarenessRange || Math.abs(ent.y - herd.center.y) > awarenessRange) continue;

                    const d = getDist(herd.center.x, herd.center.y, ent.x, ent.y);
                    if(d < awarenessRange && d < nearestUpgDist) {
                        nearestUpgDist = d;
                        nearestUpg = ent;
                    }
                }
            ...
            if(nearestUpg) {
                mode = 'forage'; // Move towards it like food
                targetX = nearestUpg.x;
                targetY = nearestUpg.y;
            }
        }
        // -----------------------------
        ...
        if(!checkHardCollision(nextX, nextY, 20, isAiCroc)) {           
            ...
            herd.dir = Math.random() * 6.28;
            herd.timer = 0.5; // Change 0 to 0.5 to give them time to turn
        }
...
function drawGame(dt) {
    ...
    world.enemyHerds.forEach(herd => {
        ...
        if(herd.members.length > 0) {
            ...
            if(count > 100) spreadMod += (count - 100) * 0.0025;
            if(count > 150) spreadMod += (count - 150) * 0.0025;
 ...
 ctx.restore(); // Restore to Screen Coordinate space (0,0 is top left)
    ...
    // Calculate Scale
    // Mobile: 30% reduction in growth rate (0.02 * 0.7 = 0.014)
    const scaleFactor = GAME_STATE.isMobile ? 0.014 : 0.02;
