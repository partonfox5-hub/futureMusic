...
function drawGame(dt) {
    ...
    world.enemyHerds.forEach(herd => {
        ...
        if(herd.members.length > 0) {
            ...
            let extraGrowth = 0;
            if(count > 100) extraGrowth = (count - 100) * 0.0025;
            
            // Base 1 + (0.25% per member) + (Extra 0.25% per member over 100)
            const rangeScale = 1 + (count * 0.0025) + extraGrowth;
            // ---------------------
    ...
    let pExtraGrowth = 0;
    if(pCount > 100) pExtraGrowth = (pCount - 100) * 0.0025;

    // Base 1 + (0.25% per member) + (Extra 0.25% per member over 100)
    const rangeScale = 1 + (pCount * 0.0025) + pExtraGrowth;
    // ---------------------
