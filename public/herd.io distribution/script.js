...
const STATS = {
    ...
    eagle: { hp: 20, dmg: 30, speed: 12, diet: 'none' },
    ...
    pig: { hp: 36.3, dmg: 9.5, speed: 4.75, diet: 'herbivore' }, // +10% hp, -5% others from base 33/10/5
    rhino: { hp: 45, dmg: 12.5, speed: 6.25, diet: 'herbivore' }, // Big boost
    anteater: { hp: 27, dmg: 9, speed: 4.5, diet: 'herbivore' }, // -10% base
...
// --- ASSET MANAGEMENT (Simulated paths) ---
function preloadAssets() {
const types = ['dog', 'chimp', 'deer', 'wolf', 'gorilla', 'moose', 'rabbit', 'snake', 'crocodile', 'pig', 'rhino', 'anteater', 'elephant', 'cat', 'lion', 'sparrow', 'eagle'];
...
['monolith', 'grass', 'apple', 'sparrow', 'eagle', 'tree', 'map', 'birdshot'].forEach(item => {         const img = new Image();
  ...
  // Touch
    if(GAME_STATE.isMobile) {
        ...
        window.addEventListener('touchmove', e => {
            ...
            if(GAME_STATE.screen === 'game') e.preventDefault();
...
this.offsetY += (this.wanderTargetY - this.offsetY) * 2 * dt;
    ...
    // Calculate max range based on herd size (Base 150 + 0.25% per member)
    const herdCount = this.isPlayer ? player.herd.length : (this.parentHerd ? this.parentHerd.members.length : 1);
    const maxRange = 150 * (1 + (herdCount * 0.0025)); 
...
draw(ctx, tick) {
        ...
        // Animals will be roughly 80px wide/tall regardless of the .png resolution
        const baseSize = 80; 
...
GAME_STATE.lastStartConfig = { heroType };
    ...
    document.getElementById('btn-settings-ingame').classList.remove('hidden');
    ...
    player.herd = [];
    player.xp = 0;
    player.totalXp = 0; // Reset lifetime XP
    player.mapTimer = 0; // Reset map
    player.growthAccumulator = 0;
    player.pos = {x: 2000, y: 2000};
    player.velocity = {x:0, y:0};
    player.moveTime = 0; // for acceleration stages
    player.moveStage = 0;
    player.monolithTimer = 0;
    updateStatsUI();
    
    playGameMusic(); // Switch to Game Music
    
    playSound(`choose_${heroType}`); // Plays choose_dog.mp3, choose_chimp.mp3, etc.
    const playerHerdObj = { members: player.herd }; // Temp wrapper if needed, or just pass null for player
    for(let i=0; i<3; i++) {
        // We pass null for parentHerd for player, handled via player.herd array manually
    ...
    world.enemyHerds = []; // <--- MOVED HERE: Clear herds only on Game Start
...
// Find safe spawn for herd center
let startX = 0, startY = 0;
let attempts = 0;
// Try up to 100 times to find a safe spot
while(attempts < 500) {
    startX = Math.random() * world.width;
    startY = Math.random() * world.height;
    
    // 1. Check Hard Collision (Walls/Water)
    let safe = !checkHardCollision(startX, startY, 200);

    // 2. Check Distance from Player
    if(safe && player && player.pos) {
        if(getDist(startX, startY, player.pos.x, player.pos.y) < 500) safe = false;
    }

    // 3. Check Distance from OTHER Herds (Prevent overlapping spawns)
    if(safe) {
        for(let existing of world.enemyHerds) {
            if(getDist(startX, startY, existing.center.x, existing.center.y) < 400) {
                safe = false;
                break;
            }
        }
...
function spawnWorld() {
    ...
    const getSafePos = (padding = 0) => {
        for(let k=0; k<10; k++) {
            const tx = Math.random() * world.width;
            const ty = Math.random() * world.height;
            if(!checkHardCollision(tx, ty, padding)) return {x: tx, y: ty};
        }
    ...
    // 2. Spawn Rabbits
    for(let i=0; i < 37 * areaMod; i++) {
        const pos = getSafePos(10);
        if(pos) {
            world.entities.push({
                type: 'rabbit',
                x: pos.x,
    ...
    // We loop for grass/apples and ensure they don't overlap trees via checkHardCollision
    const spawnResource = (type, count) => {
        for(let i=0; i<count; i++) {
            const pos = getSafePos(10); 
            if(pos) {
                // Extra check: Do not overlap other apples/grass closely (optional, but requested)
                let overlap = false;
    ...
    for(let i=0; i < 40 * areaMod; i++) {
        ...
        if(Math.random() < 0.5) {
            ...
            if(trees.length > 0) {
                ...
                // Ensure bounds
                if(checkHardCollision(pos.x, pos.y, 10)) pos = getSafePos(10);
            } else { pos = getSafePos(10); }
        } else {
            pos = getSafePos(10);
        }
...
function updateStatsUI() {
    ...
    // Base 10% + 0.5% per level
    const breed = 10 + (player.upgrades.breed.level * 0.5);
...
function fireBirdshot(targetX = null, targetY = null) {
    ...
    if(player.birdPowerupTimer > 0) cd *= 0.5;
...
function updateGame(dt) {
    ...
    if(player.birdCooldown <= 0) {
        ...
        bText.innerText = "SPACE FOR BIRDSHOT";
    } else {
 ...
 // 0. Respawn Logic (5% every 30s)
    world.respawnTimer += dt;
    if(world.respawnTimer >= 30) {
        world.respawnTimer = 0;
        // Approx 5% of initial populations
        const getSafePos = (padding) => { /* simplified inline finder */ 
             for(let k=0; k<5; k++) {
                const tx = Math.random() * world.width; 
                const ty = Math.random() * world.height;
                if(!checkHardCollision(tx, ty, padding)) return {x: tx, y: ty};
    ...
    if(GAME_STATE.herdSpawnTimer >= 60) {
        ...
        const totalHerds = 1 + world.enemyHerds.length; // Player + Enemies
        if(totalHerds < 15) {
            spawnEnemyHerds(1); // Reuse existing function to spawn 1
    ...
    // 1. Growth Logic (Every 15s -> +10% herd size)
    // 10% of herd size every 15s means: (HerdSize * 0.10) / 15 per second
        // Base 10% + 0.5% per breed level
        let baseBreed = 0.10;
    if(player.type === 'snake') baseBreed = 0.20; // Snake reproduces faster
    ...
    if(GAME_STATE.controlScheme === 'wasd') {
        ...
        if(GAME_STATE.mouse.active) {
            // Mouse is screen relative, convert to world relative offset from center screen
    ...
    if(player.monolithTimer > 0) maxSpeed *= 1.2; // Monolith +20% speed

    if(moving) {
        player.moveTime += dt;
        // Stage 1: 50% speed for 1.5s
        if(player.moveTime < 1.5) {
            speedMult = 0.5;
            player.moveStage = 1;
        } 
        // Stage 2: 75% speed for next 1.5s (total 3s)
        else if (player.moveTime < 3.0) {
            speedMult = 0.75;
            player.moveStage = 2;
        } 
        // Stage 3: 100%
 ...
 // Update Player Position
    // Speed Penalty: 0.25% slower per animal > 1
    ...
    // Check 20px radius for player leader against Water, Walls, and Trees
  ...
  // 5. Collision & Interactions
    ...
    for (let i = world.entities.length - 1; i >= 0; i--) {
        ...
        // Check against every member of the player's herd
        for(let j = 0; j < player.herd.length; j++) {
            ...
            if(dist < 50) {
                ...
                player.mapTimer = 60; // Turn on minimap for 60 seconds
                playSound('levelup'); 
                world.entities.splice(i, 1);
                eaten = true;
            }
    ...
    addXP(1, ent.x, ent.y);
    playSound('pickup');
    world.entities.splice(i, 1); // ADDED: Removes item so it can't be eaten again
    eaten = true;
}
else if (ent.type === 'apple' && (memberDiet === 'herbivore' || memberDiet === 'omnivore')) {
    addXP(3, ent.x, ent.y);
    playSound('pickup');
    world.entities.splice(i, 1); // ADDED: Removes item
    eaten = true;
}
else if (ent.type === 'rabbit' && (memberDiet === 'carnivore' || memberDiet === 'omnivore')) {
    const xpAmt = (player.type === 'dog') ? 10 : 5; // Double XP for Dogs
    ...
    eaten = true;
            ...
            if(eaten) break; // Entity is gone, stop checking other herd members
            // AI HERD EATING LOGIC
        if(!eaten) {
            // If the player didn't eat it, check if an AI herd is on top of it
            for(let h of world.enemyHerds) {
                // Check if herd center is close (optimization)
                if(getDist(h.center.x, h.center.y, ent.x, ent.y) < 150) {
                    ...
                    for(let m of h.members) {
                         if(getDist(m.x, m.y, ent.x, ent.y) < 40) {
                            ...
                            else if (ent.type === 'apple' && (diet === 'herbivore' || diet === 'omnivore')) { aiAte = true; h.xp += 2; } // Bonus XP for apple
                            else if (ent.type === 'rabbit' && (diet === 'carnivore' || diet === 'omnivore')) { aiAte = true; h.xp += 5; } // Bonus XP for rabbit
                            ...
                            if(aiAte) {
                                ...
                                world.entities.splice(i, 1);
                                eaten = true;
                                break; // Stop checking members
                            }
                         }
                    }
                }
                if(eaten) break; // Stop checking herds
            }
        }
        }
    }
    ...
    world.enemyHerds.forEach(enemyHerd => {
        // Simple check: Any player member touching any enemy member?
...
if(collisionFound) {
    // Sound Logic: Only if on screen
    const camX = player.pos.x;
    const camY = player.pos.y;
    const screenDist = getDist(fightX, fightY, camX, camY);
    
    // Assume screen diagonal approx 1500 for safety, or strict canvas size check
    if(screenDist < 1000) {
        playSound('fight_start');
        playSound(`${player.type}-fight`); 
        // Find enemy type from the herd we collided with
        if(enemyHerd.members.length > 0) {
            playSound(`${enemyHerd.members[0].type}-fight`);
        }
    }
    
    // Create Cloud
    const cloud = new FightCloud(fightX, fightY);
    // Suck in nearby units
            ...
            world.clouds.push(cloud);
            // Remove empty enemy herds from world list handled by FightCloud cleanup/kill logic logic usually, 
            // but here we rely on the visual list updating.
            console.log(`[FIGHT STARTED] Player (${player.type} x${player.herd.length}) VS Enemy Herd (${enemyHerd.members[0].type} x${enemyHerd.members.length}) at X:${Math.floor(fightX)} Y:${Math.floor(fightY)}`);
...
// AI vs AI Combat
    for(let i=0; i<world.enemyHerds.length; i++) {
        for(let j=i+1; j<world.enemyHerds.length; j++) {
            ...
            // Fix: Check if already fighting to prevent infinite clouds
            const h1Busy = h1.members.some(m => m.combatState);
            const h2Busy = h2.members.some(m => m.combatState);
            if(h1Busy || h2Busy) continue;

  // FIX: Check if actual members are touching, not just herd centers
            let touching = false;
            ...
            if(touching) {
                 ...
                 // Only add members close to the fight (radius 150)
                 ...
                 world.clouds.push(cloud);
                 console.log(`[FIGHT STARTED] AI Herd A (${h1.members[0].type} x${h1.members.length}) VS AI Herd B (${h2.members[0].type} x${h2.members.length}) at X:${Math.floor(fightPos.x)} Y:${Math.floor(fightPos.y)}`);
    ...
    for(let i=world.projectiles.length-1; i>=0; i--) {
        ...
        // Wall & Tree Collision
        if(checkHardCollision(p.x, p.y, 5)) {
 ...
 // Enemy Collision
        ...
        // 1. Check Player Collision (If projectile belongs to an Enemy)
        if (p.ownerHerd) { 
            // Only check if close to player center to save performance
            if(getDist(p.x, p.y, player.pos.x, player.pos.y) < 300) {
                for(let member of player.herd) {
                    if(getDist(p.x, p.y, member.x, member.y) < 50) {
                        // Create Fight Cloud (Visual only for hit)
                        const cloud = new FightCloud(p.x, p.y);
                        
                        // Deal damage to player animal
                        const dmg = p.isEagle ? p.damage * 2 : p.damage;
                        member.currentHp -= dmg;
                        cloud.addText(`-${Math.floor(dmg)} HP`, p.x, p.y);
                        
                        // Add visual effects
                        if(member.currentHp <= 0) {
                            // Let the main loop cleanup handle death, just mark hit
                            playSound('die');
                        } else {
                            playSound('hit');
                        }
        ...
        if(hit) continue; // Skip enemy check if player was hit

        // 2. Check Enemy Herds
        for(let h of world.enemyHerds) {
            if (p.ownerHerd === h) continue; // <--- ADD THIS LINE (Ignores self-collision)
            
            if(getDist(p.x, p.y, h.center.x, h.center.y) < 100) {
                // Create Fight Cloud with Sparrow
                const cloud = new FightCloud(p.x, p.y);
                
                // Create temporary Sparrow/Eagle animal
                const bird = new Animal(p.type, p.x, p.y, true, null);
                bird.currentHp = p.isEagle ? p.hp * 2 : p.hp;
                
                // Hack: Override getDamage for this specific bird instance
    ...
    // 7. Update Player Herd
    // Tapered Zoom Logic
    const count = player.herd.length;
    let zoomDrop = 0;
    if (count > 3) {
        // First 100 members (after base 3): 0.5% per member
        const stage1 = Math.min(count, 100) - 3;
        zoomDrop += stage1 * 0.005;

        // 100 to 200: 0.25% per member
        if (count > 100) {
            const stage2 = Math.min(count, 200) - 100;
            zoomDrop += stage2 * 0.0025;
        }

        // 200+: 0.125% per member
        if (count > 200) {
            const stage3 = count - 200;
            zoomDrop += stage3 * 0.00125;
        }
    }
    const targetZoom = Math.max(0.1, 1.0 - zoomDrop);
    // Smooth camera zoom
    GAME_STATE.camera.zoom += (targetZoom - GAME_STATE.camera.zoom) * 0.1;

 // Update Enemy Herds
    for(let i = world.enemyHerds.length - 1; i >= 0; i--) {
        let herd = world.enemyHerds[i];

                // GHOST FIX: Strictly remove dead members before checking length
  ...
  // REMOVE HERD IF EMPTY
        if(herd.members.length === 0) {
            ...
            world.clouds.push(cloud); // Push cloud just for the text

            world.enemyHerds.splice(i, 1);
            continue;
        }

  // Passive XP Gain with Difficulty Multiplier
        let difficultyMult = 1.0;
        if(GAME_STATE.difficulty === 'medium') difficultyMult = 1.33; // 33% boost
        if(GAME_STATE.difficulty === 'hard') difficultyMult = 1.75;   // 75% boost
        
        const gain = (0.5 * difficultyMult) * dt;
        herd.xp += gain;
        herd.totalXp += gain; 
        
        // Try to upgrade random stat
        if(herd.xp > 5) {
            ...
            console.log(`[AI UPGRADE] Herd ID: ${i} | Type: ${herd.members.length > 0 ? herd.members[0].type : 'Unknown'} | Upgraded: ${pick.toUpperCase()}`);
        }

        // Reproduction
        let diffBreedMod = 0;
        if(GAME_STATE.difficulty === 'medium') diffBreedMod = 0.05; // +5%
        if(GAME_STATE.difficulty === 'hard') diffBreedMod = 0.10;   // +10%
        ...
        if(herd.birdCooldown > 0) herd.birdCooldown -= dt;
            ...
            if(distToP < 500) { // In range of player
                herd.birdCooldown = 30; // Reset cooldown
// Fire at player
                ...
                world.projectiles.push({
                    ...
                    ownerHerd: herd // <--- ADD THIS LINE (Tags the projectile)
                });
            }
        }

 // AI: Behavior (Advanced Priority System)
        // Priorities: 
        // 1. Flee from Stronger Herds (Player or AI)
        // 2. Chase Weaker Herds (Player or AI)
        // 3. Hunt/Gather Food (XP)
        // 4. Wander

        const awarenessRange = 400; // Radius to see enemies
        const foodRange = 300;      // Radius to smell food
        let mode = 'wander';
        let targetX = null, targetY = null;

        // --- 1. SCAN FOR HERDS (THREATS OR PREY) ---
        let nearestHerd = null;
        let nearestDist = 9999;
        let isStrongerThanTarget = false;

        // Check Player
        const distToP = getDist(herd.center.x, herd.center.y, player.pos.x, player.pos.y);
        if(distToP < awarenessRange) {
            nearestHerd = { pos: player.pos, count: player.herd.length };
            nearestDist = distToP;
        }

        // Check Other Enemy Herds
        world.enemyHerds.forEach(otherH => {
            if(otherH === herd) return; // Don't check self
            const dist = getDist(herd.center.x, herd.center.y, otherH.center.x, otherH.center.y);
            if(dist < awarenessRange && dist < nearestDist) {
                nearestHerd = { pos: otherH.center, count: otherH.members.length };
                nearestDist = dist;
            }
        });
        ...
        if(nearestHerd) {
            // "Stronger" means having more members
            if(herd.members.length > nearestHerd.count) {
                mode = 'chase';
                targetX = nearestHerd.pos.x; 
                targetY = nearestHerd.pos.y;
            } else {
                mode = 'flee';
                // Move in opposite direction of threat
                targetX = herd.center.x - (nearestHerd.pos.x - herd.center.x);
                targetY = herd.center.y - (nearestHerd.pos.y - herd.center.y);
            }
        }

        // --- 2. IF SAFE, SCAN FOR FOOD (XP) ---
        if(mode === 'wander') {
            const myType = herd.members[0].type;
            const diet = STATS[myType].diet;
            
            let nearestFood = null;
            let nearestFoodDist = 9999;

            // Helper to check food validty
            const isValidFood = (ent) => {
                if(diet === 'herbivore' && (ent.type === 'grass' || ent.type === 'apple')) return true;
                if(diet === 'carnivore' && ent.type === 'rabbit') return true;
                if(diet === 'omnivore' && (ent.type === 'grass' || ent.type === 'apple' || ent.type === 'rabbit')) return true;
                return false;
            };

            // Optimization: Only scan entities if we are wandering
            // To prevent lag, we loop entities but skip checking geometry distance if x/y diff is huge
            for(let ent of world.entities) {
                if(!isValidFood(ent)) continue;

                // fast bounding box check before expensive sqrt
                if(Math.abs(ent.x - herd.center.x) > foodRange || Math.abs(ent.y - herd.center.y) > foodRange) continue;

                const d = getDist(herd.center.x, herd.center.y, ent.x, ent.y);
                if(d < foodRange && d < nearestFoodDist) {
                    nearestFoodDist = d;
                    nearestFood = ent;
                }
            }
        ...
        let moveAngle = herd.dir;
        
        if(mode !== 'wander') {
           // Calculate angle towards target
           moveAngle = Math.atan2(targetY - herd.center.y, targetX - herd.center.x);
           herd.timer = 0.5; // Lock direction briefly to prevent jitter
        } else {
            // Standard Wandering
            herd.timer -= dt;
            if(herd.timer <= 0) {
                herd.dir = Math.random() * 6.28;
                herd.timer = Math.random() * 3 + 2;
            }
            moveAngle = herd.dir;
        }
        ...
        let nextX = herd.center.x + Math.cos(moveAngle) * aiSpeed;
        let nextY = herd.center.y + Math.sin(moveAngle) * aiSpeed;

        // Water/Bounds/Tree Collision for AI
        const isAiCroc = (herd.members.length > 0 && herd.members[0].type === 'crocodile');
        if(!checkHardCollision(nextX, nextY, 20, isAiCroc)) {           
             herd.center.x = nextX;
             herd.center.y = nextY;
            herd.dir = moveAngle; // Update stored dir
        } else {
            // Hit wall/water, pick new random direction
            herd.dir = Math.random() * 6.28;
            herd.timer = 0;
        }
    ...
    if(world.enemyHerds.length === 0 && GAME_STATE.screen === 'game') {
        ...
        if(modal.classList.contains('hidden')) {
            // STOP ALL SFX (Safe Version)
            if(GAME_STATE.activeSFX) {
                GAME_STATE.activeSFX.forEach(a => { a.pause(); a.currentTime = 0; });
                GAME_STATE.activeSFX = [];
            }
...
function drawGame(dt) {
    ...
    world.enemyHerds.forEach(herd => {
        ...
        if(herd.members.length > 0) {
            ...
            // Increased range growth: 0.25% per member
            const rangeScale = 1 + (herd.members.length * 0.0025);
    ...
    // Isometric circle: Base 150 + 0.25% per member
    const rangeScale = 1 + (player.herd.length * 0.0025);
 ...
 ctx.restore(); // Restore to Screen Coordinate space (0,0 is top left)
    ...
    // Calculate Scale: 1.0 + 2% per animal
    const uiScale = 1.0 + (player.herd.length * 0.02);
    ...
    if (GAME_STATE.isMobile) {
        textX = drawX;
        textY = drawY;
        ctx.textAlign = "right"; // Right align for corner
    } else {
        textX = drawX - 20;
        textY = drawY;
...
function handleLockedContent(animalType) {
    ...
    // Always show purchase options first
    content.innerHTML = `
...
function checkUnlocks() {
    ...
    document.querySelectorAll('.hero-card').forEach(card => {
        ...
        // Force unlock if Debug is on, or if it's a free animal, or if user owns it
