 ...
 */
// --- DEBUG TOGGLE ---
const TEST_UNLOCK_ALL = true; // Set to true to unlock all animals locally
// --- CONFIGURATION & STATE ---
...
function playSound(name) {
    ...
    try {
        ...
        audio.volume = 0.3; // 40% reduction from 0.5
        audio.play().catch(e => { });
    } catch(err) {
...
window.addEventListener('mousedown', (e) => {
    ...
    if(GAME_STATE.isMobile) {
        ...
        window.addEventListener('touchmove', e => {
            e.preventDefault();
            GAME_STATE.mouse.x = e.touches[0].clientX;
            GAME_STATE.mouse.y = e.touches[0].clientY;
            GAME_STATE.mouse.active = true;
        }, {passive: false});
...
GAME_STATE.lastStartConfig = { heroType };
    ...
    world.width = 5000 * mapPct;
    world.height = 5000 * mapPct;
...
function fireBirdshot() {
    ...
    // Calculate direction relative to mouse (center screen)
...
// AI vs AI Combat
    for(let i=0; i<world.enemyHerds.length; i++) {
        for(let j=i+1; j<world.enemyHerds.length; j++) {
            ...
            if(getDist(h1.center.x, h1.center.y, h2.center.x, h2.center.y) < 100) {
                 const cloud = new FightCloud((h1.center.x+h2.center.x)/2, (h1.center.y+h2.center.y)/2);
                 h1.members.forEach(m => { if(!m.combatState) cloud.addCombatant(m, 'player'); }); 
                 h2.members.forEach(m => { if(!m.combatState) cloud.addCombatant(m, 'enemy'); });
                 world.clouds.push(cloud);
            }
        }
    }
 ...
 // Update Enemy Herds
  ...
  // REMOVE HERD IF EMPTY
        ...
        // Reproduction
        const breedPct = 0.10 + (herd.upgrades.breed.level * 0.005);
        ...
        // AI: Behavior (Chase/Flee)
        // Check Player
        const distToPlayer = getDist(herd.center.x, herd.center.y, player.pos.x, player.pos.y);
        const range = 150 * 2.5; // Awareness range
        let targetX = null, targetY = null;
        let mode = 'wander';

        // Check against Player
        if(distToPlayer < range) {
            if(player.herd.length < herd.members.length) {
                // Chase
                mode = 'chase';
                targetX = player.pos.x; targetY = player.pos.y;
            } else if (player.herd.length > herd.members.length) {
                // Flee
                mode = 'flee';
                targetX = herd.center.x - (player.pos.x - herd.center.x);
                targetY = herd.center.y - (player.pos.y - herd.center.y);
            }
        }
        
        // Simple Physics Movement based on mode
        let moveAngle = herd.dir;
        
        if(mode !== 'wander') {
           moveAngle = Math.atan2(targetY - herd.center.y, targetX - herd.center.x);
           herd.timer = 0.5; // Lock direction briefly
        } else {
            herd.timer -= dt;
            if(herd.timer <= 0) {
...
function drawGame(dt) {
    ...
    // --- NEW UI DRAWING (After animals so it stays on top) ---

// Calculate Scale: 1.0 + 2% per animal
const uiScale = 1.0 + (player.herd.length * 0.02);

// 1. Draw Flag (Left of the counter) - 50% Smaller
// SAFETY CHECK: Ensure flag image is valid
if(player.flagImage && player.flagImage.complete && player.flagImage.naturalWidth !== 0) {
    const fw = 50 * uiScale; 
    const fh = 30 * uiScale;
    const flagX = player.pos.x - 20 - fw - 10; 
    const flagY = player.pos.y - 80 - fh + 5; 
...
// Color changes based on size (Green to Red)
const hue = Math.min(120, player.herd.length * 5); 
ctx.fillStyle = `hsl(${hue}, 100%, 50%)`;
ctx.shadowColor = "black";
ctx.shadowBlur = 4;
ctx.lineWidth = 1;
ctx.strokeText(player.herd.length, player.pos.x - 20, player.pos.y - 80); // Outline
ctx.fillText(player.herd.length, player.pos.x - 20, player.pos.y - 80);   // Fill

// Reset shadow for other drawings
ctx.shadowBlur = 0;

    ctx.restore(); // Restore to UI Coordinate space (or rather, exit camera transform)
...
function updateRosterUI() {
    const rosterDiv = document.getElementById('roster-panel');
