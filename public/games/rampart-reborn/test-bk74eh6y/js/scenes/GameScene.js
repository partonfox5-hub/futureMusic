...
export class GameScene extends Phaser.Scene {
  ...
  async _lobbyCreate() {
    ...
    if (ok) {
      this._setLobbyStatus(
        'Room open — copy the full code, keep this tab open, wait for opponent'
      );
    } else {
      this._setLobbyStatus('Could not create room — retry (PeerJS cloud may be busy)');
    }
  }
  ...
  async _lobbyJoin() {
    ...
    this._updateLobbyPlayers();
    if (ok && this.mp.connected) {
      this._enableLobbyReady(true);
      this._unlockLobbyChat(true);
      this._setLobbyStatus('Joined — when both names show, press Ready');
    } else if (!ok) {
      this._setLobbyStatus(
        'Join failed — use the host’s full code, same game URL, host still waiting'
      );
    }
  }
