...
import { attachVrHands } from "/games/fenrest/test-n2q8k4wm/js/vr-hands.js?v=hit1";
import { heightAt as gridHeight, inSettlement, createChunkManager, snapToGround } from "/games/fenrest/test-n2q8k4wm/js/world-grid.js?v=hit1";
import { createMagic } from "/games/fenrest/test-n2q8k4wm/js/magic.js?v=hit1";
import { installLife } from "/games/fenrest/test-n2q8k4wm/js/life.js?v=hit1";
import { bindLiving, hurtLiving, tickLiving, keepOutOfSafe, inSafeRegion, regionOf } from "/games/fenrest/test-n2q8k4wm/js/living.js?v=hit1";

const TOWNS = [
  { id: "fenrest", name: "Fenrest", x: 0, z: 0, r: 48, style: "thatch", econ: "farms", skip: true },
...
const ENEMY_KIND = [
  ...
  { id: "raptor", hp: 8, spd: 4.4, dmg: 0.09, sprite: "raptor.png", pack: [2, 3], rare: 0.2 },
  { id: "dragon", hp: 16, spd: 3.4, dmg: 0.18, sprite: "dragon.png", pack: [1, 1], rare: 0.45 },
  { id: "trex", hp: 22, spd: 2.2, dmg: 0.16, sprite: "trex.png", pack: [1, 1], rare: 0.55 },
  { id: "mammoth", hp: 28, spd: 1.6, dmg: 0.12, sprite: "mammoth.png", pack: [1, 1], rare: 0.5 },
];
...
function spawnPack(root, list, px, pz) {
  if (inSafeRegion(px, pz) || inTown(px, pz)) return 0;
  const region = regionOf(px, pz);
  let pool = ENEMY_KIND.filter((k) => Math.random() > k.rare);
  if (region && region.id !== "wild") {
    if (!region.hostiles.length) return 0;
    const wanted = ENEMY_KIND.filter((k) => region.hostiles.includes(k.id));
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (inTown(x, z) || inSafeRegion(x, z)) continue;
    const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: enemySprite(def.sprite), transparent: true, color: 0xffffff }));
    sp.scale.set(def.id === "spider" ? 1.6 : def.id === "basilisk" ? 2.2 : def.id === "alien" ? 2.4 : def.id === "trex" ? 3.2 : def.id === "mammoth" ? 3.0 : 1.2, def.id === "spider" ? 1.1 : def.id === "trex" ? 2.8 : 2.1, 1);
    ...
    bindLiving(sp, { hp: def.hp, hostile: true });
    root.add(sp);
    list.push(sp);
...
function tickFoes(list, dt, store, bolts, root) {
  ...
  const px = tmp.x;
  const pz = tmp.z;
  const playerSafe = inSafeRegion(px, pz);
  for (const f of list) {
    try {
      const u = f.userData || {};
      if (u.lifeBeast || u.npc || u.kind === "turtle") continue;
      tickLiving(f, dt, heightAt);
      keepOutOfSafe(f, dt);
      if (u.dead || u.dying || u.stagger > 0) continue;
      if (u.frozen > 0) {
        u.frozen -= dt;
        continue;
      }
      ...
      if (playerSafe || inSafeRegion(f.position.x, f.position.z)) {
        f.position.x -= (dx / d) * 1.4 * dt;
        f.position.z -= (dz / d) * 1.4 * dt;
        continue;
      }
      ...
      for (const b of bolts) {
        if (b.userData.life > 0 && f.position.distanceTo(b.userData.hit) < 2.4) {
          hurtLiving(f, 4, { x: (Math.random() - 0.5) * 2, z: (Math.random() - 0.5) * 2 }, root);
        }
      }
    } catch (err) {
  ...
  function loop(now) {
    ...
    if (cam) {
      ...
      if (encounterIn <= 0) {
        ...
        if (!inTown(tmp.x, tmp.z) && !inSafeRegion(tmp.x, tmp.z) && Math.random() < 0.38) {
          const n = spawnPack(root, foes, tmp.x, tmp.z);
          if (n) store?.getState?.().setHud?.({ prompt: "Something moves in the grass." });
        }
      }
      ...
      if (vr && trig && !trigPrev && hasThunder(store) && (st.mp ?? 1) > 0.12) {
        ...
        foes.forEach((f) => {
          if (!f.userData?.dead && f.position.distanceTo(hit) < 3.2) {
            hurtLiving(f, 6, { x: (Math.random() - 0.5) * 3, z: (Math.random() - 0.5) * 3 }, root);
          }
        });
      }
      trigPrev = trig;
