...
import { heightAt, raiseTerrain, inSettlement } from "/games/fenrest/test-n2q8k4wm/js/world-grid.js?v=hit1";
import { bindLiving, hurtLiving, tickLiving, inSafeRegion } from "/games/fenrest/test-n2q8k4wm/js/living.js?v=hit1";
...
export function spawnHsUnit(root, list, def, x, z, friendly) {
  ...
  bindLiving(sp, { hp: def.hp, hostile: !friendly, friendly: !!friendly });
  root.add(sp);
  list.push(sp);
...
export function createMagic({ root, foes, allies, store, gl, cam }) {
  ...
  function tick(dt, gl) {
    ...
    for (const u of allies) {
      tickLiving(u, dt, heightAt);
      if (u.userData.dead || u.userData.dying || u.userData.stagger > 0) continue;
  ...
  function seedEvils() {
    ...
    spots.forEach(([x, z], i) => {
      if (inSettlement(x, z) || inSafeRegion(x, z)) return;
