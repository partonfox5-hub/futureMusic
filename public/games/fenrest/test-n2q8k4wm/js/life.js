...
import { heightAt, inSettlement, CHUNK } from "/games/fenrest/test-n2q8k4wm/js/world-grid.js?v=hit1";
import {
  bindLiving,
  tickLiving,
  tickSouls,
  ...
  REGIONS,
  markRegions,
} from "/games/fenrest/test-n2q8k4wm/js/living.js?v=hit1";
...
export function installLife({ root, store, loot, foes, heightAt: hAt }) {
  ...
  function spawnTurtle(x, z) {
    ...
    bindLiving(s, { hp: 2, hostile: false });
    turtles.push(s);
    foes.push(s);
    hitables.push(s);
    return s;
  ...
  function stompTurtle(t, head) {
    if (!t.visible || t.userData.dead || t.userData.dying || t.userData.kind !== "turtle") return;
    t.visible = false;
  ...
  function spawnBeast(kind, x, z, parent) {
    ...
    foes.push(s);
    hitables.push(s);
    bindLiving(s, { hp: def.hp, hostile: true });
    return s;
  }
  ...
  function spawnNpc(def, x, z) {
    ...
    s.position.set(x, height(x, z) + 2.05 * 0.5, z);
    ...
    bindLiving(s, { hp: 10, friendly: true });
    root.add(s);
    npcs.push(s);
    hitables.push(s);
    return s;
  ...
  for (let i = 0; i < 6; i++) {
    const a = i * 1.05;
    spawnTurtle(Math.cos(a) * 52, Math.sin(a) * 52 + 8);
  }
  const wolves = REGIONS.find((r) => r.id === "wolves");
  const rapt = REGIONS.find((r) => r.id === "raptors");
  const mega = REGIONS.find((r) => r.id === "megafauna");
  const drags = REGIONS.find((r) => r.id === "dragons");
  spawnBeast("wolf", wolves.x + 8, wolves.z - 6);
  spawnBeast("wolf", wolves.x - 10, wolves.z + 12);
  spawnBeast("wolf", wolves.x + 16, wolves.z + 8);
  spawnBeast("wolf", wolves.x - 4, wolves.z - 14);
  spawnBeast("raptor", rapt.x + 6, rapt.z + 4);
  spawnBeast("raptor", rapt.x - 8, rapt.z - 6);
  spawnBeast("raptor", rapt.x + 12, rapt.z - 10);
  spawnBeast("trex", mega.x + 10, mega.z - 8);
  spawnBeast("trex", mega.x - 14, mega.z + 6);
  spawnBeast("mammoth", mega.x + 4, mega.z + 16);
  spawnBeast("mammoth", mega.x - 18, mega.z - 12);
  spawnBeast("raptor", mega.x + 20, mega.z + 4);
  spawnBeast("dragon", drags.x + 12, drags.z - 10);
  spawnBeast("dragon", drags.x - 16, drags.z + 14);
  spawnBeast("trex", drags.x + 28, drags.z + 8);
  for (let i = 0; i < 10; i++) {
  ...
  function tick(dt, ctx) {
    ...
    for (const tu of turtles) {
      tickLiving(tu, dt, height);
      if (tu.userData.dead || tu.userData.dying || tu.userData.stagger > 0) continue;
    ...
    for (const b of beasts) {
      if (!b.parent && !b.userData.dying) continue;
      tickLiving(b, dt, height);
      keepOutOfSafe(b, dt);
      const u = b.userData;
      ...
      const d = Math.hypot(dx, dz) || 1;
      if (inSafeRegion(head.x, head.z)) {
        u.wander += dt * 0.4;
        b.position.x += Math.cos(u.wander) * 0.4 * dt;
        b.position.z += Math.sin(u.wander) * 0.4 * dt;
      } else if (d < u.aggro) {
    ...
    for (const n of npcs) {
      if (!n.parent && !n.userData.dying) continue;
      tickLiving(n, dt, height);
      const nu = n.userData;
      if (nu.dead || nu.dying || nu.stagger > 0) continue;
