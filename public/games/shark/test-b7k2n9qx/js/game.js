...
hatchLever.position.set(-0.46, -0.38, -0.48);
cockpit.add(hatchLever);
let hatchKnob = null;
let exteriorHatch = null;
(function buildHatchLever() {
  ...
  const knob = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xe0c040, emissive: 0xaa6600, metalness: 0.4, emissiveIntensity: 0.8 })
  );
...
function paintVrPlaque(ctx, w, h) {
  ...
  const lines = [
    ...
    "POKE the YELLOW lever to exit.",
    "OUTSIDE: flashlight is on your hands.",
    "Poke the GOLD hatch on the hull to re-enter.",
    "Hold TRIGGER on a crack to weld.",
...
(function buildExterior() {
  ...
  const hatch = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xc8a020, metalness: 0.55, roughness: 0.4, emissive: 0x664400, emissiveIntensity: 0.7 })
  );
  hatch.position.set(0, -0.62, 0.55);
  hatch.userData.extHatch = true;
  exterior.add(hatch);
  exteriorHatch = hatch;
...
function applyWorldMode(m) {
  ...
  bubbles.material.opacity = space ? 0.8 : 0.22;
  bubbles.material.size = space ? 0.07 : 0.1;
  floor.material.color.setHex(space ? 0x08080c : 0x0a0908);
  cellWalls.traverse((o) => {
    if (o.material && o.material.color) o.material.color.setHex(space ? 0x101828 : 0x081410);
  });
  cellHelper.material.color.setHex(space ? 0x334466 : 0x0c221c);
  fill.color.setHex(space ? 0x445577 : 0x3a6a52);
  ambient.color.setHex(space ? 0x101018 : 0x061018);
  hemi.color.setHex(space ? 0x1a2840 : 0x0c1c28);
...
function applyAtmosphere() {
  ...
  if (space) {
    ...
    scene.fog = new THREE.FogExp2(0x000003, vr ? 0.007 : 0.0022);
    ambient.intensity = vr ? 0.05 : 0.22;
    hemi.intensity = vr ? 0.07 : 0.22;
    renderer.toneMappingExposure = vr ? 0.48 : 0.62;
    lamp.intensity = vr ? 14 : 48;
    lamp.distance = vr ? 28 : 78;
  } else {
    scene.background = new THREE.Color(vr ? 0x000203 : 0x010308);
    scene.fog = new THREE.FogExp2(vr ? 0x02090e : 0x031018, vr ? 0.11 : 0.024);
    ambient.intensity = vr ? 0.025 : 0.16;
    hemi.intensity = vr ? 0.04 : 0.18;
    renderer.toneMappingExposure = vr ? 0.42 : 0.82;
    lamp.intensity = vr ? 12 : 40;
    lamp.distance = vr ? 16 : 70;
    lamp.angle = vr ? 0.38 : 0.46;
  }
  const cabin = !vr || mode === "pilot";
  fill.intensity = cabin ? (vr ? 1.15 : 2.2) : 0.12;
  dashLamp.intensity = cabin ? (vr ? 1.25 : 1.6) : 0.08;
  if (typeof helm !== "undefined" && helm) helm.visible = false;
}
...
function makeFlash() {
  const g = new THREE.Group();
  const light = new THREE.SpotLight(0xd8ecff, 0, 14, 0.33, 0.6, 1.2);
  ...
  const lens = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xf2f7ff, transparent: true, opacity: 0, depthWrite: false })
  );
  lens.position.z = -0.07;
  g.add(lens);
  const cone = new THREE.Mesh(
    new THREE.ConeGeometry(0.16, 0.7, 12, 1, true),
    new THREE.MeshBasicMaterial({ color: 0x9cc4e8, transparent: true, opacity: 0, depthWrite: false, side: THREE.DoubleSide })
  );
  cone.rotation.x = -Math.PI / 2;
  cone.position.z = -0.4;
  g.add(cone);
  g.userData = { light, lens, cone };
  return g;
}
const flash0 = makeFlash();
const flash1 = makeFlash();
ctrl0.add(flash0);
ctrl1.add(flash1);

function tickFlashlights() {
  const eva = xrOn && mode === "eva";
  const boost = xrTrigger;
  [flash0, flash1].forEach((f) => {
    const on = eva;
    f.userData.light.intensity = on ? (boost ? 16 : 8.5) : 0;
    f.userData.light.distance = on ? (boost ? 18 : 12) : 1;
    f.userData.lens.material.opacity = on ? (boost ? 1 : 0.75) : 0;
    f.userData.cone.material.opacity = on ? (boost ? 0.11 : 0.055) : 0;
  });
}
...
function enterEva() {
  ...
  applyAtmosphere();
  setMsg(xrOn ? "EVA — FLASHLIGHT ON  ·  POKE GOLD HATCH TO RE-ENTER" : "EVA — HOLD R NEAR THE HULL TO PATCH", 3);
}
...
function readXrMove(dt, move) {
  ...
  if (mode === "pilot") {
    ...
    const hatchHit = !!nearWorld(hatchKnob || hatchLever, 0.28);
    if (hatchHit && xrHatchArmed) {
      xrHatchArmed = false;
      tryToggleEva();
    }
    if (!hatchHit) xrHatchArmed = true;
  } else {
    xrGrab = null;
    helmHeld = false;
    helmHands.forEach((h) => {
    ...
    paintHelmHeld(false);
    const nearExt = !!(exteriorHatch && nearWorld(exteriorHatch, 0.55));
    const nearHull = evaDummy.position.distanceTo(sub.position) < 2.6 && !!nearWorld(sub, 1.8);
    const reenter = nearExt || (nearHull && (xrTrigger || xrGrip));
    if (reenter && xrHatchArmed) {
      xrHatchArmed = false;
      tryToggleEva();
    }
    if (!reenter) xrHatchArmed = true;
  }
}
  ...
  if (mode === "pilot") {
    ...
    if (welding) {
      ...
      if (repairHold > 3) {
        ...
        crack.visible = false;
        repairHold = 0;
        setMsg(xrOn ? "HULL SEALED — POKE THE GOLD HATCH TO RE-ENTER" : "HULL SEALED — F TO RE-ENTER", 2);
...
renderer.xr.addEventListener("sessionstart", () => {
  ...
  kickRadio();
  applyAtmosphere();
  setMsg("TOUCH THE WHEEL · POKE YELLOW LEVER TO EXIT", 3.2);
});
renderer.xr.addEventListener("sessionend", () => {
  ...
  torch0.visible = false;
  torch1.visible = false;
  flash0.userData.light.intensity = 0;
  flash1.userData.light.intensity = 0;
  applyAtmosphere();
  if (cam.parent) cam.parent.remove(cam);
  if (mode === "eva") {
    evaDummy.add(cam);
