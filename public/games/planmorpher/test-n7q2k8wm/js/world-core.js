...
/** Lower-atmosphere crust shell: crossing this swaps orbital pegs for full surface LOD. */
export const LOWER_ATMOS = ATMOS;
/** Thicker outer sky volume for god-game zoom (Planmorpher). RTS keeps ATMOS. */
...
/** True once a view point is inside the lower-atmosphere sphere (crust detail + full creatures). */
export function insideLowerAtmos(planet, dist, slack = 0.08) {
  if (!planet) return false;
  return dist < atmosR(planet, LOWER_ATMOS) + slack;
}

/** True inside the outer god-sky / visible blue shell. */
export function insideOuterAtmos(planet, dist, slack = 0.08) {
  if (!planet) return false;
  const mul = planet.atmosMul || GOD_ATMOS;
  return dist < atmosR(planet, mul) + slack;
}
...
function setPegVisible(obj, showPeg) {
  if (!obj) return;
  const peg = obj.userData?.peg;
  if (peg) peg.visible = showPeg;
}
...
export function setSurfaceLod(world, on) {
  ...
  for (const c of world.life || []) {
    setPegVisible(c, !on);
    c.visible = on;
    if (c.userData) c.userData.surfaceOn = on;
  }
  ...
  const lower = world.atmosShell?.userData?.lower;
  if (lower?.material) {
    lower.material.opacity = on ? 0.04 : 0.22;
    lower.material.color.setHex(on ? 0xffe27a : 0xd4af37);
  }
}
...
void main(){
  ...
  if(uDetail > 0.5){
    ...
    float fine = noise(pn*96.0);
    dirt *= 0.82 + grit*0.28;
    rock *= 0.86 + grit*0.22;
    float blade = step(0.55, noise(pn*70.0)) * (1.0-uArid) * (1.0-uIce);
    dirt = mix(dirt, uVeg*1.35, blade*0.55);
    dirt = mix(dirt, uVeg*0.62, step(0.7, fine)*0.28*(1.0-uArid));
  }
...
export function makeAtmosShell(radius, worldRef, mul = ATMOS) {
  ...
  const lower = new THREE.Mesh(
    new THREE.SphereGeometry(LOWER_ATMOS, 20, 14),
  ...
  lower.scale.setScalar(radius);
  lower.renderOrder = 4;
  lower.userData.kind = "lowerAtmos";
  g.add(lower);
