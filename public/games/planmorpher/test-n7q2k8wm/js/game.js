...
function enterSurface(w) {
  ...
  sun.intensity = 1.25;
  w.ensureDetail();
  const dist = minDistToWorld(w);
  setSurfaceLod(w, insideLowerAtmos(w, dist));
  setAtmosMode(w, true);
  headPos();
  _v.copy(_v3);
  ...
  hudHint.textContent = "Inside " + w.template.name + "'s sky. Cross the gold lower-atmosphere shell for surface detail and full creatures. Press X to return to the galaxy.";
...
function considerDist(obj, center, best) {
  if (!obj) return best;
  if (obj.isVector3) return Math.min(best, obj.distanceTo(center));
  obj.updateMatrixWorld?.(true);
  obj.getWorldPosition(_v);
  return Math.min(best, _v.distanceTo(center));
}

/** Closest of rig, scene camera, and XR eye cameras — WebXR ArrayCamera pose is not always under the fly rig. */
function minDistToWorld(w) {
  const c = w.group.position;
  let d = 1e9;
  d = considerDist(camera, c, d);
  d = considerDist(rig, c, d);
  d = considerDist(rig.position, c, d);
  if (renderer.xr.isPresenting) {
    const xrCam = renderer.xr.getCamera();
    d = considerDist(xrCam, c, d);
    const cams = xrCam?.cameras;
    if (cams) {
      for (let i = 0; i < cams.length; i++) d = considerDist(cams[i], c, d);
    }
  }
  return d;
}
...
function headPos() {
  ...
  if (renderer.xr.isPresenting) {
    ...
    if (xrCam) {
      ...
      // Prefer the pose that is actually near the fly rig (the view we steer).
      const rigD = _v3.distanceTo(rig.position);
      const xrD = _v.distanceTo(rig.position);
      if (xrD < rigD + 0.35) _v3.copy(_v);
    }
  }
  return _v3;
}

function lookDir(into) {
  const out = into || _v;
  camera.updateMatrixWorld(true);
  ...
  if (renderer.xr.isPresenting) {
    ...
    if (xrCam) {
      xrCam.updateMatrixWorld(true);
      xrCam.getWorldDirection(out);
    }
  }
  return out;
}

function syncWorldLod(w, dist) {
  if (!w || !w.born) return;
  const inLower = insideLowerAtmos(w, dist);
  if (w.surfaceLod !== inLower) setSurfaceLod(w, inLower);
  if (inLower) w.ensureDetail();
}
...
function loop(t) {
  ...
  let near = null;
  let nd = 1e9;
  let insideOuter = null;
  for (const w of worlds) {
    if (!w.born || w === growing) continue;
    const d = minDistToWorld(w);
    syncWorldLod(w, d);
    if (d < nd) {
      nd = d;
      near = w;
    }
    if (!insideOuter && insideOuterAtmos(w, d)) insideOuter = w;
  }
  if (!lockedWorld) {
    if (insideOuter) {
      enterSurface(insideOuter);
    } else if (near) {
      const shell = atmosR(near, GOD_ATMOS);
      const warnR = shell * 1.85 + 0.6;
      if (nd < warnR) {
        hudHint.textContent = "Approaching " + near.template.name + " — cross the gold lower-atmosphere cage for surface detail and full creatures.";
      }
    }
  } else {
    confineToAtmos(rig, head, lockedWorld, GOD_ATMOS);
    const dLock = minDistToWorld(lockedWorld);
    syncWorldLod(lockedWorld, dLock);
    setAtmosMode(lockedWorld, true);
  ...
  if (lockedWorld) {
    hudEl.classList.add("atmo");
    const dLock = minDistToWorld(lockedWorld);
    const detailed = insideLowerAtmos(lockedWorld, dLock);
    hudStatus.textContent = detailed
      ? "LOWER ATMOS · " + lockedWorld.template.name + " · " + lives + " lives · full models · X leaves"
      : "UPPER SKY · " + lockedWorld.template.name + " · " + lives + " lives · orbital pegs · descend through the gold cage";
    hudHint.textContent = detailed
      ? "Lower atmosphere of " + lockedWorld.template.name + ". Surface detail and complete spore creatures are on the crust. Press X to leave."
      : "Upper sky of " + lockedWorld.template.name + ". Descend through the gold lower-atmosphere shell to load vegetation and full creatures.";
  } else {
    hudEl.classList.remove("atmo");
...
window.__pm = {
  ...
  leaveSurface,
  minDistToWorld,
  insideLower: (w) => insideLowerAtmos(w, minDistToWorld(w)),
  rig,
  camera,
  scene,
  renderer,
