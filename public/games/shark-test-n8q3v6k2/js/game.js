...
const ASSET = "/games/shark-test-n8q3v6k2";
const CELL = 156;
const FLOOR = 0;
const CEIL = 92;
...
function scareMaterials(root) {
  ...
  root.traverse((o) => {
    ...
    list.forEach((m) => {
      if (!m) return;
      m.roughness = Math.min(0.78, m.roughness == null ? 0.55 : m.roughness);
      m.metalness = Math.min(0.12, m.metalness || 0);
      if (m.color) m.color.offsetHSL(0.0, 0.06, -0.04);
      const eye = /eye/i.test(m.name || "") || /eye/i.test(o.name || "");
      if (eye) {
        m.emissive = new THREE.Color(0xff1408);
        m.emissiveIntensity = 0.85;
        m.color = new THREE.Color(0x220000);
      }
      skins.push(m);
    });
  });
...
function layFishFlat(scene) {
  const box = new THREE.Box3().setFromObject(scene);
  const size = box.getSize(new THREE.Vector3());
  if (size.y >= size.x && size.y >= size.z) scene.rotation.x = -Math.PI / 2;
  else if (size.x >= size.z) scene.rotation.y = Math.PI / 2;
  else scene.rotation.y = Math.PI;
  scene.updateMatrixWorld(true);
}

function mountFish(gltf, targetLen) {
  const hasAnim = !!(gltf.animations && gltf.animations.length);
  const cloned = hasAnim ? cloneSkinned(gltf.scene) : gltf.scene.clone(true);
  ...
  inner.scale.setScalar(targetLen / len);
  const wrap = new THREE.Group();
  wrap.add(inner);
  wrap.userData.inner = inner;
  wrap.userData.skins = scareMaterials(wrap);
  wrap.userData.hitR = targetLen * 0.22;
  wrap.userData.torpR = targetLen * 0.3;
  wrap.userData.skin = wrap.userData.skins[0] || null;
  if (hasAnim) {
    ...
    const swim = actions.Swimming_Normal || actions.Swim || Object.values(actions)[0];
    if (swim) {
      swim.play();
      wrap.userData.clip = swim;
    }
  } else {
    const list = [];
    wrap.traverse((o) => {
      ...
      [].concat(o.material).forEach((m) => {
        list.push(attachSwim(m, { amp: 0.045, freq: 22, speed: 7.4, lat: 1, vert: 0.18 }));
      });
    });
    wrap.userData.swimList = list;
  }
  const glow = new THREE.PointLight(0xff2208, 0, targetLen * 0.55);
  glow.position.set(0, 0.4, -targetLen * 0.28);
  wrap.add(glow);
  wrap.userData.glow = glow;
  return wrap;
}

function playFish(wrap, name, speed) {
  const acts = wrap.userData && wrap.userData.actions;
  if (!acts) return;
  const a = acts[name];
  if (!a) return;
  if (speed != null) a.timeScale = speed;
  if (wrap.userData.clip === a) return;
  if (wrap.userData.clip) wrap.userData.clip.fadeOut(0.22);
  a.reset().fadeIn(0.22).play();
  wrap.userData.clip = a;
}

function tickFish(wrap, dt, nowt, ampMul) {
  if (wrap.userData.mixer) wrap.userData.mixer.update(dt);
  tickSwim(wrap, nowt, ampMul);
  if (!wrap.userData.mixer && wrap.userData.inner) {
    wrap.userData.inner.rotation.y = Math.sin(nowt * 4.4) * (0.08 + (ampMul || 1) * 0.06);
  }
}
...
function paintVrPlaque(ctx, w, h) {
  ...
  const lines = [
    "SQUEEZE GRIP the wheel and TURN to steer.",
    "LEFT stick  —  accelerate / reverse",
    "RIGHT stick up/down  —  depth",
...
shark.userData.hitR = 22;
shark.userData.torpR = 30;
scene.add(shark);
const pack = [];
const fallbackShark = makeSharkGeom();
fallbackShark.scale.setScalar(5.4);
shark.add(fallbackShark);

function adoptFish(host, fish) {
  while (host.children.length) host.remove(host.children[0]);
  host.add(fish);
  host.userData = Object.assign(host.userData, fish.userData);
  host.userData.body = fish;
}

loadGltf(ASSET + "/models/goblin-shark.glb")
  .then((gltf) => {
    adoptFish(shark, mountFish(gltf, 108));
    const extra = mountFish(gltf, 78);
    extra.position.set(-CELL * 0.88, 18, CELL * 0.4);
    extra.userData.face = 2;
    extra.userData.phase = 1.7;
    scene.add(extra);
    pack.push(extra);
    setSharkMood();
  })
  .catch((err) => {
    console.warn("[shark] goblin glb failed", err);
    fallbackShark.scale.setScalar(5.4);
  });

loadGltf(ASSET + "/models/great-white.glb")
  .then((gltf) => {
    const leviathan = mountFish(gltf, 92);
    leviathan.position.set(CELL * 0.2, 28, -CELL * 0.9);
    leviathan.userData.face = 3;
    leviathan.userData.phase = 0.4;
    scene.add(leviathan);
    pack.push(leviathan);
  })
  .catch((err) => console.warn("[shark] great white glb failed", err));
...
function setSharkMood() {
  ...
  const skins = shark.userData.skins || (shark.userData.skin ? [shark.userData.skin] : []);
  skins.forEach((m) => {
    if (m && m.emissive && !/eye/i.test(m.name || "")) m.emissive.setHex(em);
  });
  if (shark.userData.glow) {
    shark.userData.glow.intensity = sharkState === "attack" ? 5.5 : sharkState === "hunt" ? 2.2 : 0.15;
  }
  if (sharkState === "attack") playFish(shark, "Attack", 1.15);
  else if (sharkState === "hunt") playFish(shark, "Swimming_Fast", 1.35);
  else if (sharkState === "retreat") playFish(shark, "Swimming_Impulse", 1.2);
  else playFish(shark, "Swimming_Normal", 0.95);
}
...
function sharkAi(dt) {
  ...
  if (sharkState === "patrol" || sharkState === "retreat") {
    ...
    if (shark.position.distanceTo(prey) < 36) {
      sharkState = "attack";
      setSharkMood();
    ...
    want = tmp.copy(prey);
    want.y += 0.4;
    if (shark.position.distanceTo(prey) < (shark.userData.hitR || 22)) {
      if (mode === "eva") {
        burst(prey, 16);
  ...
  tickFish(shark, dt, nowt, sharkState === "attack" ? 1.55 : sharkState === "hunt" ? 1.2 : 1);
  pack.forEach((p, i) => {
    const face = (p.userData.face ?? i) & 3;
    const wantP = wallPoint(face, t * 0.7 + (p.userData.phase || 0), 14 + i * 9);
    tmp2.copy(wantP).sub(p.position);
    const d = tmp2.length();
    if (d > 0.2) {
      tmp3.copy(p.position).add(tmp2);
      p.lookAt(tmp3);
      p.position.add(tmp2.multiplyScalar(Math.min(1, (7.2 * dt) / d)));
    }
    playFish(p, "Swimming_Normal", 0.85);
    tickFish(p, dt, nowt + i, 1);
  });
}
...
function updateTorps(dt) {
  ...
  torps.forEach((t) => {
    ...
    let hit = false;
    if (t.position.distanceTo(shark.position) < (shark.userData.torpR || 28)) {
      t.userData.life = 0;
      t.visible = false;
...
function drawMap() {
  ...
  g.fill();
  g.restore();
  const blips = [shark.position, ...pack.map((p) => p.position), ...whales.map((w) => w.position)];
  g.fillStyle = "#e8d27a";
  blips.forEach((p) => {
...
function readXrMove(dt, move) {
  ...
  if (mode === "pilot") {
    ...
    if (xrGrip && hand) {
      ...
      wheel.rotation.x = clamp(dy * 0.9, -0.35, 0.35);
    } else {
      xrGrab = null;
    }
    if (session) {
...
window.__controlsTest = {
  ...
  getShark: () => sharkState,
  getSharkPos: () => ({ x: shark.position.x, y: shark.position.y, z: shark.position.z }),
  setSharkPos: (x, y, z) => shark.position.set(x, y, z),
  lookSharkAtSub: () => {
    shark.position.set(0, 20, -26);
    shark.lookAt(sub.position);
  },
  setYaw: (v) => {
    yaw = v;
    lookYaw = v;
