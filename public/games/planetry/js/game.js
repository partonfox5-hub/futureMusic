...
const HP = { scout: 40, settler: 55, miner: 35, tank: 70, heli: 50, warship: 200, cannon: 120, dome: 80, tower: 140 };
const START_MAT = 500;
...
function sit(mesh, planet, n, alt) {
  mesh.position.copy(planet.group.position).addScaledVector(n, planet.radius * alt);
...
class World {
  constructor(pos, radius, seed) {
    ...
    this.atmos = new THREE.Mesh(
      new THREE.SphereGeometry(ATMOS, 24, 16),
      new THREE.MeshBasicMaterial({ color: 0x7ec8ff, transparent: true, opacity: 0.07, side: THREE.BackSide, depthWrite: false }),
    );
  ...
  stage(dt) {
    ...
    this.atmos.material.opacity = 0.05 + land * 0.05;
    if (t >= 1 && !this.seededMines) this.seedMines();
  }
  seedMines() {
    this.seededMines = true;
    const n = 2 + ((this.seed * 3) % 3 | 0);
    for (let i = 0; i < n; i++) {
      const nor = new THREE.Vector3().randomDirection();
...
function seedGalaxy() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const rad = 0.45 + Math.random() * 1.15;
    const w = new World(new THREE.Vector3(Math.cos(a) * r, y, Math.sin(a) * r - 2), rad, Math.random() * 90);
    scene.add(w.group);
    worlds.push(w);
  }
}
...
function moveUnit(u, dt) {
  ...
  if (u.cmd && u.cmd.pos) {
    ...
    if (u.kind === "settler" && u.cmd.planet) {
      ...
      if (u.mesh.position.distanceTo(target) < 0.18 && pl.mature()) {
...
function tickMiner(u, dt) {
  ...
  if (u.state === "mine" && u.cmd?.mine) {
    ...
    if (u.cargo >= 10) {
      const col = pl.colonies.find((c) => c.team === u.team);
      if (col) {
...
function aiTick(dt) {
  for (const t of teams) {
    ...
    const empty = worlds.find((w) => w.mature() && !w.colonies.some((c) => c.team === t.id) && w.colonies.length < 2);
