...
const xrStickL = { x: 0, y: 0 };
const xrStickR = { x: 0, y: 0 };
let weldEmitT = 0;
...
function paintVrPlaque(ctx, w, h) {
  ...
  const lines = [
    "SQUEEZE GRIP on the wheel to STEER.",
    "  turn the wheel  ·  lift / press for depth",
    "BOTH THUMBSTICKS = thrust (port + stbd).",
    "  both forward = go the way the nose points",
    "  both back = reverse. One stick = half power.",
    "PUSH the red button to fire torpedoes.",
    "YELLOW lever — hatch (exit / re-enter).",
    "OUTSIDE: hold TRIGGER near the hull to weld.",
  ];
...
(function buildExterior() {
  const hullMat = new THREE.MeshStandardMaterial({ color: 0x2c3a42, metalness: 0.72, roughness: 0.4 });
  const dark = new THREE.MeshStandardMaterial({ color: 0x1a242c, metalness: 0.68, roughness: 0.48 });
  const brass = new THREE.MeshStandardMaterial({ color: 0x8a7a48, metalness: 0.78, roughness: 0.32 });
  const body = new THREE.Mesh(new THREE.CylinderGeometry(1.38, 1.32, 11.4, 22, 1), hullMat);
  body.rotation.x = Math.PI / 2;
  body.position.set(0, -0.55, 0.85);
  exterior.add(body);
  const nose = new THREE.Mesh(new THREE.SphereGeometry(1.32, 18, 14), hullMat);
  nose.scale.set(1.15, 0.95, 1.35);
  nose.position.set(0, -0.5, -4.85);
  exterior.add(nose);
  const tail = new THREE.Mesh(new THREE.SphereGeometry(1.22, 16, 12), hullMat);
  tail.scale.set(1, 0.88, 1.4);
  tail.position.set(0, -0.52, 6.4);
  exterior.add(tail);
  const sail = new THREE.Mesh(new THREE.BoxGeometry(0.85, 1.85, 2.4), dark);
  sail.position.set(0, 0.85, -0.15);
  exterior.add(sail);
  const sailCap = new THREE.Mesh(new THREE.BoxGeometry(0.92, 0.16, 2.55), hullMat);
  sailCap.position.set(0, 1.78, -0.15);
  exterior.add(sailCap);
  const peri = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 1.3, 8), brass);
  peri.position.set(0.18, 2.45, -0.55);
  exterior.add(peri);
  const hatch = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.36, 0.2, 12), brass);
  hatch.position.set(0, 1.92, 0.35);
  exterior.add(hatch);
  exterior.userData.hatch = hatch;
  const keel = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.7, 6.5), dark);
  keel.position.set(0, -1.55, 0.6);
  exterior.add(keel);
  const rudder = new THREE.Mesh(new THREE.BoxGeometry(0.12, 1.6, 0.85), dark);
  rudder.position.set(0, -0.15, 7.15);
  exterior.add(rudder);
  for (const s of [-1, 1]) {
    const plane = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.1, 0.7), dark);
    plane.position.set(s * 1.55, -0.55, -3.4);
    exterior.add(plane);
    const stPlane = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.1, 0.55), dark);
    stPlane.position.set(s * 1.35, -0.4, 6.35);
    exterior.add(stPlane);
    const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.18, 1.6, 10), hullMat);
    tube.rotation.x = Math.PI / 2;
    tube.position.set(s * 0.72, -0.72, -5.2);
    exterior.add(tube);
    const hub = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.1, 0.28, 10), brass);
    hub.rotation.x = Math.PI / 2;
    hub.position.set(s * 0.55, -0.62, 7.35);
    exterior.add(hub);
    const prop = new THREE.Mesh(
      new THREE.BoxGeometry(0.08, 1.15, 0.22),
      new THREE.MeshStandardMaterial({ color: 0xb8c4cc, metalness: 0.85, roughness: 0.28 })
    );
    prop.position.set(s * 0.55, -0.62, 7.52);
    exterior.add(prop);
    if (s < 0) exterior.userData.propL = prop;
    else exterior.userData.propR = prop;
  }
  exterior.userData.prop = exterior.userData.propR;
...
crack.visible = false;
crack.position.set(1.42, -0.2, -1.1);
exterior.add(crack);
...
(function buildAtlanteanTemple() {
  const stone = new THREE.MeshStandardMaterial({ color: 0x6e6a60, roughness: 0.92, metalness: 0.08 });
  const moss = new THREE.MeshStandardMaterial({ color: 0x3a4a40, roughness: 0.88, emissive: 0x0a1814, emissiveIntensity: 0.35 });
  const glow = new THREE.MeshStandardMaterial({ color: 0x2a6a68, emissive: 0x123a38, roughness: 0.7 });
  const temple = new THREE.Group();
  temple.position.set(72, FLOOR, -58);
  temple.rotation.y = 0.18;
  const plaza = new THREE.Mesh(new THREE.BoxGeometry(86, 1.6, 120), stone);
  plaza.position.y = 0.7;
  temple.add(plaza);
  const nave = new THREE.Mesh(new THREE.BoxGeometry(28, 0.8, 88), moss);
  nave.position.set(0, 1.55, -4);
  temple.add(nave);
  for (let i = 0; i < 7; i++) {
    const z = -38 + i * 13;
    for (const side of [-1, 1]) {
      if (Math.random() < 0.18) continue;
      const h = 10 + (i % 3) * 3.5 + Math.random() * 4;
      const col = new THREE.Mesh(new THREE.CylinderGeometry(1.35, 1.7, h, 10), i % 2 ? stone : moss);
      col.position.set(side * 16.5, 1.6 + h / 2, z);
      col.rotation.z = side * (Math.random() * 0.12);
      temple.add(col);
      if (Math.random() > 0.45) {
        const cap = new THREE.Mesh(new THREE.BoxGeometry(3.4, 0.7, 3.4), stone);
        cap.position.set(side * 16.5, 1.7 + h, z);
        cap.rotation.z = side * 0.08;
        temple.add(cap);
      }
    }
    const span = 18;
    const arch = new THREE.Mesh(new THREE.TorusGeometry(span * 0.5, 1.15, 8, 28, Math.PI), stone);
    arch.rotation.y = Math.PI / 2;
    arch.position.set(0, 8.2 + (i % 2) * 1.4, z);
    temple.add(arch);
    const inner = new THREE.Mesh(new THREE.TorusGeometry(span * 0.5 - 0.15, 0.22, 6, 24, Math.PI), glow);
    inner.rotation.y = Math.PI / 2;
    inner.position.copy(arch.position);
    temple.add(inner);
  }
  for (let k = 0; k < 5; k++) {
    const slab = new THREE.Mesh(new THREE.BoxGeometry(6 + Math.random() * 8, 1.1, 2.2 + Math.random() * 3), stone);
    slab.position.set((Math.random() - 0.5) * 40, 2.4, -20 + Math.random() * 50);
    slab.rotation.set(Math.random() * 0.4, Math.random() * 2, (Math.random() - 0.5) * 0.8);
    temple.add(slab);
  }
  const shrine = new THREE.Mesh(new THREE.CylinderGeometry(7, 9, 3.2, 12), moss);
  shrine.position.set(0, 3.2, -52);
  temple.add(shrine);
  const idol = new THREE.Mesh(new THREE.SphereGeometry(3.4, 16, 12), glow);
  idol.position.set(0, 8.4, -52);
  temple.add(idol);
  const lamp = new THREE.PointLight(0x3ad0c4, 7.5, 70);
  lamp.position.set(0, 14, -20);
  temple.add(lamp);
  scene.add(temple);
})();
...
function makeTorch() {
  const g = new THREE.Group();
  const handle = new THREE.Mesh(
    new THREE.CylinderGeometry(0.016, 0.02, 0.16, 8),
    new THREE.MeshStandardMaterial({ color: 0x2a3036, metalness: 0.75, roughness: 0.32 })
  );
  handle.rotation.x = Math.PI / 2;
  handle.position.z = -0.05;
  const clamp = new THREE.Mesh(
    new THREE.BoxGeometry(0.04, 0.03, 0.05),
    new THREE.MeshStandardMaterial({ color: 0x6a5840, metalness: 0.55, roughness: 0.4 })
  );
  clamp.position.z = -0.13;
  const rod = new THREE.Mesh(
    new THREE.CylinderGeometry(0.006, 0.008, 0.12, 6),
    new THREE.MeshStandardMaterial({ color: 0xc0a878, metalness: 0.7, roughness: 0.35 })
  );
  rod.rotation.x = Math.PI / 2;
  rod.position.z = -0.2;
  const cone = new THREE.Mesh(
    new THREE.ConeGeometry(0.034, 0.18, 8),
    new THREE.MeshBasicMaterial({ color: 0x66ddff, transparent: true, opacity: 0.9 })
  );
  cone.rotation.x = -Math.PI / 2;
  cone.position.z = -0.28;
  const core = new THREE.Mesh(
    new THREE.ConeGeometry(0.014, 0.11, 6),
    new THREE.MeshBasicMaterial({ color: 0xfff6d0, transparent: true, opacity: 0.98 })
  );
  core.rotation.x = -Math.PI / 2;
  core.position.z = -0.24;
  const light = new THREE.PointLight(0x88e8ff, 0, 2.4);
  light.position.z = -0.26;
  g.add(handle, clamp, rod, cone, core, light);
  g.visible = false;
  g.userData.cone = cone;
  g.userData.core = core;
  g.userData.light = light;
  g.userData.tip = new THREE.Vector3(0, 0, -0.3);
  return g;
}
...
const weldBubbles = [];
(function makeWeldBubbles() {
  const geo = new THREE.SphereGeometry(0.035, 6, 5);
  for (let i = 0; i < 48; i++) {
    const b = new THREE.Mesh(
      geo,
      new THREE.MeshBasicMaterial({ color: 0xc8f4ff, transparent: true, opacity: 0.55, depthWrite: false })
    );
    b.visible = false;
    b.userData.life = 0;
...
function emitWeldBubbles(worldPos, n) {
  let left = n || 3;
  for (let i = 0; i < weldBubbles.length && left > 0; i++) {
    const b = weldBubbles[i];
    if (b.userData.life > 0) continue;
    b.visible = true;
    b.position.copy(worldPos);
    b.position.x += (Math.random() - 0.5) * 0.08;
    b.position.y += (Math.random() - 0.5) * 0.06;
    b.position.z += (Math.random() - 0.5) * 0.08;
    b.userData.life = 0.45 + Math.random() * 0.55;
    b.userData.v.set((Math.random() - 0.5) * 0.35, 0.55 + Math.random() * 0.7, (Math.random() - 0.5) * 0.35);
    b.scale.setScalar(0.6 + Math.random() * 0.8);
    left--;
  }
}

function tickWeldBubbles(dt) {
  weldBubbles.forEach((b) => {
    if (b.userData.life <= 0) return;
    b.userData.life -= dt;
    b.position.add(tmp.copy(b.userData.v).multiplyScalar(dt));
    b.userData.v.y += 1.8 * dt;
    b.userData.v.multiplyScalar(0.98);
    const a = Math.max(0, b.userData.life);
    b.material.opacity = Math.min(0.6, a * 1.1);
...
function sharkAi(dt) {
  ...
  if (sharkState === "patrol" || sharkState === "retreat") {
    ...
    if (shark.position.distanceTo(prey) < 33) {
      sharkState = "attack";
      setSharkMood();
    ...
    want = tmp.copy(prey);
    want.y += 0.4;
    if (shark.position.distanceTo(prey) < 16.5) {
      if (mode === "eva") {
        burst(prey, 16);
...
function pollXrButtons() {
  ...
  xrTrigger = false;
  xrGrip = false;
  xrStickL.x = xrStickL.y = 0;
  xrStickR.x = xrStickR.y = 0;
  ...
  for (const src of session.inputSources) {
    ...
    const dead = 0.14;
    const hx = Math.abs(sx) > dead ? sx : 0;
    const hy = Math.abs(sy) > dead ? sy : 0;
...
function setTorch(on) {
  ...
  [torch0, torch1].forEach((t) => {
    t.visible = !!active;
    if (t.userData.light) t.userData.light.intensity = active ? 1.8 + Math.random() * 1.4 : 0;
    if (t.visible) {
  ...
  if (!active) {
    ...
    if (torch0.userData.light) torch0.userData.light.intensity = 0;
    if (torch1.userData.light) torch1.userData.light.intensity = 0;
  }
}
...
function enterEva() {
  ...
  tmp.set(2.8, 1.6, 0.4).applyQuaternion(sub.quaternion);
  evaDummy.position.add(tmp);
  ...
  if (sub.userData.dome) sub.userData.dome.visible = false;
  helm.visible = true;
...
function enterPilot() {
  ...
  exterior.visible = false;
  if (sub.userData.dome) sub.userData.dome.visible = true;
  helm.visible = false;
  torch0.visible = false;
  torch1.visible = false;
...
function tryToggleEva() {
  ...
  else if (mode === "eva" && evaDummy.position.distanceTo(sub.position) < 5.2) enterPilot();
}
...
function readXrMove(dt, move) {
  ...
  pollXrButtons();
  if (mode === "eva") {
    if (Math.abs(xrStickL.x) > 0.02) move.ax += xrStickL.x;
    if (Math.abs(xrStickL.y) > 0.02) move.az += xrStickL.y;
    if (Math.abs(xrStickR.y) > 0.02) move.ay -= xrStickR.y;
  }
  if (mode === "pilot") {
    const port = xrStickL.y;
    const stbd = xrStickR.y;
    move.az += (port + stbd) * 0.5;
    ...
    xrGrab = null;
    if (xrGrip && evaDummy.position.distanceTo(sub.position) < 5.2) {
      if (xrHatchArmed) {
        xrHatchArmed = false;
        tryToggleEva();
      }
...
function step(dt) {
  ...
  if (mode === "pilot") {
    ...
    const spinL = Math.abs(xrOn ? xrStickL.y : move.az) + speed * 0.35;
    const spinR = Math.abs(xrOn ? xrStickR.y : move.az) + speed * 0.35;
    if (exterior.userData.propL) exterior.userData.propL.rotation.z += (xrOn ? -xrStickL.y : speed) * dt * 8 + spinL * dt * 3;
    if (exterior.userData.propR) exterior.userData.propR.rotation.z += (xrOn ? -xrStickR.y : speed) * dt * 8 + spinR * dt * 3;
    else if (exterior.userData.prop) exterior.userData.prop.rotation.z += speed * dt * 4;
    if (!xrOn) {
      if (shake > 0) {
    ...
    const nearHull = evaDummy.position.distanceTo(sub.position) < 5.4;
    const welding = xrOn
      ? xrTrigger && hull < 100 && nearHull
      : keys.has("KeyR") && hull < 100 && nearHull;
    if (welding) {
      repairHold += dt;
      ...
      if (weldEmitT <= 0) {
        ...
        const tip = tmp3;
        if (xrOn) {
          const hand = nearWorld(sub, 5.6) || ctrl0;
          hand.getWorldPosition(tip);
          tmp.set(0, 0, -0.28).applyQuaternion(hand.getWorldQuaternion(xrInvQ));
          tip.add(tmp);
        } else {
          crack.getWorldPosition(tip);
        }
        emitWeldBubbles(tip, 4);
        sfx(1200 + Math.random() * 400, 0.04, "sawtooth", 0.018);
      }
      if (repairHold > 3) {
        hull = 100;
        crack.visible = false;
