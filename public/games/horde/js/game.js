...
const WEPS = {
  ...
  ar: { id: "ar", name: "Assault rifle", dmg: 1, rpm: 6.845, mag: 40, speed: 156, spread: 0.016, pellets: 1, hitscan: true, pierce: 1, laser: true, cost: 170, reload: 6.4 },
  ...
  tank: { id: "tank", name: "Cyber tank", dmg: 6, rpm: 0.48, mag: 6, speed: 28, spread: 0.01, pellets: 1, aoe: 7.5, tank: true, missile: true, cost: 1500, reload: 8.8 },
  ...
  landship: { id: "landship", name: "Landship", dmg: 7, rpm: 0.42, mag: 10, speed: 26, spread: 0.01, pellets: 1, aoe: 8.8, landship: true, missile: true, cost: 3000, reload: 9.2 },
...
const SHOP = [
  ...
  { kind: "allyveh", id: "jeep", name: "Army jeep", cost: 1250, blurb: "Autopilot jeep with two pistol gunners. Unlimited." },
  { kind: "allyveh", id: "humvee", name: "Humvee", cost: 2500, blurb: "1.35× the jeep, double armor. Driver SMG, general with assault rifle." },
  { kind: "allyveh", id: "tank", name: "Ally tank", cost: 3000, blurb: "Autopilot tank. Same gun as yours, army-man AI." },
  { kind: "allyveh", id: "heli", name: "Ally heli", cost: 3000, blurb: "Autopilot attack heli. Missiles and a search beam, army-man AI." },
  { kind: "allyveh", id: "landship", name: "Ally landship", cost: 6000, blurb: "Twice the tank: length, armor, health. Three turrets hunt different foes." },
  ...
  { kind: "wep", id: "tank", name: "Cyber tank", cost: 1500, blurb: "Drive it. Fire artillery. Shells explode on impact." },
  { kind: "wep", id: "heli", name: "Attack heli", cost: 2000, blurb: "Fly it. Trigger fires missiles. Search beam on." },
  { kind: "wep", id: "landship", name: "Landship", cost: 3000, blurb: "Drive it. 2× tank length, armor, and health. Three turrets lock different enemies." },
...
const WASTE_PLANETS = [PLANETS[4], PLANETS[1], PLANETS[2], PLANETS[5]];
const MAP_EDGE = 220;
...
let player = { x: 0, y: 1.6, z: 0, vx: 0, vy: 0, vz: 0, hp: MAX_HP0, grounded: true, coins: 0, ammo: 48, mag: 12, wep: "pistol", tank: false, heli: false, landship: false, jumpsLeft: 1, sprinting: false, sprintT: 0, sprintCdT: 0, mom: 0, pounding: false, bike: false, ride: null, hullHp: 0, hullMax: 0 };
let mobs = [];
let debris = [];
let loot = [];
let shots = [];
...
const LS_OWMAP = "horde.owmap.v1";
const LS_TEAMCOL = "horde.teamcol.v1";
const TEAM_COLORS = [
  { id: "gold", name: "Gold", hex: 0xd4af37 },
  { id: "red", name: "Red", hex: 0xc42b2b },
  { id: "blue", name: "Blue", hex: 0x3366cc },
  { id: "green", name: "Green", hex: 0x2f6b32 },
  { id: "purple", name: "Purple", hex: 0x8844cc },
  { id: "white", name: "White", hex: 0xf4f1ea },
  { id: "black", name: "Black", hex: 0x1a1a1e },
  { id: "orange", name: "Orange", hex: 0xff8822 },
];
let gameMode = "horde";
let warTeam = "solo";
...
let owExtras = null;
let zoneFlags = [];
let landshipMesh = null;
let landshipYaw = 0;
let landshipTurretCd = [0, 0, 0];
let contElapsed = 0;
let contSpawnT = 0;
try {
  const savedMode = localStorage.getItem(LS_MODE);
  if (savedMode === "horde" || savedMode === "army" || savedMode === "mixed" || savedMode === "war" || savedMode === "open") gameMode = savedMode;
  ...
  const savedOw = localStorage.getItem(LS_OWMAP);
  if (savedOw) selectedOwMap = savedOw;
  const savedCol = localStorage.getItem(LS_TEAMCOL);
  if (TEAM_COLORS.some((c) => c.id === savedCol)) teamColorId = savedCol;
} catch {}
...
const ARMY_WEPS = {
  ...
  smg: { id: "smg", dmg: 1, rpm: 5.2, speed: 36, spread: 0.07 },
  shotgun: { id: "shotgun", dmg: 1, rpm: 0.62, speed: 26, spread: 0.14, pellets: 5 },
  lmg: { id: "lmg", dmg: 1, rpm: 7.4, speed: 44, spread: 0.045 },
  bazooka: { id: "bazooka", dmg: 2, rpm: 0.28, speed: 22, spread: 0.02, aoe: 3.4 },
  mg: { id: "mg", dmg: 1, rpm: 6.2, speed: 46, spread: 0.04 },
  ar: { id: "ar", dmg: 1, rpm: 6.845, speed: 52, spread: 0.02 },
  cannon: { id: "cannon", dmg: 3, rpm: 0.22, speed: 28, spread: 0.01, aoe: 4.2 },
  missile: { id: "missile", dmg: 3, rpm: 0.55, speed: 32, spread: 0.02, aoe: 3.6 },
};
let menuOpen = false;
let paused = false;
let menuHits = [];
...
function teamColorHex() {
  return (TEAM_COLORS.find((c) => c.id === teamColorId) || TEAM_COLORS[0]).hex;
}
function setTeamColor(id) {
  if (!TEAM_COLORS.some((c) => c.id === id)) return;
  teamColorId = id;
  try { localStorage.setItem(LS_TEAMCOL, id); } catch {}
  paintShop();
  paintShop3d();
  paintTeamColorRow();
}
...
function shopSectionOf(it) {
  if (it.kind === "bike" || it.kind === "allyveh" || it.id === "tank" || it.id === "heli" || it.id === "landship") return "vehicles";
...
function hillsAt(x, z) {
  ...
  if (runMap === "wastes") h *= 0.52;
  if (runMap === "shore") {
    const t = clamp((x - 6) / 16, 0, 1);
    if (t > 0) {
      const bed = WATER_LEVEL - 2.6 - Math.sin(z * 0.045) * 0.35;
      h = h * (1 - t) + bed * t;
    }
  }
...
function inWater(x, z) {
  return runMap === "shore" && x > 16 && hillsAt(x, z) < WATER_LEVEL - 0.12;
}

function isFlyer(m) {
  if (!m) return false;
  if (m.armyFly || m.aquatic || m.vehicle === "heli") return true;
  return (m.limbs || []).some((l) => l.userData?.live && l.userData.form === "wing");
}

function clampToMap(x, z) {
  if (runMap !== "shore") return { x, z };
  return { x: clamp(x, -MAP_EDGE + 0.8, MAP_EDGE - 0.8), z: clamp(z, -MAP_EDGE + 0.8, MAP_EDGE - 0.8) };
}
...
function heightAt(x, z, y) {
  ...
  for (const b of buildings) {
    for (const cell of b.cells) {
      ...
      if (Math.abs(x - cell.x) > cell.w || Math.abs(z - cell.z) > cell.d) continue;
      if (y != null && y < cell.top - 0.55) continue;
      h = Math.max(h, cell.top);
    }
  }
  if (owExtras) h = owHeight(owExtras, x, z, y, h);
  return h;
}

function pushOutOfBuildings(x, z, y) {
  if (owExtras) {
    const p = owPush(owExtras, x, z, y);
    x = p.x; z = p.z;
  }
  let nx = x, nz = z;
  for (const b of buildings) {
...
function makeGun(id) {
  ...
  if (id === "smg") {
    ...
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.024, 0.5, 8), iron);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.set(0, 0.05, -0.22);
    const box = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.09, 0.2), dark);
    box.position.set(0, 0, 0.04);
    g.add(barrel, box);
    g.userData.muzzle = new THREE.Vector3(0, 0.05, -0.48);
  } else if (id === "heli") {
...
function equip(id) {
  ...
  player.landship = !!WEPS[id]?.landship;
  player.mag = Math.min(player.mag, magCap());
  reloadT = 0;
  reloadMax = 0;
  if (player.tank || player.heli || player.landship) {
    player.bike = false;
    if (player.ride) dismountRide();
  }
  if (!player.tank && tankMesh) tankMesh.visible = false;
  if (!player.heli && heliMesh) heliMesh.visible = false;
  if (!player.landship && landshipMesh) landshipMesh.visible = false;
  if (player.landship) {
    player.hullMax = 40;
    player.hullHp = 40;
  } else if (!player.tank && !player.heli) {
    player.hullHp = 0;
    player.hullMax = 0;
  }
  ...
  gunMesh.visible = !xrOn && !player.heli && !player.tank && !player.landship;
...
function planetForWave(w) {
  if (runMap === "shore") return PLANETS[3];
  if (runMap === "wastes") {
    const i = Math.floor(Math.max(0, (Math.min(w, 100) - 1) / 5)) % WASTE_PLANETS.length;
    return WASTE_PLANETS[i];
  }
...
function makeHordeCreature(w, ang, dist) {
...
function armyMat(hex, extra) {
  return new THREE.MeshPhongMaterial({
    color: hex ?? ARMY_GREEN,
    shininess: 92,
    specular: 0x7aaa70,
    ...extra,
  });
...
function hiredVehCount() {
  return mobs.filter((m) => m.alive && m.allyVeh).length;
}

function shopItemFromPickup(def) {
  if (!def) return null;
  const id = def.id === "tank-ai" ? "tank" : def.id === "heli-ai" ? "heli" : def.id === "landship-ai" ? "landship" : def.id;
  const kind = def.id === "tank-ai" || def.id === "heli-ai" || def.id === "landship-ai" ? "allyveh" : def.kind;
  return SHOP.find((it) => it.id === id && (kind ? it.kind === kind || it.kind === def.kind : true))
    || SHOP.find((it) => it.id === id)
    || { id, kind: def.kind || "wep", name: def.name || id, cost: 0 };
}

function armyWepFromPickup(id) {
  if (id === "smg") return "smg";
  if (id === "shotgun") return "shotgun";
  if (id === "ar" || id === "rail" || id === "lsword") return "lmg";
  if (id === "nuke" || id === "thunder" || id === "nova" || id === "plasma" || id === "ripple") return "bazooka";
  if (id === "pistol") return "pistol";
  return "rifle";
}

function grantPickup(def, who) {
  const it = shopItemFromPickup(def);
  if (!it) return false;
  if (who === "player") {
    if (it.kind === "wep") {
      ...
      owned.add(it.id);
      equip(it.id);
      showBanner(it.name.toUpperCase());
      announcing = 1.1;
      return true;
    }
    if (it.kind === "ammo") {
    ...
    if (it.kind === "bike") {
      if (player.bike) return false;
      player.bike = true;
      showBanner("DIRT BIKE");
      announcing = 1;
      return true;
    }
    if (it.kind === "allyveh") {
      return spawnHiredVehicle(it.id);
    }
    if (it.kind === "troop") return spawnHiredArmy();
    if (it.kind === "drone") return spawnDrone();
    if (it.kind === "turret") return spawnTurret();
    if (it.kind === "ball") return spawnBall(it.hp || 8);
    if (it.kind === "up") {
      if (it.id === "sprint" && stats.sprint) return false;
      if (it.id === "wheelie" && stats.wheelie) return false;
      if (it.id === "autoreload" && stats.autoReload) return false;
      if (it.id === "jump2" && stats.jumps >= 2) return false;
      if (it.id === "jump3" && stats.jumps >= 3) return false;
      if (it.id === "orbcannon" && orbCannon) return false;
      buy({ ...it, cost: 0 });
      return true;
    }
    return false;
  ...
  const m = who;
  if (!m || !m.alive || !m.army) return false;
  if (it.kind === "wep" && !WEPS[it.id]?.tank && !WEPS[it.id]?.heli && !WEPS[it.id]?.landship) {
    const wepId = armyWepFromPickup(it.id);
    m.armyWep = wepId;
    m.shotDef = ARMY_WEPS[wepId] || m.shotDef;
    m.shotShape = wepId === "bazooka" ? "rocket" : "pellet";
    m.ranged = true;
    return true;
  }
  if (it.kind === "allyveh" || it.id === "tank" || it.id === "heli" || it.id === "landship") {
    const kind = it.kind === "allyveh" ? it.id : it.id;
    const v = makeArmyVehicle(Math.max(12, diffWave()), 0, 0.2, kind, { hired: m.hired, tan: m.tan });
    v.team = mobFaction(m);
    v.x = m.x; v.z = m.z; v.y = m.y;
    v.mesh.position.set(m.x, m.y, m.z);
    if (m.hired) { v.hired = true; v.allyVeh = true; }
    mobs.push(v);
    m.ride = v;
    m.rideSeat = 0;
    return true;
  }
  if (it.kind === "bike" || it.id === "speed") {
    m.spd *= 1.35;
    return true;
  }
  if (it.id === "hp") {
    for (const l of m.limbs) if (l.userData) l.userData.maxHits = (l.userData.maxHits || 1) + 1;
    return true;
  }
  return false;
}
...
function owApi() {
  return {
    ...
    crushMob: (m) => { if (m?.alive) killMob(m, true); },
    trapShot: (x, y, z, tx, ty, tz, def) => {
      const dir = new THREE.Vector3(tx - x, ty - y, tz - z).normalize();
      const mesh = def.dart
        ? new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.32, 5), new THREE.MeshBasicMaterial({ color: 0xffe08a }))
        : makeBolt(0xffaa44, 0.03);
      mesh.position.set(x, y, z);
      if (!def.dart) aimBolt(mesh, dir);
      else mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir);
      scene.add(mesh);
      eShots.push({ mesh, dir, speed: def.speed || 36, life: 1.6, dmg: def.dmg || 1, team: "horde", aoe: def.aoe || 0, from: null, missile: !!def.aoe });
    },
    trapFlame: (x, y, z, tx, ty, tz) => {
      const dir = new THREE.Vector3(tx - x, 0, tz - z).normalize();
      spawnFireWall(new THREE.Vector3(x, y, z), dir);
    },
    pitAt: (x, z) => craterAt(x, z, 2.2, 2.4),
    onFlagCapture: (f, coins) => {
      player.coins += coins;
      sfx.flag();
      showBanner("ZONE TAKEN  +25 ◎");
      announcing = 1.6;
    },
    onFlagLost: () => {
      showBanner("ZONE LOST");
      announcing = 1.2;
    },
    countNear: (x, z, r, kind) => mobs.filter((m) => m.alive && Math.hypot(m.x - x, m.z - z) < r && (kind === "mixed" || (kind === "army" ? m.army : !m.army))).length,
    spawnAt: (kind, x, z) => {
      const k = kind === "mixed" ? (rng() < 0.5 ? "army" : "horde") : kind;
      const ang = Math.atan2(x - player.x, z - player.z);
      const dist = Math.hypot(x - player.x, z - player.z);
      const w = Math.max(4, diffWave());
      let spawned;
      if (k === "army") spawned = makeArmyUnit(w, ang, dist);
      else spawned = makeHordeCreature(w, ang, dist);
      spawned.x = x; spawned.z = z;
      spawned.y = heightAt(x, z);
      spawned.mesh.position.set(x, spawned.y, z);
      spawned.team = spawned.army ? "army" : "horde";
      mobs.push(spawned);
    },
    holdRope: (r) => {
      player.x += (r.x - player.x) * 0.2;
      player.z += (r.z - player.z) * 0.2;
...
async function loadOpenWorldMap() {
  ...
  if (data.start) {
    player.x = data.start.x || 0;
    player.z = data.start.z || 0;
    yaw = data.start.yaw || 0;
  }
}

function spawnHiredVehicle(kind) {
  if (hiredVehCount() >= 12) {
    showBanner("TOO MANY VEHICLES");
    announcing = 1.2;
    return false;
  }
  const ang = rng() * Math.PI * 2;
  const dist = 4 + rng() * 3;
  const w = Math.max(12, diffWave());
  const v = makeArmyVehicle(w, ang, dist, kind, { hired: true, tan: playerArmyTan() });
  v.team = playerFaction();
  v.hired = true;
  v.allyVeh = true;
  mobs.push(v);
  if (kind === "jeep") {
    for (let i = 0; i < 2; i++) {
      const s = makeArmySoldier(Math.max(6, w), ang + i * 0.4, dist * 0.2, { hired: true, tan: playerArmyTan(), wep: "pistol" });
      s.team = playerFaction();
      s.hired = true;
      s.ride = v;
      s.rideSeat = i;
      mobs.push(s);
  ...
  if (kind === "humvee") {
    const drv = makeArmySoldier(w, ang, dist * 0.2, { hired: true, tan: playerArmyTan(), wep: "smg" });
    drv.team = playerFaction(); drv.hired = true; drv.ride = v; drv.rideSeat = 0;
    const gen = makeArmySoldier(w, ang + 0.3, dist * 0.2, { hired: true, tan: playerArmyTan(), general: true, wep: "ar" });
    gen.team = playerFaction(); gen.hired = true; gen.ride = v; gen.rideSeat = 1;
    mobs.push(drv, gen);
  }
  sfx.armyRadio();
  showBanner(kind.toUpperCase() + " DEPLOYED");
  announcing = 1.2;
  return true;
}

function spawnOneKind(kind) {
  const liveCap = Math.round(MAX_LIVE * 1.2);
  if (mobs.filter((m) => m.alive).length >= liveCap) return;
  let ang = rng() * Math.PI * 2;
  if (kind === "army") ang = (rng() - 0.5) * 1.8;
  else ang = Math.PI + (rng() - 0.5) * 1.8;
  let dist = SPAWN_MIN + rng() * (SPAWN_MAX - SPAWN_MIN);
  if (runMap === "shore" && kind !== "aqua") {
    const x = player.x + Math.cos(ang) * dist;
    if (inWater(x, player.z + Math.sin(ang) * dist)) dist = Math.abs(player.x) + 40;
  }
  const w = diffWave();
  let spawned;
  if (kind === "army") spawned = makeArmyUnit(w, ang, dist);
  else if (kind === "aqua") spawned = rng() < 0.45 ? makeKraken(w, ang, dist) : makeShark(w, ang, dist);
  else spawned = makeHordeCreature(w, ang, dist);
  if (!spawned.team) spawned.team = spawned.army ? "army" : "horde";
  mobs.push(spawned);
}
...
function tickContinuous(dt) {
  ...
  contSpawnT -= dt;
  if (contSpawnT <= 0) {
    contSpawnT = 15 + rng() * 15;
    spawnOneKind("army");
    spawnOneKind("horde");
    if (runMap === "shore" && rng() < 0.55) spawnOneKind("aqua");
  }
}
...
function isHostileToPlayer(m) {
  return factionsHostile(mobFaction(m), playerFaction());
}

function playerHitsMob(m) {
  return isHostileToPlayer(m);
}

function setGameMode(mode) {
  if (mode !== "horde" && mode !== "army" && mode !== "mixed" && mode !== "war" && mode !== "open") return;
  gameMode = mode;
...
function setWarTeam(team) {
  if (team !== "solo" && team !== "army" && team !== "horde") return;
  warTeam = team;
  try { localStorage.setItem(LS_WARTEAM, team); } catch {}
  paintModeButtons();
}
...
function setSelectedMap(id) {
  ...
  if (String(id).startsWith("ow:") || id === "demo-fort" || id === "preview") {
    selectedOwMap = String(id).replace(/^ow:/, "");
    try { localStorage.setItem(LS_OWMAP, selectedOwMap); } catch {}
    paintModeButtons();
    paintOwMapRow();
    return;
  }
...
function mapBlurb() {
  if (gameMode === "open") return "Open world: load a map from the 2D maker. Capture flags for 25 ◎ — they stay and take your team color. Pickups equip if you do not already have them. Army men can grab them too.";
  if (selectedMap === "shore") return "Shore city: tall towers on a land/water split, picket fence at the rim. Land units avoid the sea; flyers pass over. Krakens and shark fins hunt the water and can come ashore.";
  if (selectedMap === "wastes") return "Clock wastes: desert and Mars ground, jet-black spiral spires, hollow clock towers with inner catwalks, tattered brick walls and trenches. Meteors hit harder and more often.";
  return "Green stair zone: infinite hills, hex stairs, shifting worlds every five waves.";
}

function modeBlurb() {
  let mode = "";
  if (gameMode === "army") mode = "Plastic army men. Guns get heavier with the waves. Generals bring floodlights and airstrikes. Jeeps, humvees, tanks, and helis later.";
  ...
  } else if (gameMode === "open") mode = "Open world. No waves — explore a custom map. Zone flags stay on the field. Right grip (or G) rides AI vehicles.";
  else mode = "Classic doubling waves of silhouette creatures. Shoot every limb.";
  if (devMode) mode += " Sandbox on: infinite gold and HP.";
  return mode + " " + mapBlurb();
}
...
function paintModeButtons() {
  ...
  document.querySelectorAll("[data-team]").forEach((b) => {
    b.classList.toggle("on", b.dataset.team === warTeam);
  });
  document.querySelectorAll("[data-map]").forEach((b) => {
    b.classList.toggle("on", b.dataset.map === selectedMap);
  });
  const sb = $("sandbox-btn");
  if (sb) {
    sb.classList.toggle("on", !!devMode);
    sb.textContent = devMode ? "Sandbox on" : "Sandbox";
  }
  const wrap = $("war-team");
  ...
  const flowSec = $("flow-row")?.closest(".setup-cat");
  if (flowSec) flowSec.hidden = gameMode === "open";
  const mapSec = $("map-row")?.closest(".setup-cat");
  if (mapSec && gameMode !== "open") {
    document.querySelectorAll("#map-row [data-map]").forEach((b) => {
      if (!String(b.dataset.map).startsWith("ow") && b.dataset.map !== "demo-fort" && b.dataset.map !== "preview") {
        b.hidden = false;
      }
    });
  }
...
function fillOwMapButtons(maps) {
  ...
  for (const m of items) {
    ...
    b.type = "button";
    b.className = "mode-btn" + (selectedOwMap === m.id ? " on" : "");
    b.dataset.map = "ow:" + m.id;
    b.dataset.ow = "1";
    b.textContent = m.name || m.id;
    b.onclick = () => setSelectedMap("ow:" + m.id);
    row.appendChild(b);
  }
  if (!$("mapmaker-link")) {
...
function pickMobTarget(m) {
  ...
  const huntForHire = !!(m.hired || m.allyVeh);
  const anchor = (!huntForHire && m.army && !m.vehicle) ? armyDefendAnchor(m) : (huntForHire ? { x: player.x, y: player.y, z: player.z, r: 5 } : null);
  const ox = huntForHire ? m.x : (anchor ? anchor.x : m.x);
  const oz = huntForHire ? m.z : (anchor ? anchor.z : m.z);
  let best = null;
  let bestScore = 1e9;
  if (factionsHostile(my, pTeam) && !huntForHire) {
  ...
  if (runMode === "war" || huntForHire || anchor) {
    for (const o of mobs) {
      if (o === m || !o.alive) continue;
      if (!factionsHostile(my, mobFaction(o))) continue;
      const d = Math.hypot(o.x - m.x, o.z - m.z);
      const dp = Math.hypot(o.x - player.x, o.z - player.z);
      if (huntForHire && d > 40 && dp > 48) continue;
      if (anchor && !huntForHire && Math.hypot(o.x - ox, o.z - oz) > 32 && d > 14) continue;
      const score = huntForHire ? d * 0.55 : d * 0.68;
      if (score < bestScore) {
        bestScore = score;
...
function armyWeaponForWave(w) {
  if (w >= 18) return rng() < 0.22 ? "bazooka" : (rng() < 0.4 ? "lmg" : (rng() < 0.5 ? "shotgun" : "smg"));
  if (w >= 12) return rng() < 0.18 ? "bazooka" : (rng() < 0.45 ? "lmg" : (rng() < 0.5 ? "shotgun" : "smg"));
  if (w >= 8) return rng() < 0.28 ? "lmg" : (rng() < 0.5 ? "shotgun" : (rng() < 0.6 ? "smg" : "rifle"));
  if (w >= 5) return rng() < 0.4 ? "smg" : "rifle";
  if (w >= 2) return "rifle";
  return "pistol";
}

function armyVehicleForWave(w) {
  if (w < 12) return null;
  if (w >= 36) return rng() < 0.22 ? "landship" : (rng() < 0.4 ? "heli" : (rng() < 0.55 ? "tank" : (rng() < 0.5 ? "humvee" : "jeep")));
  if (w >= 30) return rng() < 0.42 ? "heli" : (rng() < 0.55 ? "tank" : (rng() < 0.5 ? "humvee" : "jeep"));
  if (w >= 22) return rng() < 0.28 ? "heli" : (rng() < 0.5 ? "tank" : (rng() < 0.55 ? "humvee" : "jeep"));
  if (w >= 16) return rng() < 0.22 ? "tank" : (rng() < 0.6 ? "humvee" : "jeep");
  return "jeep";
}

function addArmyPart(group, limbs, mesh, x, y, z, opts) {
  const g = new THREE.Group();
  g.add(mesh);
  ...
  g.userData = {
    ...
    phase: rng() * 6,
    hits: 0,
    maxHits: opts.maxHits || 1,
    baseYaw: opts.baseYaw || 0,
    basePitch: opts.basePitch || 0,
    baseRoll: 0,
...
function makeArmyGun(kind) {
  const g = new THREE.Group();
  const dark = armyMat(0x243226);
  if (kind === "bazooka") {
    const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.06, 0.62, 8), dark);
    tube.rotation.x = Math.PI / 2;
    const pack = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.08, 0.16), dark);
    pack.position.z = 0.12;
    g.add(tube, pack);
  } else if (kind === "shotgun") {
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.032, 0.38, 6), dark);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.z = -0.1;
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.055, 0.16), dark);
    g.add(body, barrel);
  } else if (kind === "lmg") {
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.024, 0.46, 6), dark);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.z = -0.14;
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.06, 0.22), dark);
    const mag = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.04, 0.05), dark);
    mag.position.set(0.05, -0.02, 0.02);
    g.add(body, barrel, mag);
  } else if (kind === "smg") {
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.018, 0.26, 6), dark);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.z = -0.08;
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.05, 0.14), dark);
    g.add(body, barrel);
  } else if (kind === "pistol") {
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.045, 0.1), dark);
    const barrel = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.022, 0.08), dark);
    barrel.position.z = -0.07;
    g.add(body, barrel);
  } else {
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.02, 0.48, 6), dark);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.z = -0.14;
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.05, 0.18), dark);
    const stock = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.04, 0.1), dark);
    stock.position.set(0, -0.01, 0.1);
...
function armyPlace(ang, dist) {
  let x = player.x + Math.cos(ang) * dist;
  let z = player.z + Math.sin(ang) * dist;
  if (runMap === "shore" && inWater(x, z)) x = 8 - rng() * 36;
  return clampToMap(x, z);
}

function aquaPlace(ang, dist) {
  let z = player.z + Math.sin(ang || rng() * Math.PI * 2) * (dist || (18 + rng() * 50));
  let x = Math.max(26, 24 + rng() * 70);
  if (player.x > 18) x = Math.max(22, player.x + (rng() - 0.3) * 36);
  const cl = clampToMap(x, z);
  if (!inWater(cl.x, cl.z)) cl.x = Math.min(MAP_EDGE - 4, 28 + rng() * 40);
  return cl;
}
...
function makeArmySoldier(w, ang, dist, opts) {
  ...
  const wepId = opts.wep || (general ? (w >= 14 ? "bazooka" : "lmg") : armyWeaponForWave(w));
...
function makeArmyVehicle(w, ang, dist, kind, opts) {
  opts = opts || {};
  const tan = !!opts.tan;
  const hired = !!opts.hired;
  const humveeUp = kind === "humvee" && hired;
  const col = tan ? (rng() < 0.5 ? ARMY_TAN : ARMY_TAN2) : ARMY_GREEN;
  const green = armyMat(col);
  const dark = armyMat(0x243226);
  const group = new THREE.Group();
  const limbs = [];
  const { x, z } = armyPlace(ang, dist);
  ...
  if (kind === "jeep") {
    ...
    addArmyPart(group, limbs, hood, 0, 0.62, -0.55, { thick: 0.28, maxHits: 2 });
    const windshield = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.32, 0.06), armyMat(0x4a6a48, { transparent: true, opacity: 0.45 }));
    windshield.position.set(0, 0.78, -0.18);
    group.add(windshield);
    const gun = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.045, 0.7, 6), dark);
    gun.rotation.x = Math.PI / 2;
    addArmyPart(group, limbs, gun, 0, 0.92, -0.1, { thick: 0.12, maxHits: 1, dmg: 1 });
    for (const [sx, sz] of [[-0.48, 0.7], [0.48, 0.7], [-0.48, -0.7], [0.48, -0.7]]) {
      const wh = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.14, 8), dark);
      wh.rotation.z = Math.PI / 2;
      addArmyPart(group, limbs, wh, sx, 0.22, sz, { thick: 0.18, asLeg: true, form: "club", maxHits: 1 });
    }
    hitR = 1.35; spd = 3.4; lift = 0; wepId = "mg";
  } else if (kind === "humvee") {
    scale = hired && humveeUp ? 1.265 * 1.35 : 1.25 * 1.1;
    ...
    addArmyPart(group, limbs, cabin, 0, 1.0, 0.15, { thick: 0.4, maxHits: 2 });
    const gun = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.05, 0.9, 6), dark);
    gun.rotation.x = Math.PI / 2;
    addArmyPart(group, limbs, gun, 0, 1.32, -0.2, { thick: 0.14, maxHits: 2, dmg: 2 });
    for (const [sx, sz] of [[-0.62, 0.85], [0.62, 0.85], [-0.62, -0.85], [0.62, -0.85]]) {
      const wh = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.28, 0.18, 8), dark);
      wh.rotation.z = Math.PI / 2;
      addArmyPart(group, limbs, wh, sx, 0.28, sz, { thick: 0.22, asLeg: true, maxHits: 2 });
    }
    hitR = 1.7; spd = 2.7; lift = 0; wepId = "lmg";
    if (hired) {
      const plate = new THREE.Mesh(new THREE.BoxGeometry(1.52, 0.14, 2.52), dark);
      addArmyPart(group, limbs, plate, 0, 0.92, 0, { thick: 0.22, maxHits: 3, dmg: 2 });
      const skirt = new THREE.Mesh(new THREE.BoxGeometry(1.58, 0.22, 2.5), dark);
      addArmyPart(group, limbs, skirt, 0, 0.38, 0, { thick: 0.28, maxHits: 3 });
      const bumper = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.2, 0.18), dark);
      addArmyPart(group, limbs, bumper, 0, 0.4, -1.22, { thick: 0.2, maxHits: 2 });
      const grate = new THREE.Mesh(new THREE.BoxGeometry(1.05, 0.08, 0.7), dark);
      addArmyPart(group, limbs, grate, 0, 0.72, -0.7, { thick: 0.16, maxHits: 2 });
    }
  } else if (kind === "tank") {
    scale = 1.45 * 1.1;
    ...
    addArmyPart(group, limbs, turret, 0, 1.05, 0.05, { thick: 0.45, maxHits: 3, dmg: 2 });
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.09, 1.7, 8), dark);
    barrel.rotation.x = Math.PI / 2;
    addArmyPart(group, limbs, barrel, 0, 1.08, -0.95, { thick: 0.16, maxHits: 2, dmg: 3 });
    const skirtL = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.38, 2.4), dark);
    addArmyPart(group, limbs, skirtL, -0.95, 0.32, 0, { thick: 0.3, maxHits: 2, asLeg: true });
    const skirtR = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.38, 2.4), dark);
    addArmyPart(group, limbs, skirtR, 0.95, 0.32, 0, { thick: 0.3, maxHits: 2, asLeg: true });
    hitR = 2.1; spd = 1.55; lift = 0; wepId = "cannon";
  } else if (kind === "landship") {
    scale = 1.45 * 1.1;
    const hull = new THREE.Mesh(new THREE.BoxGeometry(2.05, 0.82, 5.2), green);
    core = addArmyPart(group, limbs, hull, 0, 0.62, 0, { thick: 0.85, len: 2.8, maxHits: 8, dmg: 3 });
    const superstructure = new THREE.Mesh(new THREE.BoxGeometry(1.7, 0.45, 2.4), green);
    addArmyPart(group, limbs, superstructure, 0, 1.18, 0.2, { thick: 0.5, maxHits: 6, dmg: 2 });
    const t1 = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.5, 0.36, 10), green);
    addArmyPart(group, limbs, t1, 0, 1.52, -1.35, { thick: 0.4, maxHits: 6, dmg: 3, part: "turret" });
    const b1 = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.09, 1.8, 8), dark);
    b1.rotation.x = Math.PI / 2;
    addArmyPart(group, limbs, b1, 0, 1.55, -2.3, { thick: 0.16, maxHits: 4, dmg: 3 });
    const t2 = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.36, 0.28, 8), green);
    addArmyPart(group, limbs, t2, 0.55, 1.42, 0.35, { thick: 0.32, maxHits: 5, dmg: 2, part: "turret" });
    const b2 = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 1.1, 8), dark);
    b2.rotation.x = Math.PI / 2;
    addArmyPart(group, limbs, b2, 0.55, 1.44, -0.3, { thick: 0.12, maxHits: 3, dmg: 2 });
    const t3 = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.32, 0.24, 8), green);
    addArmyPart(group, limbs, t3, -0.55, 1.38, 1.55, { thick: 0.28, maxHits: 5, dmg: 2, part: "turret" });
    const b3 = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.05, 0.9, 8), dark);
    b3.rotation.x = Math.PI / 2;
    addArmyPart(group, limbs, b3, -0.55, 1.4, 0.95, { thick: 0.1, maxHits: 3, dmg: 2 });
    const skirtL = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.42, 5.0), dark);
    addArmyPart(group, limbs, skirtL, -1.05, 0.36, 0, { thick: 0.34, maxHits: 4, asLeg: true });
    const skirtR = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.42, 5.0), dark);
    addArmyPart(group, limbs, skirtR, 1.05, 0.36, 0, { thick: 0.34, maxHits: 4, asLeg: true });
    hitR = 3.4; spd = 1.25; lift = 0; wepId = "cannon";
  } else {
    kind = "heli";
    scale = 1.485;
    fly = true;
    ...
    core = addArmyPart(group, limbs, hull, 0, 0.2, 0, { thick: 0.5, len: 1.4, maxHits: 3, dmg: 2 });
    const cabin = new THREE.Mesh(new THREE.SphereGeometry(0.34, 8, 6), armyMat(0x4a6a48));
    addArmyPart(group, limbs, cabin, 0, 0.22, -0.55, { thick: 0.32, maxHits: 2 });
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.1, 1.35), green);
    addArmyPart(group, limbs, tail, 0, 0.28, 1.1, { thick: 0.14, maxHits: 2 });
    const rotor = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.04, 0.12), dark);
    addArmyPart(group, limbs, rotor, 0, 0.72, 0, { thick: 0.2, maxHits: 2 });
    const rotor2 = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.04, 3.2), dark);
    rotor2.position.y = 0.72;
    group.add(rotor2);
    group.userData.rotor = rotor;
    group.userData.rotor2 = rotor2;
    hitR = 1.9; spd = 3.9; lift = 4.2; wepId = "missile";
  }
  group.scale.setScalar(scale);
  const walkH = heightAt(x, z) + lift;
  group.position.set(x, walkH, z);
  scene.add(group);
  ...
  const unit = {
    ...
    hpLimbs: limbs.filter((l) => l.userData.live).length,
    x, z, y: walkH,
    spd: spd * waveSpeedMul(w) / Math.sqrt(scale),
    boost: 1.1,
    bodyScale: scale,
    giant: true,
    bob: rng() * 6,
  ...
  if (hired) {
    ...
    unit.allyVeh = true;
    unit.tan = tan;
    unit.team = playerFaction();
    if (kind === "jeep") {
      unit.gunners = 2;
      unit.gunnerWep = "pistol";
      unit.gunCd = 0;
      unit.ranged = false;
    }
    if (kind === "humvee") {
      ...
      unit.genCd = 0;
      unit.general = true;
      for (const l of unit.limbs) l.userData.maxHits = (l.userData.maxHits || 1) * 2;
      unit.hpLimbs = unit.limbs.filter((l) => l.userData.live).length;
    }
    if (kind === "tank") {
      unit.shotDef = { dmg: 6, rpm: 0.48, speed: 28, spread: 0.01, aoe: 7.5 };
      unit.shotShape = "rocket";
    }
    if (kind === "heli") {
      unit.shotDef = { dmg: 7, rpm: 1.6, speed: 52, spread: 0.012, aoe: 5.4 };
      unit.shotShape = "rocket";
    }
    if (kind === "landship") {
      unit.shotDef = { dmg: 7, rpm: 0.42, speed: 26, spread: 0.01, aoe: 8.8 };
      unit.shotShape = "rocket";
      unit.multiTurret = true;
      unit.turretDefs = [
        { dmg: 7, rpm: 0.42, speed: 26, spread: 0.01, aoe: 8.8, missile: true },
        { dmg: 7, rpm: 1.4, speed: 52, spread: 0.012, aoe: 5.4, missile: true },
        { dmg: 2, rpm: 5.5, speed: 44, spread: 0.04, aoe: 0 },
      ];
      unit.turretOff = [
        { x: 0, y: 1.7, z: -2.2 },
        { x: 0.7, y: 1.55, z: 0.2 },
      ...
      unit.turretCd = [0.2, 0.5, 0.1];
      for (const l of unit.limbs) l.userData.maxHits = (l.userData.maxHits || 1);
      unit.hpLimbs = unit.limbs.filter((l) => l.userData.live).length;
    }
  }
  if (kind === "landship") {
    ...
    unit.turretCd = unit.turretCd || [0.2, 0.5, 0.1];
    unit.shotShape = "rocket";
    unit.shotDef = unit.turretDefs[0];
  }
  if (kind === "jeep" || kind === "humvee") attachVehicleHeadlights(unit);
  else attachArmyFlash(unit, kind === "tank" || kind === "heli" || kind === "landship" ? 2.4 : 1.6, -1);
  return unit;
}

function makeShark(w, ang, dist) {
  const group = new THREE.Group();
  const col = 0x1a2430;
  const s = (1.1 + w * 0.02) * 1.15;
  const body = new THREE.Mesh(new THREE.ConeGeometry(0.28, 1.6, 7), mat(col));
  body.rotation.x = Math.PI / 2;
  const fin = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.55, 4), mat(0x101820));
  fin.position.set(0, 0.42, -0.1);
  const tail = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.45, 0.28), mat(col));
  tail.position.set(0, 0.05, 0.85);
  group.add(body, fin, tail);
  const limb = new THREE.Group();
  limb.add(new THREE.Mesh(new THREE.SphereGeometry(0.2, 6, 5), mat(col)));
  limb.userData = { live: true, thick: 0.28, dmg: 2, len: 0.8, asLeg: false, form: "club", hits: 0, maxHits: 2 };
  group.add(limb);
  const { x, z } = aquaPlace(ang, dist);
  const y = Math.max(hillsAt(x, z), WATER_LEVEL) + 0.2;
  group.position.set(x, y, z);
  group.scale.setScalar(s);
  scene.add(group);
  return {
    mesh: group, core: limb, limbs: [limb], hpLimbs: 1,
    x, z, y, spd: 2.4 + w * 0.06, boost: 1, bodyScale: s, giant: false,
    bob: rng() * 6, hitR: 0.9 * s, hitCd: 0, nLegs: 0, alive: true,
    aquatic: true, shark: true, team: "horde", ranged: false, fireCd: 0,
    orbit: rng() * 6, orbitR: 2, sepJit: 0.8, jabPhase: 0, jabLimb: null,
    threatCd: 2, threatPose: 0, threatT: 0, threatDur: 1,
  };
}

function makeKraken(w, ang, dist) {
  const group = new THREE.Group();
  const col = 0x1a1218;
  const s = 1.35 + w * 0.025;
  const hub = new THREE.Mesh(new THREE.SphereGeometry(0.42, 8, 6), mat(col));
  group.add(hub);
  const limbs = [];
  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2;
    const tent = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.12, 1.4, 5), mat(col));
    tent.position.set(Math.cos(a) * 0.3, -0.4, Math.sin(a) * 0.3);
    tent.rotation.z = Math.cos(a) * 0.7;
    tent.rotation.x = Math.sin(a) * 0.7;
    const g = new THREE.Group();
    g.add(tent);
    g.userData = { live: true, thick: 0.16, dmg: 2, len: 1.2, asLeg: false, form: "tentacle", hits: 0, maxHits: 1, phase: a, basePitch: 0 };
    group.add(g);
    limbs.push(g);
  }
  const { x, z } = aquaPlace(ang, dist);
  const y = Math.max(hillsAt(x, z), WATER_LEVEL) + 0.15;
  group.position.set(x, y, z);
  group.scale.setScalar(s);
  scene.add(group);
  return {
    mesh: group, core: hub, limbs, hpLimbs: limbs.length,
    x, z, y, spd: 1.55 + w * 0.04, boost: 1.1, bodyScale: s, giant: true,
    bob: rng() * 6, hitR: 1.3 * s, hitCd: 0, nLegs: 0, alive: true,
    aquatic: true, kraken: true, team: "horde", ranged: true,
    shotShape: "spike", fireCd: 0.8, shotDef: { dmg: 2, rpm: 0.55, speed: 22, spread: 0.04 },
    orbit: rng() * 6, orbitR: 3, sepJit: 1, jabPhase: 0, jabLimb: null,
    threatCd: 2, threatPose: 0, threatT: 0, threatDur: 1,
  };
}
...
function spawnWave() {
  ...
  waveLeft = count;
  announcing = 2.6;
  const tag = runMode === "army" ? " · ARMY MAN" : runMode === "mixed" ? " · MIXED" : runMode === "war" ? " · WAR" : "";
...
function dripSpawn(dt) {
  ...
  while (pending > 0) {
    ...
    if (runMap === "shore" && aquaLive() < 8 && rng() < 0.32) spawnOneKind("aqua");
  }
}

function aquaLive() {
  return mobs.filter((m) => m.alive && m.aquatic).length;
}
...
function killMob(m, explode) {
  ...
  if (explode) {
    sfx.boom();
    const bitCol = m.army ? ARMY_GREEN : 0x111114;
    for (let i = 0; i < 8; i++) {
      const bit = new THREE.Mesh(new THREE.TetrahedronGeometry(0.08 + rng() * 0.08, 0), mat(bitCol));
  ...
  const lootN = m.army
    ? Math.max(1, Math.round((8 + ((rng() * 7) | 0) + (m.vehicle ? 5 : 0)) * 0.85))
...
function tickMobs(dt) {
  ...
  for (const m of mobs) {
    ...
    const ox = tgt.x + Math.cos(m.orbit + m.bob * 0.12) * hold * 0.35;
    const oz = tgt.z + Math.sin(m.orbit + m.bob * 0.12) * hold * 0.35;
    const tdx = ox - m.x;
    const tdz = oz - m.z;
    ...
    if (insect) {
      ...
      const ix = tgt.x + Math.cos(ang) * rad;
      const iz = tgt.z + Math.sin(ang) * rad;
      vx = (ix - m.x) * (2.6 + flyPow * 0.45);
      vz = (iz - m.z) * (2.6 + flyPow * 0.45);
    }
    m.leapCd = Math.max(0, (m.leapCd || 0) - dt);
    if (runMap === "shore" && !isFlyer(m) && !m.aquatic) {
      const nx = m.x + vx * 0.2, nz = m.z + vz * 0.2;
      if (inWater(nx, nz) && !inWater(m.x, m.z)) {
        vx *= 0.05;
        vz *= 0.15;
        if (inWater(m.x + 1.5, m.z)) vx -= 4;
        if (inWater(m.x - 1.5, m.z)) vx += 4;
      }
    }
    if (inWater(m.x, m.z) && !isFlyer(m)) {
      vx *= m.aquatic ? 1 : 0.4;
      vz *= m.aquatic ? 1 : 0.4;
    }
    if (!m.ride) {
    ...
    if (insect) {
      hover = (tgt.y || player.y) + 0.35 + nW * 0.4
        + Math.sin((m.buzzT || m.bob) * 6.4) * (1.55 + flyPow * 0.35)
        + Math.sin((m.buzzT || m.bob) * 12.2) * 0.7;
      hover = Math.max(groundY + 0.35, hover);
    }
    if (m.armyFly) {
      hover = Math.max(groundY + 3.4, (tgt.y || player.y) + 2.6 + Math.sin(m.bob * 0.85) * 0.55);
    }
    if (m.aquatic) {
      hover = inWater(m.x, m.z)
        ? WATER_LEVEL + (m.shark ? 0.42 : 0.22) + Math.sin(m.bob * 1.4) * 0.08
        : groundY + 0.08;
    }
    if (dodge.threat && nW) {
      const dodgeScale = 0.2 + 0.8 * Math.min(1, (diffWave() - 1) / 99);
    ...
    if (m.leaper && m.leaping) {
      ...
      if (inWater(m.x, m.z) && !isFlyer(m)) m.y = WATER_LEVEL - (m.bodyScale || 1) * 0.28;
      ...
      if (m.leaper && m.leapCd <= 0 && dist > midLo && dist < midHi) {
        ...
        let tx, tz;
        if (roll < 0.34) {
          tx = tgt.x + lastFwdX * 1.55;
          tz = tgt.z + lastFwdZ * 1.55;
        } else if (roll < 0.67) {
          tx = tgt.x - lastFwdX * 1.85;
          tz = tgt.z - lastFwdZ * 1.85;
        } else {
          tx = tgt.x;
          tz = tgt.z;
        }
    ...
    if (m.vehicle === "heli") {
      ...
      if (m.mesh.userData.rotor) m.mesh.userData.rotor.rotation.y = spin;
      if (m.mesh.userData.rotor2) m.mesh.userData.rotor2.rotation.y = -spin * 1.05;
    }
    for (const limb of m.limbs) {
      if (!limb.userData.live) continue;
    ...
    const ridingThis = player.ride === m;
    const combat = !ridingThis && (tgt.kind === "player" || tgt.kind === "mob");
    ...
    if (close) {
      ...
      if (m.jabLimb?.userData.live && !m.vehicle) {
      ...
      if (m.jabPhase >= 0.42 && dist < reach && m.hitCd <= 0) {
        const dmg = (m.jabLimb?.userData.dmg || 1);
        if (tgt.kind === "player" && iFrame <= 0) {
          damage(dmg);
        } else if (tgt.kind === "mob" && tgt.m?.alive) {
          const limb = nearestLiveLimb(tgt.m, new THREE.Vector3(m.x, m.y, m.z));
          if (limb) hitLimb(limb, tgt.m);
        }
        m.hitCd = 0.62;
        m.x -= (dx / dist) * 0.85;
        m.z -= (dz / dist) * 0.85;
    ...
    if (m.ranged && combat) {
      if (m.multiTurret) tickLandshipTurrets(m, dt);
      else {
  ...
  if (running && !dead && pending <= 0 && dripLeft <= 0 && wave > 0 && !runContinuous && !runOpen) {
...
function paintRadar() {
  ...
  for (const m of mobs) {
    if (!m.alive) continue;
    let col = "#ff3344";
    if (m.team === "army") col = "#3dcc55";
    if (runMode === "war" && !isHostileToPlayer(m)) col = "#66aaff";
    plot(m.x, m.z, col, m.vehicle ? 6 : 4, !!m.vehicle);
  }
...
function paintHud3d() {
  ...
  hudCtx.fillText(runMode === "war" ? "Green army  ·  red horde  ·  blue allies" : runMode === "army" ? "Green = army men" : "Gold flag waypoint  ·  red = horde", 16, 304);
...
function hud() {
  ...
  $("wave").textContent = runOpen ? "OPEN" : String(wave);
  paintHud3d();
}
...
function toggleDev() {
  ...
  if (devMode) {
    ...
    showBanner("SANDBOX ON  ·  INFINITE HP / GOLD");
  } else showBanner("SANDBOX OFF");
  announcing = 1.4;
  paintShop();
  paintShop3d();
  paintModeButtons();
...
function paintDevToggle() {
  const el = $("dev-toggle");
  if (!el) return;
  el.textContent = devMode ? "SANDBOX: ON" : "SANDBOX: OFF";
...
function damage(n) {
  ...
  if (player.landship && player.hullHp > 0) {
    const soak = Math.min(player.hullHp, n);
    player.hullHp -= soak;
    n -= soak;
    if (player.hullHp <= 0) {
      explodeAt(new THREE.Vector3(player.x, player.y, player.z), 6);
      player.landship = false;
      if (landshipMesh) landshipMesh.visible = false;
      if (player.wep === "landship") equip("pistol");
      showBanner("LANDSHIP DESTROYED");
      announcing = 1.4;
    }
    n = n * 0.5;
    if (n <= 0) return;
  } else if (player.landship) n *= 0.5;
  player.hp -= n;
  hurtT = 0.28;
  iFrame = 0.55;
  sfx.hurt();
...
function addMenuHit(x, y, w, h, kind, waveN, extra) {
  menuHits.push({ x, y, w, h, kind, wave: waveN || 1, extra });
}
...
function paintMenu3d(kind) {
  ...
  if (kind === "over") {
    ...
    const taken = runMode === "army" ? "TAKEN BY THE ARMY" : runMode === "war" ? "FALLEN IN THE WAR" : runMode === "mixed" ? "TAKEN BY BOTH SIDES" : "TAKEN BY THE HORDE";
    overCtx.fillText(taken, 40, 48);
    ...
    overCtx.fillStyle = "#111";
    overCtx.font = "700 56px Cinzel, serif";
    ...
    const modes = [["horde", "HORDE"], ["army", "ARMY"], ["mixed", "MIXED"], ["war", "WAR"], ["open", "OPEN"]];
...
function gameOver() {
  ...
  const kick = document.querySelector("#over .kicker");
  if (kick) {
    kick.textContent = runMode === "army" ? "Taken by the army" : runMode === "war" ? "Fallen in the war" : runMode === "mixed" ? "Taken by both sides" : "Taken by the horde";
  }
  paintLb();
  paintWaveButtons();
  paintOver3d();
...
function resetRun() {
  ...
  if (landshipMesh) { landshipMesh.removeFromParent(); landshipMesh = null; }
  if (bikeMesh) { bikeMesh.removeFromParent(); bikeMesh = null; }
  clearOpenWorld(owExtras);
  owExtras = null;
  zoneFlags = [];
  ...
  skyFlash = 0;
  clearHexes();
  for (const b of buildings) if (b.mesh) b.mesh.removeFromParent();
  buildings = [];
  if (waterMesh) { waterMesh.removeFromParent(); waterMesh = null; }
  if (fenceGroup) { fenceGroup.removeFromParent(); fenceGroup = null; }
  ...
  player = { x: 0, y: 1.6, z: 0, vx: 0, vy: 0, vz: 0, hp: MAX_HP0, grounded: true, coins: 0, ammo: 48, mag: 12, wep: "pistol", tank: false, heli: false, landship: false, jumpsLeft: 1, sprinting: false, sprintT: 0, sprintCdT: 0, mom: 0, pounding: false, bike: false, ride: null, hullHp: 0, hullMax: 0 };
  tankYaw = 0;
  heliYaw = 0;
  landshipYaw = 0;
  landshipTurretCd = [0, 0, 0];
  runOpen = false;
  dead = false;
  shopOpen = false;
  shopSel = 0;
...
async function startRunFrom(startWave) {
  ...
  if (overMesh) overMesh.visible = false;
  running = true;
  if (startWave > 1 && !runOpen) {
    wave = startWave - 1;
    player.coins = Math.max(player.coins, startWave * 50);
    applyPlanetTex(true);
  }
  startMusic();
  if (runOpen) {
    ...
    if (runContinuous) {
      ...
      spawnOneKind("army");
      spawnOneKind("horde");
      if (runMap === "shore") spawnOneKind("aqua");
      contSpawnT = 15 + rng() * 15;
...
function explodeAt(pos, r) {
  if (!pos) return;
  const rad = Math.max(0.7, r || 2.2);
  spawnMeteorBlast(pos, rad * 0.42);
  skyFlash = Math.max(skyFlash, 0.35 + Math.min(0.6, rad * 0.04));
}
...
function detonateShot(s) {
  ...
  if (s.def.missile || (s.def.aoe && (s.def.tank || s.def.heli || s.def.landship || s.missile))) {
    const p = s.mesh.position;
    const r = s.def.aoe || 5;
    craterAt(p.x, p.z, Math.min(4.2, r * 0.38), Math.min(1.8, r * 0.18));
    explodeAt(p, r);
  } else if (s.def.aoe && !s.def.nuke && !s.def.fireball && !s.def.lightning) {
    explodeAt(s.mesh.position, s.def.aoe);
  }
  if (s.def.fireball) {
...
function tickShots(dt) {
  for (let i = shots.length - 1; i >= 0; i--) {
    ...
    if (!s.dummy && s.mesh.position) {
      ...
      const groundPad = (s.missile || s.def?.missile || s.def?.aoe) ? 0.28 : (s.fireball ? 0.55 : -0.15);
      if (s.mesh.position.y < hy + groundPad) {
        detonateShot(s);
        if (s.orb) {
      ...
      if (s.life > 0 && owShotHit(owExtras, s.mesh.position)) {
        detonateShot(s);
        owHitSolid(owExtras, s.mesh.position, 0.4, s.def?.dmg || 2);
        s.life = 0;
      }
      if (s.orb && s.life > 0) {
      ...
      let hit = false;
      const prox = s.fireball ? 0.95 : (s.missile || s.def?.missile || s.def?.aoe ? 0.82 : 0.32);
      if (s.life > 0) {
      for (const m of mobs) {
        if (!m.alive) continue;
        if (!playerHitsMob(m)) continue;
...
function aoeAt(pos, r, def) {
  sfx.boom();
  damageBuildings(pos, r, def?.dmg || 2);
  for (const m of mobs) {
    if (!m.alive) continue;
    if (def?.team) {
      if (!factionsHostile(def.team, mobFaction(m))) continue;
    } else if (!playerHitsMob(m)) continue;
  ...
  if (def?.team && factionsHostile(def.team, playerFaction()) && iFrame <= 0) {
    if (Math.hypot(player.x - pos.x, player.z - pos.z) < r + 0.7) damage(def.dmg || 1);
  }
}
...
function fireEnemyShotFrom(m, tgt, def, off) {
  if (!tgt || tgt.kind === "none") return;
  const yaw = m.mesh.rotation.y || 0;
  const ox = off ? (Math.sin(yaw) * off.x - Math.cos(yaw) * off.z) : 0;
  const oz = off ? (Math.cos(yaw) * off.x + Math.sin(yaw) * off.z) : 0;
  const origin = new THREE.Vector3(m.x + ox, m.y + (off?.y || 0.75), m.z + oz);
  const aimY = tgt.kind === "player" ? player.y - 0.2 : (tgt.y || m.y);
  const dir = new THREE.Vector3(tgt.x - origin.x, aimY - origin.y, tgt.z - origin.z).normalize();
  const rocket = !!(def?.aoe || def?.missile);
  let mesh;
  if (rocket) {
    mesh = makeMissileMesh();
    aimBolt(mesh, dir);
  } else {
    mesh = makeBolt(0xffe14a, 0.03);
    aimBolt(mesh, dir);
  }
  mesh.position.copy(origin);
  scene.add(mesh);
  eShots.push({
    mesh, dir, speed: def?.speed || 28, life: 2.5,
    dmg: def?.dmg || 1, team: mobFaction(m), aoe: def?.aoe || 0, from: m, missile: rocket,
  });
  sfx.armyShoot();
}

function pickDistinctTargets(m, n) {
  const my = mobFaction(m);
  const cands = [];
  if (factionsHostile(my, playerFaction())) {
    cands.push({ kind: "player", x: player.x, y: player.y, z: player.z, d: Math.hypot(player.x - m.x, player.z - m.z) });
  }
  for (const o of mobs) {
    if (o === m || !o.alive) continue;
    if (!factionsHostile(my, mobFaction(o))) continue;
    cands.push({ kind: "mob", m: o, x: o.x, y: o.y, z: o.z, d: Math.hypot(o.x - m.x, o.z - m.z) });
  }
  cands.sort((a, b) => a.d - b.d);
  const out = [];
  const used = new Set();
  for (const c of cands) {
    const key = c.kind === "player" ? "player" : c.m;
...
function tickLandshipTurrets(m, dt) {
  const defs = m.turretDefs || [m.shotDef];
  const offs = m.turretOff || [{ x: 0, y: 0.8, z: 0 }];
  if (!m.turretCd) m.turretCd = defs.map(() => 0.2);
  const targs = pickDistinctTargets(m, defs.length);
  for (let i = 0; i < defs.length; i++) {
    m.turretCd[i] -= dt;
    const tgt = targs[i] || targs[0];
    if (!tgt || m.turretCd[i] > 0) continue;
    const d = Math.hypot(tgt.x - m.x, tgt.z - m.z);
    if (d < 4 || d > 52) continue;
    const rpm = defs[i].rpm || 0.5;
    m.turretCd[i] = (1 / Math.max(0.12, rpm)) * (0.75 + rng() * 0.35);
    fireEnemyShotFrom(m, tgt, defs[i], offs[i]);
  }
}

function tickPlayerLandshipGuns(dt) {
  if (!player.landship || shopOpen || dead) return;
  landshipTurretCd[0] -= dt;
  landshipTurretCd[1] -= dt;
  const defs = [
    { dmg: 7, rpm: 1.4, speed: 52, spread: 0.012, aoe: 5.4, missile: true },
    { dmg: 2, rpm: 6, speed: 46, spread: 0.035, aoe: 0 },
  ];
  const offs = [
    { x: 0.55, y: 1.4, z: 0.2 },
    { x: -0.55, y: 1.35, z: 1.4 },
  ];
  const targs = pickDistinctTargets({ x: player.x, z: player.z, team: playerFaction(), mesh: { rotation: { y: landshipYaw } } }, 4)
    .filter((t) => t.kind !== "player");
  for (let i = 0; i < 2; i++) {
    const tgt = targs[i];
    if (!tgt || landshipTurretCd[i] > 0) continue;
    const d = Math.hypot(tgt.x - player.x, tgt.z - player.z);
    if (d < 5 || d > 48) continue;
    landshipTurretCd[i] = 1 / defs[i].rpm;
    const yaw = landshipYaw;
    const off = offs[i];
    const ox = Math.sin(yaw) * off.x - Math.cos(yaw) * off.z;
    const oz = Math.cos(yaw) * off.x + Math.sin(yaw) * off.z;
    const origin = new THREE.Vector3(player.x + ox, heightAt(player.x, player.z) + off.y, player.z + oz);
    const dir = new THREE.Vector3(tgt.x - origin.x, tgt.y - origin.y, tgt.z - origin.z).normalize();
    if (defs[i].missile) {
      const rocket = makeMissileMesh();
      rocket.position.copy(origin);
      aimBolt(rocket, dir);
      scene.add(rocket);
      shots.push({ mesh: rocket, dir, speed: defs[i].speed, life: 2.3, def: { ...WEPS.heli, ...defs[i] }, pierce: 1, hitSet: new Set(), missile: true });
    } else {
      const bolt = makeBolt(0x44ddff, 0.03);
      bolt.position.copy(origin);
      aimBolt(bolt, dir);
      scene.add(bolt);
      shots.push({ mesh: bolt, dir, speed: defs[i].speed, life: 0.7, def: WEPS.smg, pierce: 1, hitSet: new Set() });
    }
    sfx.shoot();
  }
}
...
function rideHeight(v) {
  if (!v) return 1.6;
  if (v.vehicle === "heli") return 1.15;
  if (v.vehicle === "landship") return 1.85;
  if (v.vehicle === "tank") return 1.55;
  if (v.vehicle === "humvee") return 1.45;
  return 1.25;
}

function nearestRideVehicle() {
  let best = null, bd = 3.6;
  for (const m of mobs) {
    if (!m.alive || !m.vehicle) continue;
    const d = Math.hypot(m.x - player.x, m.z - player.z);
    const dy = Math.abs(m.y - player.y);
    if (d < bd && dy < 3.4) { bd = d; best = m; }
  }
  return best;
}
...
function dismountRide() {
  ...
  player.ride = null;
  if (v && v.alive) {
    const yaw = v.mesh.rotation.y || 0;
    player.x = v.x + Math.cos(yaw) * 2.1;
    player.z = v.z + Math.sin(yaw) * 2.1;
    player.y = heightAt(player.x, player.z) + 1.6;
  }
  player.grounded = true;
  showBanner("DISMOUNT");
  announcing = 0.8;
...
function toggleRide() {
  if (!running || dead || shopOpen || menuOpen) return;
  if (player.tank || player.heli || player.landship) {
    showBanner("LEAVE YOUR VEHICLE FIRST");
    announcing = 1;
    return;
  }
  if (player.ride) {
  ...
  const v = nearestRideVehicle();
  if (!v) {
    showBanner("NO VEHICLE");
    announcing = 0.8;
    return;
  }
  player.ride = v;
  player.bike = false;
  showBanner("RIDING " + String(v.vehicle || "VEHICLE").toUpperCase());
  announcing = 1.1;
}

function fireEnemyShot(m, tgt) {
  tgt = tgt || pickMobTarget(m);
  if (!tgt || tgt.kind === "none") return;
  const origin = new THREE.Vector3(m.x, m.y + (m.vehicle ? 0.75 : 0.45), m.z);
  const aimY = tgt.kind === "player" ? player.y - 0.2 : (tgt.y || m.y);
  const base = new THREE.Vector3(tgt.x - origin.x, aimY - origin.y, tgt.z - origin.z).normalize();
  const def = m.shotDef;
  ...
  for (let p = 0; p < pellets; p++) {
    ...
    if (spread) {
      ...
      dir.y += (rng() - 0.5) * spread * 0.4;
      dir.z += (rng() - 0.5) * spread;
      dir.normalize();
    }
    let mesh;
    const rocket = army && (m.shotShape === "rocket" || def?.aoe || def?.missile);
    if (army) {
      ...
      if (rocket) {
        ...
        mesh = new THREE.Mesh(new THREE.SphereGeometry(0.034, 7, 6), new THREE.MeshBasicMaterial({ color: 0xffe14a }));
      }
    } else {
      ...
      else if (shape === "spike") geo = new THREE.ConeGeometry(0.1, 0.55, 5);
      else geo = new THREE.SphereGeometry(0.14, 7, 6);
      mesh = new THREE.Mesh(geo, new THREE.MeshBasicMaterial({ color: shape === "spike" ? 0xccaa88 : 0xff3344 }));
      if (shape === "spike") mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir);
    }
    mesh.position.copy(origin);
    scene.add(mesh);
    eShots.push({
      mesh,
      dir,
      speed: def?.speed || (13 + diffWave() * 0.12),
      life: rocket ? 2.6 : 2.4,
      dmg: def?.dmg || 1,
      team: mobFaction(m),
      aoe: def?.aoe || 0,
...
function tickEshots(dt) {
  for (let i = eShots.length - 1; i >= 0; i--) {
    ...
    const team = s.team || "horde";
    if (hitP && iFrame <= 0 && factionsHostile(team, playerFaction())) {
      sfx.impact();
      if (s.aoe) {
        explodeAt(s.mesh.position, s.aoe);
        aoeAt(s.mesh.position.clone(), s.aoe, { dmg: s.dmg || 1, team, knock: 1.2 });
      } else damage(s.dmg || 1);
      s.life = 0;
    } else {
      let hitM = false;
      for (const m of mobs) {
        if (!m.alive || m === s.from) continue;
        if (!factionsHostile(team, mobFaction(m))) continue;
        if (Math.hypot(m.x - s.mesh.position.x, m.z - s.mesh.position.z) > m.hitR + 0.35) continue;
        if (Math.abs(m.y - s.mesh.position.y) > 1.6 + m.hitR) continue;
        sfx.impact();
        if (s.aoe) {
          explodeAt(s.mesh.position, s.aoe);
          aoeAt(s.mesh.position.clone(), s.aoe, { dmg: s.dmg || 1, team, knock: 1.2 });
      ...
      if (hitM) s.life = 0;
      else if (s.mesh.position.y < hy + (s.aoe ? 0.25 : -0.1)) {
        sfx.impact();
        if (s.aoe) {
          explodeAt(s.mesh.position, s.aoe * 0.7);
          aoeAt(s.mesh.position.clone(), s.aoe * 0.7, { dmg: s.dmg || 1, team, knock: 0.8 });
        }
        s.life = 0;
      } else if (owShotHit(owExtras, s.mesh.position)) {
        if (s.aoe) {
          explodeAt(s.mesh.position, s.aoe);
          aoeAt(s.mesh.position.clone(), s.aoe, { dmg: s.dmg || 1, team, knock: 1.2 });
        }
...
function spawnLandmark() {
  if (runMap === "shore" || runMap === "wastes") return null;
  const ang = rng() * Math.PI * 2;
...
function spawnHexCluster() {
  if (runMap === "shore") {
    if (buildings.length >= 28) return;
    const ang = rng() * Math.PI * 2;
    const dist = 28 + rng() * 56;
    let cx = player.x + Math.cos(ang) * dist;
    const cz = player.z + Math.sin(ang) * dist;
    if (cx > 12) cx = 4 + rng() * 8;
    spawnCityBuilding(cx, cz, 3 + ((rng() * 7) | 0));
    return;
  }
  if (runMap === "wastes") {
    const ang = rng() * Math.PI * 2;
    const dist = 26 + rng() * 62;
    const cx = player.x + Math.cos(ang) * dist;
    const cz = player.z + Math.sin(ang) * dist;
    const roll = rng();
    if (roll < 0.26) spawnClockTower(cx, cz);
    else if (roll < 0.52) spawnSpire(cx, cz);
    else spawnBrickWall(cx, cz);
    return;
  }
...
function ensureHexes(dt) {
  ...
  for (let i = buildings.length - 1; i >= 0; i--) {
    const b = buildings[i];
    if (Math.hypot(b.x - player.x, b.z - player.z) > 200) {
      if (b.mesh) b.mesh.removeFromParent();
      buildings.splice(i, 1);
    }
  }
  if (runMap === "shore") {
    if (buildings.length < 16 && hexSpawnCd <= 0) {
      spawnHexCluster();
      hexSpawnCd = 2.2;
    }
    return;
...
function tickTurrets(dt) {
  for (let i = turrets.length - 1; i >= 0; i--) {
    ...
    for (const m of mobs) {
      if (!m.alive) continue;
      if (!isHostileToPlayer(m)) continue;
...
function makeLandshipMesh() {
  const g = new THREE.Group();
  const hull = new THREE.Mesh(new THREE.BoxGeometry(1.85, 0.7, 4.4), mat(0x3a4a40, { emissive: 0x111 }));
  hull.position.y = 0.55;
  const deck = new THREE.Mesh(new THREE.BoxGeometry(1.55, 0.32, 2.2), mat(0x2a332c));
  deck.position.set(0, 1.02, 0.15);
  const t1 = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.46, 0.32, 10), mat(0x2a332c));
  t1.position.set(0, 1.32, -1.35);
  const b1 = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.07, 1.7, 8), mat(0x1a1e1a));
  b1.rotation.x = Math.PI / 2;
  b1.position.set(0, 1.34, -2.25);
  const t2 = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.32, 0.24, 8), mat(0x2a332c));
  t2.position.set(0.52, 1.22, 0.25);
  const b2 = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.045, 1.05, 8), mat(0x1a1e1a));
  b2.rotation.x = Math.PI / 2;
  b2.position.set(0.52, 1.24, -0.35);
  const t3 = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.28, 0.22, 8), mat(0x2a332c));
  t3.position.set(-0.52, 1.18, 1.45);
  const b3 = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.035, 0.85, 8), mat(0x1a1e1a));
  b3.rotation.x = Math.PI / 2;
  b3.position.set(-0.52, 1.2, 0.9);
  const skirtL = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.38, 4.2), mat(0x1a1e1a));
  skirtL.position.set(-0.95, 0.32, 0);
  const skirtR = skirtL.clone();
  skirtR.position.x = 0.95;
  g.add(hull, deck, t1, b1, t2, b2, t3, b3, skirtL, skirtR);
  g.userData.turrets = [t1, t2, t3];
  return g;
}
...
function tickDrones(dt) {
  for (let i = drones.length - 1; i >= 0; i--) {
    ...
    for (const m of mobs) {
      if (!m.alive || !isHostileToPlayer(m)) continue;
...
function spawnMeteor(ox, oz) {
  const ang = rng() * Math.PI * 2;
  const dist = 14 + rng() * 72;
  const tx = ox != null ? ox : player.x + Math.cos(ang) * dist;
  const tz = oz != null ? oz : player.z + Math.sin(ang) * dist;
  const sx = tx + (rng() - 0.5) * 48;
  const sz = tz + (rng() - 0.5) * 48;
  const sy = 62 + rng() * 24;
  const sizeMul = runMap === "wastes" ? 0.5 + rng() : 1;
  const rad = (3.2 + rng() * 2.8) * sizeMul;
...
function tickMeteors(dt) {
  ...
  if (meteorT <= 0 && running && !dead && !shopOpen) {
    meteorT = (16 + rng() * 38) / (runMap === "wastes" ? 1.5 : 1);
    if (rng() < 0.72) {
      const a = spawnMeteor();
      if (runMap === "wastes" && rng() < 0.1 && a) {
        spawnMeteor(a.tx + (rng() - 0.5) * 9, a.tz + (rng() - 0.5) * 9);
      }
    }
  }
...
function spawnMushroomCloud(pos, scale) {
  scale = scale || 2.6;
  const g = new THREE.Group();
  const stem = new THREE.Mesh(
    new THREE.CylinderGeometry(scale * 0.55, scale * 1.15, scale * 4.2, 12),
    new THREE.MeshBasicMaterial({ color: 0xffaa44, transparent: true, opacity: 0.82, blending: THREE.AdditiveBlending, depthWrite: false }),
  );
  stem.position.y = scale * 2.1;
  const cap = new THREE.Mesh(
    new THREE.SphereGeometry(scale * 2.4, 16, 12),
    new THREE.MeshBasicMaterial({ color: 0xff6622, transparent: true, opacity: 0.78, blending: THREE.AdditiveBlending, depthWrite: false }),
  );
  cap.scale.y = 0.45;
  cap.position.y = scale * 4.4;
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(scale * 2.8, scale * 0.45, 10, 24),
    new THREE.MeshBasicMaterial({ color: 0xffee88, transparent: true, opacity: 0.55, blending: THREE.AdditiveBlending, depthWrite: false }),
  );
  ring.rotation.x = Math.PI / 2;
...
function smashNear(pos, r) {
  ...
  damageBuildings(pos, r, 4);
}

function damageBuildings(pos, r, dmg) {
  dmg = dmg || 2;
  for (const b of buildings) {
    for (const cell of b.cells) {
      if (!cell.live) continue;
      const d = Math.hypot(cell.x - pos.x, cell.z - pos.z);
      const dy = Math.abs((cell.y || 0) - pos.y);
      if (d > r + cell.w * 0.8 || dy > cell.h + r * 0.35) continue;
      cell.hp -= dmg;
...
function destroyBuildingCell(b, cell) {
  cell.live = false;
  if (cell.mesh) {
    const p = cell.mesh.getWorldPosition(new THREE.Vector3());
    spawnMeteorBlast(p, 0.9);
    cell.mesh.removeFromParent();
  }
  sfx.boom();
  for (const n of b.cells) {
    if (!n.live || !n.mesh) continue;
    const same = n.floor === cell.floor && (Math.abs(n.corner - cell.corner) === 1 || Math.abs(n.corner - cell.corner) === 3);
    const above = n.floor === cell.floor + 1 && n.corner === cell.corner;
    const below = n.floor === cell.floor - 1 && n.corner === cell.corner;
    if (!same && !above && !below) continue;
    const jag = new THREE.Mesh(
      new THREE.BoxGeometry(same ? 0.22 + rng() * 0.22 : n.w * (0.5 + rng() * 0.4), same ? n.h * (0.35 + rng() * 0.45) : 0.16, 0.18 + rng() * 0.2),
      mat(0x6a6058),
    );
    jag.position.set(
      same ? (cell.x - n.x) * 0.45 : (rng() - 0.5) * 0.3,
      above ? 0.08 : below ? n.h - 0.08 : rng() * 0.45,
      same ? (cell.z - n.z) * 0.45 : (rng() - 0.5) * 0.3,
    );
    jag.rotation.z = (rng() - 0.5) * 0.45;
    jag.rotation.x = (rng() - 0.5) * 0.25;
    n.mesh.add(jag);
  }
  if (!b.cells.some((c) => c.live) && b.mesh) b.mesh.removeFromParent();
}

function spawnCityBuilding(cx, cz, floors) {
  floors = floors || (4 + ((rng() * 6) | 0));
  const foot = 6.4 + rng() * 3.2;
  const group = new THREE.Group();
  const glass = mat(0x88aacc, { transparent: true, opacity: 0.35 });
  const conc = mat(0x8a8680);
  const cells = [];
  const fh = 3.15;
  const hw = foot * 0.25;
  ...
  for (let f = 0; f < floors; f++) {
    for (let c = 0; c < 4; c++) {
      ...
      const slab = new THREE.Mesh(new THREE.BoxGeometry(hw * 2, 0.16, hw * 2), conc);
      slab.position.y = 0.08;
      const wallX = new THREE.Mesh(new THREE.BoxGeometry(0.18, fh, hw * 2), conc);
      wallX.position.set(sx * hw, fh * 0.5, 0);
      const wallZ = new THREE.Mesh(new THREE.BoxGeometry(hw * 2, fh, 0.18), conc);
      wallZ.position.set(0, fh * 0.5, sz * hw);
      const win = new THREE.Mesh(new THREE.BoxGeometry(hw * 1.2, fh * 0.45, 0.05), glass);
      win.position.set(sx * (hw - 0.12), fh * 0.55, sz * (hw - 0.12));
      cellG.add(slab, wallX, wallZ, win);
      const lx = sx * hw;
      const lz = sz * hw;
      cellG.position.set(lx, f * fh, lz);
      group.add(cellG);
      cells.push({
        live: true, hp: 4 + f, mesh: cellG, floor: f, corner: c,
        x: cx + lx, z: cz + lz, y: base + f * fh + fh * 0.5,
        w: hw, d: hw, h: fh, top: base + (f + 1) * fh,
      });
    }
  }
  group.position.set(cx, base, cz);
  scene.add(group);
  const b = { x: cx, z: cz, r: foot * 0.8, floors, cells, mesh: group };
  buildings.push(b);
  return b;
}

function spawnBrickWall(cx, cz) {
  const group = new THREE.Group();
...
function spawnSpire(cx, cz) {
  const group = new THREE.Group();
  const black = mat(0x050508, { emissive: 0x111 });
  const base = hillsAt(cx, cz);
  const h = 18 + rng() * 16;
...
function spawnClockTower(cx, cz) {
  const group = new THREE.Group();
  const black = mat(0x08080c);
  const dark = mat(0x1a1a22);
  const base = hillsAt(cx, cz);
  const h = 38 + rng() * 18;
  const r = 4.6 + rng() * 1.8;
  const floors = 6;
...
function spawnWaterAndFence() {
  if (waterMesh) { waterMesh.removeFromParent(); waterMesh = null; }
  if (fenceGroup) { fenceGroup.removeFromParent(); fenceGroup = null; }
  if (runMap !== "shore") return;
  waterMesh = new THREE.Mesh(
    new THREE.PlaneGeometry(MAP_EDGE, MAP_EDGE * 2.15, 1, 1),
    new THREE.MeshLambertMaterial({ color: 0x1a4a6a, transparent: true, opacity: 0.72 }),
  );
  waterMesh.rotation.x = -Math.PI / 2;
  waterMesh.position.set(16 + MAP_EDGE * 0.5, WATER_LEVEL, 0);
  scene.add(waterMesh);
  fenceGroup = new THREE.Group();
  const white = mat(0xf4f1ea);
  const post = mat(0xe8e0d4);
  function rail(x0, z0, x1, z1) {
    const dx = x1 - x0, dz = z1 - z0;
    const len = Math.hypot(dx, dz) || 1;
    const n = Math.max(4, (len / 8) | 0);
    for (let i = 0; i <= n; i++) {
      const t = i / n;
      const px = x0 + dx * t, pz = z0 + dz * t;
      const p = new THREE.Mesh(new THREE.BoxGeometry(0.12, 1.15, 0.12), post);
      p.position.set(px, hillsAt(px, pz) + 0.55, pz);
      fenceGroup.add(p);
    }
    const railM = new THREE.Mesh(new THREE.BoxGeometry(len, 0.08, 0.06), white);
    railM.position.set((x0 + x1) / 2, 0.7, (z0 + z1) / 2);
    railM.rotation.y = Math.atan2(dx, dz);
    fenceGroup.add(railM);
  }
  const e = MAP_EDGE;
  rail(-e, -e, e, -e); rail(-e, e, e, e); rail(-e, -e, -e, e); rail(e, -e, e, e);
  scene.add(fenceGroup);
}

function spawnMapWorld() {
  spawnWaterAndFence();
  if (runMap === "shore") {
    for (let i = 0; i < 10; i++) {
      const z = -80 + i * 18 + (rng() - 0.5) * 6;
      spawnCityBuilding(8 + rng() * 5, z, 5 + ((rng() * 7) | 0));
    }
    for (let i = 0; i < 6; i++) spawnCityBuilding(-20 - rng() * 40, (rng() - 0.5) * 90, 3 + ((rng() * 4) | 0));
    return;
  }
  if (runMap === "wastes") {
    for (let i = 0; i < 5; i++) {
      const a = rng() * Math.PI * 2, d = 22 + rng() * 70;
      spawnClockTower(player.x + Math.cos(a) * d, player.z + Math.sin(a) * d);
    }
    for (let i = 0; i < 7; i++) {
      const a = rng() * Math.PI * 2, d = 18 + rng() * 80;
      spawnSpire(player.x + Math.cos(a) * d, player.z + Math.sin(a) * d);
    }
    for (let i = 0; i < 16; i++) {
      const a = rng() * Math.PI * 2, d = 12 + rng() * 90;
      spawnBrickWall(player.x + Math.cos(a) * d, player.z + Math.sin(a) * d);
    }
  }
}
...
function tickLandship() {
  if (!player.landship) {
    if (landshipMesh) landshipMesh.visible = false;
    return;
  }
  if (!landshipMesh) {
    landshipMesh = makeLandshipMesh();
    scene.add(landshipMesh);
  }
  landshipMesh.visible = true;
  landshipMesh.position.set(player.x, heightAt(player.x, player.z), player.z);
  landshipMesh.rotation.y = landshipYaw;
}
...
function buy(it) {
  ...
  if (it.kind === "wep") {
    ...
    player.heli = false;
    player.landship = false;
    if (player.wep === "tank" || player.wep === "heli" || player.wep === "landship") equip("pistol");
    player.bike = true;
  } else if (it.kind === "up") {
    if (it.id === "sprint") {
  ...
  } else if (it.kind === "allyveh") {
    if (!debit(it.cost)) return false;
    if (!spawnHiredVehicle(it.id)) {
      player.coins += it.cost;
      return false;
    }
  } else return false;
...
function drawShopIcon(ctx, it, x, y, s) {
  ...
  if (it.kind === "troop") {
    ...
    if (id === "heli") {
      ...
      ctx.fillRect(-s * 0.4, -s * 0.02, s * 0.8, s * 0.05);
      ctx.fillRect(s * 0.12, -s * 0.22, s * 0.05, s * 0.18);
    } else if (id === "tank") {
      ctx.fillRect(-s * 0.32, -s * 0.02, s * 0.64, s * 0.22);
      ctx.fillRect(-s * 0.16, -s * 0.16, s * 0.32, s * 0.16);
      ctx.fillRect(-s * 0.42, -s * 0.1, s * 0.28, s * 0.06);
    } else if (id === "landship") {
      ctx.fillRect(-s * 0.4, 0, s * 0.8, s * 0.18);
      ctx.fillRect(-s * 0.22, -s * 0.16, s * 0.16, s * 0.16);
      ctx.fillRect(s * 0.04, -s * 0.14, s * 0.12, s * 0.14);
      ctx.fillRect(s * 0.22, -s * 0.12, s * 0.1, s * 0.12);
    } else {
      ctx.fillRect(-s * 0.3, -s * 0.02, s * 0.6, id === "humvee" ? s * 0.22 : s * 0.16);
      ctx.fillStyle = "#243226";
      ctx.beginPath(); ctx.arc(-s * 0.2, s * 0.16, s * 0.08, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(s * 0.2, s * 0.16, s * 0.08, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#ffe08a";
      ctx.fillRect(-s * 0.12, -s * 0.16, s * 0.08, s * 0.14);
      ctx.fillRect(s * 0.04, -s * 0.16, s * 0.08, s * 0.14);
    }
  } else if (it.kind === "up") {
    if (id === "hp") {
      ctx.fillStyle = "#c42b2b";
    ...
    ctx.fillRect(-s * 0.42, -s * 0.06, s * 0.84, s * 0.2);
    ctx.fillRect(-s * 0.28, -s * 0.2, s * 0.18, s * 0.16);
    ctx.fillRect(s * 0.02, -s * 0.18, s * 0.14, s * 0.14);
    ctx.fillRect(s * 0.22, -s * 0.16, s * 0.12, s * 0.12);
  } else if (id === "heli") {
    ctx.fillRect(-s * 0.28, -s * 0.06, s * 0.56, s * 0.16);
...
function paintShopCard(it, i, highlight) {
  ...
  if (it.kind === "allyveh") price = it.cost + " ◎  ·  deploy another";
...
function paintTeamColorRow() {
  const host = $("team-colors");
  const wrap = $("team-color-row");
  if (!host || !wrap) return;
  wrap.hidden = !runOpen && gameMode !== "open";
  host.innerHTML = "";
  for (const c of TEAM_COLORS) {
    const b = document.createElement("button");
    b.type = "button";
    b.className = "mode-btn team-swatch" + (teamColorId === c.id ? " on" : "");
    b.style.background = "#" + c.hex.toString(16).padStart(6, "0");
    b.style.color = c.id === "white" || c.id === "gold" ? "#111" : "#f4f1ea";
    b.textContent = c.name;
    b.onclick = () => setTeamColor(c.id);
    host.appendChild(b);
  }
}
...
function rebuildShopCards() {
  ...
  tctx.fillText(runOpen ? "COMMAND  ·  team color + armory" : "ARMORY  ·  point and shoot a card to buy", 28, 48);
  tctx.fillStyle = "#111";
  tctx.font = "800 58px Cinzel, serif";
  tctx.fillText((runOpen ? "GEAR   " : "BUY   ") + (player.coins | 0) + " ◎", 28, 118);
  ...
  dx.textAlign = "center";
  dx.fillText(devMode ? "SANDBOX ON" : "SANDBOX", 160, 50);
  ...
  TEAM_COLORS.forEach((c, i) => {
    const sw = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: c.hex, side: THREE.DoubleSide }),
    );
    sw.position.set(-0.72 * 1.1 + i * 0.1, 0.86, 0.05);
    sw.userData.teamColor = c.id;
    shopRoot.add(sw);
    shopHits.push(sw);
    if (teamColorId === c.id) {
      const ring = new THREE.Mesh(
        new THREE.RingGeometry(0.042, 0.05, 16),
        new THREE.MeshBasicMaterial({ color: 0xffffff, side: THREE.DoubleSide }),
      );
      ring.position.copy(sw.position);
...
function tintShopSel() {
  for (const m of shopHits) {
    if (!m.material || m.userData.teamColor) continue;
...
function tryShopShot(xr) {
  ...
  if (obj?.userData?.teamColor) {
    setTeamColor(obj.userData.teamColor);
    showBanner("TEAM " + obj.userData.teamColor.toUpperCase());
    announcing = 0.9;
    return true;
  }
  ...
  if (uv && menuHits.length) {
    ...
    for (const hit of menuHits) {
      if (x >= hit.x && x <= hit.x + hit.w && y >= hit.y && y <= hit.y + hit.h) {
        ...
        else if (hit.kind === "map") { setSelectedMap(hit.extra); paintMenu3d("main"); }
        else if (hit.kind === "sandbox") { toggleDev(); paintMenu3d("main"); }
...
function pollXr() {
  ...
  if (!session) return { moveX: 0, moveY: 0, lookX: 0, lookY: 0, jump: false, fire: false, fireTap: false, reload: false, shop: false, lClick: false, rClick: false, flash: false, cycle: false, menu: false, ride: false, right: null, leftFire: false };
  ...
  return {
    ...
    ride: !!(right && right.gripBtn && !right.gripPrev),
  };
}
...
function physics(dt, xr) {
  ...
  let wishX = 0, wishZ = 0;
  const drive = player.tank || player.heli || player.landship;
  if (drive) {
    const turn = player.heli ? heliYaw : player.landship ? landshipYaw : tankYaw;
    const turnSpd = player.heli ? 1.45 : player.landship ? 1.15 : 1.7;
    if (xrOn) {
      const next = turn - (shopOpen ? 0 : xr.moveX) * turnSpd * dt;
      if (player.heli) heliYaw = next; else if (player.landship) landshipYaw = next; else tankYaw = next;
      ...
      let next = turn;
      if (keys.has("KeyA") || keys.has("ArrowLeft")) next += (player.heli ? 1.5 : player.landship ? 1.2 : 1.8) * dt;
      if (keys.has("KeyD") || keys.has("ArrowRight")) next -= (player.heli ? 1.5 : player.landship ? 1.2 : 1.8) * dt;
      if (player.heli) heliYaw = next; else if (player.landship) landshipYaw = next; else tankYaw = next;
      let fwd = 0;
  ...
  if (stats.sprint && !player.tank && !player.heli && !player.landship && !shopOpen) {
  ...
  let spd = (player.heli ? 12.2 : player.landship ? 3.5 : player.tank ? 4.6 : player.bike ? 9.4 : 6.4) * stats.speed * (shopOpen ? 0.12 : 1);
  if (player.sprinting) spd *= 1.7 * (stats.sprintMul || 1);
  spd *= 1 + player.mom;
  if (inWater(player.x, player.z) && !player.heli && !player.tank && !player.landship) spd *= 0.6;
  player.vx = wishX * spd;
  player.vz = wishZ * spd;
  const canJump = !player.tank && !player.heli && !player.landship && (player.grounded || player.jumpsLeft > 0);
...
function doFire(xr) {
  ...
  player.pounding = !!(airborne && !player.tank && !player.heli && !player.landship && dir.y < -0.62);
...
function loop() {
  ...
  const xr = xrOn ? pollXr() : { moveX: 0, moveY: 0, lookX: 0, lookY: 0, jump: false, fire: false, fireTap: false, reload: false, shop: false, lClick: false, rClick: false, flash: false, cycle: false, menu: false, ride: false, right: null };
  if (xr.menu) toggleMainMenu();
  if (!dead && !menuOpen && xr.shop) toggleShop();
  if (!dead && !menuOpen && xr.ride) toggleRide();
  ...
  if (dead || menuOpen) {
    ...
    if (shopOpen) {
      ...
      tickHeli();
      tickLandship();
      tickPlayerLandshipGuns(dt);
      if (owExtras) tickOpenWorld(owExtras, dt, owApi());
      tickDebris(dt);
      tickLoot(dt);
      tickAmmoField(dt);
      tickFlag(dt);
...
document.querySelectorAll("[data-map]").forEach((b) => {
  b.onclick = () => setSelectedMap(b.dataset.map);
});
if ($("sandbox-btn")) $("sandbox-btn").onclick = () => toggleDev();
paintModeButtons();
try {
