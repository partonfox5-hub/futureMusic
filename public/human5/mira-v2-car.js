...
export class Car {
 ...
 build(){
  ...
  for(let i=0;i<3;i++){const dot=new T.Mesh(new T.BoxGeometry(.07,.008,.010),alloy);dot.position.set(0,.002,(i-1)*.075);this.shifterBase.add(dot);}
  if(typeof document!=='undefined'){const c=document.createElement('canvas');c.width=128;c.height=256;const ctx=c.getContext('2d');ctx.fillStyle='#182126';ctx.fillRect(0,0,128,256);ctx.fillStyle='#eeeecc';ctx.font='bold 48px sans-serif';ctx.textAlign='center';for(let i=0;i<3;i++)ctx.fillText(GEARS[i],64,58+i*75);this.gearTexture=new T.CanvasTexture(c);const labels=new T.Mesh(new T.PlaneGeometry(.072,.23),new T.MeshBasicMaterial({map:this.gearTexture}));labels.rotation.set(-Math.PI/2,0,0);labels.position.set(-.075,.008,0);this.shifterBase.add(labels);}
  this.wheelRoot=new T.Group();this.wheelRoot.position.set(-.39,1.035,-.53);this.wheelRoot.rotation.x=-.22;this.group.add(this.wheelRoot);this.steeringWheel=new T.Group();this.wheelRoot.add(this.steeringWheel);this.steeringWheel.add(new T.Mesh(new T.TorusGeometry(.172,.018,8,24),trim));for(let i=0;i<3;i++){const s=new T.Mesh(new T.BoxGeometry(.029,.17,.025),alloy);s.position.y=-.07;s.rotation.z=i*Math.PI*2/3;this.steeringWheel.add(s);}const hub=new T.Mesh(new T.CylinderGeometry(.045,.045,.033,12),trim);hub.rotation.x=Math.PI/2;this.steeringWheel.add(hub);
 ...
 reset(){
  ...
  for(const w of this.wheels){this.group.add(w.group);w.group.position.set(w.side*.89,.34,w.z);w.group.rotation.set(0,0,0);w.group.scale.set(1,1,1);w.spin.rotation.set(0,0,0);w.popped=w.broken=false;w.health=w.max;w.roll=w.omega=0;w.compression=.035;w.load=this.mass*9.81/4;}
 ...
 setGear(gear){if(!GEARS.includes(gear))return false;this.gear=gear;this.shifterLever.position.z=(GEARS.indexOf(gear)-1)*.075;this.updateOccupancy();return true;}
 cycleGear(){this.setGear(GEARS[(GEARS.indexOf(this.gear)+1)%3]);return this.gear;}
 ...
 grip(i){
  ...
  const local=this.wheelRoot.worldToLocal(p.clone()),radius=Math.hypot(local.x,local.y);if(this.driving&&Math.abs(local.z)<.16&&Math.abs(radius-.172)<.11){this.grips.set(i,{type:'wheel',angle:Math.atan2(local.y,local.x),steer:this.steerTarget});return true;}
  for(const h of this.hinges){if(h.panel.broken||h.grabbed!==null)continue;let grab=h.handle.getWorldPosition(V());if(h.kind!=='door'){grab=h.root.localToWorld(new T.Vector3(0,-.10,h.kind==='hood'?-.95:.90));}if(p.distanceTo(grab)<.085){const local=this.group.worldToLocal(p.clone()).sub(h.pivot);this.grips.set(i,{type:'hinge',hinge:h,start:this.hingeHandAngle(h,local),angle:h.angle});h.grabbed=i;h.latched=false;h.velocity=0;return true;}}
 ...
 release(i){const g=this.grips.get(i);if(!g)return;if(g.type==='shifter'){const j=clamp(Math.round(this.shifterLever.position.z/.075)+1,0,2);this.setGear(GEARS[j]);this.pulse(i);}if(g.type==='hinge'){const h=g.hinge;h.grabbed=null;h.target=h.angle<8*Math.PI/180?0:h.max;h.latched=h.target===0;this.pulse(i);}this.grips.delete(i);}
 click(ray){const hit=this.props.hit(ray,15,false);if(hit?.object.userData.carControl==='shifter'){this.cycleGear();return true;}const h=hit?.object.userData.carHinge;if(h&&!h.panel.broken){h.target=h.target>0?0:h.max;h.latched=false;return true;}return hit?.object.userData.carPart?this.enter():false;}
 ...
 step(dt,input){
  ...
  let newSpeed=this.velocity.dot(forward);if(newSpeed>28)this.velocity.addScaledVector(forward,28-newSpeed);if(newSpeed< -4)this.velocity.addScaledVector(forward,-4-newSpeed);
 ...
 collide(dt){
  ...
  for(const point of points){for(const o of this.world.nearby(point,.16)||[]){
    ...
    if(closing<-.20){
      const J=this.mass*(-closing)*1.08,impulse=n.clone().multiplyScalar(J),normalSpeed=closing,energy=.5*this.mass*normalSpeed*normalSpeed*.18;
      ...
      const ray=new T.Raycaster(point.clone().addScaledVector(n,.5),n.clone().negate(),0,1.3),meshes=this.parts.filter(p=>!p.broken).map(p=>p.mesh),hit=ray.intersectObjects(meshes,false)[0];
      if(hit)this.damage(hit,Math.min(85,energy*.08),'blunt',n.clone(),impulse.clone());
      else{let nearest=null,d=Infinity;for(const p of this.parts)if(!p.broken){p.mesh.geometry.computeBoundingBox();const center=p.mesh.geometry.boundingBox.getCenter(V()).applyMatrix4(p.mesh.matrixWorld),next=center.distanceToSquared(point);if(next<d){d=next;nearest=p;}}if(nearest)this.damage({object:nearest.mesh,point},Math.min(85,energy*.08),'blunt',n.clone(),impulse.clone());}
  ...
  for(const actor of this.props.system.actors||[]){
    const bodyPoint=actor.group.getWorldPosition(V()),local=this.group.worldToLocal(bodyPoint.clone());let overlap=Math.abs(local.x)<1.07&&Math.abs(local.z)<2.35;
    ...
    if(!overlap)continue;const hip=actor.bones?.Hip?.getWorldPosition(V())||bodyPoint,push=bodyPoint.clone().sub(this.group.position).setY(0).normalize();if(push.lengthSq()<.1)push.set(1,0,0);actor.group.position.addScaledVector(push,.12);actor.knockDown?.(push,this.velocity.clone().multiplyScalar(.06));
  ...
  const extent=Math.min(50,this.world.extent||12)-1;if(Math.abs(this.group.position.x)>extent||Math.abs(this.group.position.z)>extent)this.velocity.multiplyScalar(.6);this.group.position.x=clamp(this.group.position.x,-extent,extent);this.group.position.z=clamp(this.group.position.z,-extent,extent);
 }
 tick(dt){dt=clamp(dt,0,.1);this.time+=dt;if(this.revision!==this.world.revision){this.revision=this.world.revision;this.reset();}this.group.visible=this.world.name==='Living room'&&this.world.root.visible;for(const d of this.debris)d.mesh.visible=this.group.visible;if(!this.group.visible){if(this.driving)this.exit();return;}this.updateOccupancy();this.updateGrips(dt);const input=this.input(),steps=Math.max(1,Math.ceil(this.velocity.length()*dt/.10));for(let i=0;i<steps;i++){this.step(dt/steps,input);this.group.updateMatrixWorld(true);this.collide(dt/steps);}this.syncCollider();this.syncSeat();for(const d of this.debris){d.velocity.y-=9.81*dt;const before=d.mesh.position.clone();d.mesh.position.addScaledVector(d.velocity,dt);if(this.world.projectSphere(d.mesh.position,.10)){const n=d.mesh.position.clone().sub(before).normalize(),vn=d.velocity.dot(n);if(vn<0)d.velocity.addScaledVector(n,-1.15*vn);}if(d.mesh.position.y<.12){d.mesh.position.y=.12;d.velocity.y=Math.abs(d.velocity.y)*.15;d.velocity.x*=.9;d.velocity.z*=.9;d.spin.multiplyScalar(.94);}d.mesh.rotateX(d.spin.x*dt);d.mesh.rotateZ(d.spin.z*dt);}this.mirrorClock+=dt;this.updateStatus();}
