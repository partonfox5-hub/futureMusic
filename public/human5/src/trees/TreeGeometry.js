...
export const SPECIES=Object.freeze([
 {name:'Oak',bark:0x71604a,leaf:0x577535,height:8,spread:3.1,branches:14,up:.36,deciduous:true},
 {name:'Silver birch',bark:0xb8b5a4,leaf:0x719448,height:10,spread:2.2,branches:17,up:.62,deciduous:true},
 {name:'Scots pine',bark:0x85604b,leaf:0x425c39,height:12,spread:2.8,branches:21,up:.13,deciduous:false},
 {name:'Aspen',bark:0x96977b,leaf:0x779148,height:9,spread:2.1,branches:16,up:.73,deciduous:true}
]);
...
export function makeTreeGeometry(species=0,variant=0,lod=0){
 ...
 function leaf(at,size,angle,tilt,col){
  ...
  for(const [x,y,z,u,w] of [[0,0,0,.5,0],[-.46,.43,0,0,.43],[0,.52,.10,.5,.52],[.46,.43,0,1,.43],[0,1,0,.5,1]])vertex(v(x*size,y*size,z*size).applyQuaternion(q).add(at),col,u,w,1);
 ...
 branch(v(0,-.12,0),trunkTop,.15+(species===0?.09:.02),.018,lean.clone().multiplyScalar(.7));
 const count=sp.branches;
 for(let i=0;i<count;i++){
  const t=(i+.6)/count,angle=i*2.399963+variant*.7+(rnd()-.5)*.8;
  const y=H*(species===2?.26+t*.64:.34+t*.52),a=lean.clone().multiplyScalar(y/H).add(v(0,y,0));
  const taper=species===2?1-t*.8:Math.sin(t*Math.PI*.78)*.65+.35;
  const length=sp.spread*taper*(.64+rnd()*.66),dir=v(Math.cos(angle),sp.up+(rnd()-.5)*.45,Math.sin(angle)).normalize();
  const end=a.clone().addScaledVector(dir,length),bend=v((rnd()-.5)*.30,-.14-rnd()*.24,(rnd()-.5)*.30);
  tips.push(end.toArray());
  if(lod<2)branch(a,end,(.055+(1-t)*.055)*(species===0?1.2:.75),.009,bend);
  if(lod===2){if(i%2===0)crown(end,.57+length*.22);continue;}
  const forks=lod===0?3:2;
  for(let j=0;j<forks;j++){
   const start=a.clone().lerp(end,.48+j*.17),az=angle+(j-1)*.65+(rnd()-.5)*.55;
   const twig=start.clone().add(v(Math.cos(az),species===2?.10:.48,Math.sin(az)).multiplyScalar(length*(.34+rnd()*.36)));
   if(lod===0)branch(start,twig,.018,.002,v(0,species===1?-.18:.10,0));
   tips.push(twig.toArray());
   const n=lod===0?(species===2?18:15):5;
   for(let k=0;k<n;k++){
    const leafPos=start.clone().lerp(twig,.25+rnd()*.85).add(v((rnd()-.5)*.65,(rnd()-.5)*.47,(rnd()-.5)*.65));
    const size=(species===2?.18:.25)*(lod===0?1:2.1)*(.65+rnd()*.7);
    leaf(leafPos,size,rnd()*Math.PI*2,(rnd()-.5)*2.4,green.clone().multiplyScalar(.72+rnd()*.42));
   }
  }
 }
