...
export class DogPaws {
  ...
  solve(leg,target){
    const {upper,lower,foot,length1:a,length2:b,root}=leg;
    if(root){
      ...
      if(aim.lengthSq()>.01)root.quaternion.slerp(new THREE.Quaternion().setFromUnitVectors(rest.clone().normalize(),aim),.35);
    ...
    const pole=new THREE.Vector3(0,leg.front?.25:-.15,leg.front?-1:1).transformDirection(this.model.root.matrixWorld);pole.addScaledVector(d,-pole.dot(d)).normalize();
