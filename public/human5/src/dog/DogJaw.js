...
import { clamp } from './Dog.js?v=15.1';
...
export class DogJaw {
  constructor(model){
    ...
    if(cat){
      ...
      this.mesh=model.ellipsoid('jaw',[0,.696,.422],[.026,.014,.018],'Jaw',model.coat,'coat',14,7);
      const g=this.mesh.geometry,pos=g.getAttribute('position'),delta=new Float32Array(pos.count*3);
      for(let i=0;i<pos.count;i++)delta[i*3+1]=-.002*clamp((pos.getZ(i)-.45)/.08);
      ...
      model.ellipsoid('tongue',[0,.706,.436],[.010,.004,.012],'Jaw',model.material({color:0xc27e85,roughness:.28}),'coat',10,6);
      for(const s of [-1,1])for(const z of [.418,.436]){
        model.ellipsoid('teeth',[s*.014,.708,z],[.0028,.005,.0035],'Jaw',tooth,'coat',7,4);
        model.ellipsoid('teeth',[s*.014,.714,z],[.0028,.005,.0035],'Head',tooth,'coat',7,4);
      }
    }else{
