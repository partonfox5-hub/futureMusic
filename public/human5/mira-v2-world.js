import {captureFurniture,tagMovable} from './mira-v2-furniture.js?v=12.9';
import {Destruction} from './mira-v2-destruction.js?v=11.0';
import {buildHouse} from './mira-v2-house.js?v=12.6';
import {plantTerrain,scatterTrees,tickNature,chopTree as chopNature,terrainHeight} from './mira-v2-nature.js?v=12.9';
...
export class MiraWorld {
 ...
 setScene(name){
  ...
  this.root.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();}});this.root.clear();this.obstacles=[];this.seats=[];this.pickables=[];this.movables=[];this.stairs=[];this.trees=[];this.terrain=null;this.grid=null;this.name=name;this.extent=72;this.builder?.clear();this.revision++;this.fractures.clear();
  const jungle=name==='Jungle',beach=name==='Beach';this.scene.background=new T.Color(jungle?0x637f76:beach?0xaedced:0xc2b5a3);this.scene.fog=new T.Fog(this.scene.background,56,180);
  this.terrainPad=jungle||beach?3.4:12.2;this.terrainAmp=beach?.4:jungle?1.15:1;
  plantTerrain(this,{size:156,pad:this.terrainPad,amp:this.terrainAmp,grass:jungle?0x4e6640:beach?0xd7c08c:0x5d7048,dirt:jungle?0x4a4030:beach?0xc2a56e:0x6a5a3e});
  if(!jungle&&!beach){
   buildHouse(this);
   scatterTrees(this,{count:QUEST?22:56,pad:14,extent:40,flat:this.terrainPad});
   for(const [x,z,s] of [[8.6,-6.4,.52],[-9.4,7.2,.44],[12.1,3.4,.36],[-7.6,-11.2,.58],[15.2,-8.1,.4],[28,-18,.62],[-24,22,.48],[32,14,.4],[-30,-12,.55]]){
   ...
   scatterTrees(this,{count:QUEST?28:70,pad:5.2,extent:44,palm:beach,flat:this.terrainPad,avoid:beach?(x,z)=>z<-6:null});
   ...
   if(beach){const water=this.mesh(new T.PlaneGeometry(96,36,1,1),this.mat(0x539daa,.27),0,-.04,-24);water.rotation.x=-Math.PI/2;water.castShadow=false;this.water=water;}
  ...
  for(const [x,z,w,d] of [[-72.2,0,.2,145],[72.2,0,.2,145],[0,-72.2,145,.2],[0,72.2,145,.2]])this.obstacle(x,z,w,d,0,4);
 }
