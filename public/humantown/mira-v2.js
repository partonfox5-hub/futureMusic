import {viewerPose} from './mira-xr-controls.js';
...
export function createMiraSystem({ scene, renderer, camera, xrOn, rig }) {
  ...
  function resetPhysics({release=true}={}) {
    physicsAccumulator = 0;
    for (let i = 0; i < 2; i++) { if(release)tryRelease(i); hands.prevReady[i] = false; hands.vel[i].set(0, 0, 0); }
    for (const a of actors) {if(release)a.resetPhysics();else {a.softAccumulator=0;for(const s of a.soft||[])s.vx=s.vy=s.vz=0;}}
  ...
  function tick(dt, tAbs, keys) {
    ...
    camPos.copy(viewerPose(renderer,rig,camera).position);
    pollSpawnBalls(keys);
