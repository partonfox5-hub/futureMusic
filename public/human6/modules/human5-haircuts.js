...
export function installHairCutting({mira,props,world}={}){
 ...
 function trimGuides(a,point,radius){const groom=a.h5Groom;if(!groom?.group.visible)return 0;const s=attach(a),mesh=groom.group.children.find(m=>m.name==='Guided shoulder locks');if(!mesh)return 0;let changed=0;
  ...
  if(changed){const ids=[],nodes=9;for(let l=0;l<groom.locks.length;l++)for(let j=0;j<(s.guideCuts.get(l)??nodes-1);j++)for(let k=0;k<2;k++){const a=l*nodes*3+j*3+k,b=a+3;ids.push(a,b,a+1,a+1,b,b+1);}mesh.geometry.setIndex(ids);}return changed;
 }
 ...
 function regrow(a){const s=states.get(a);if(!s)return;for(const [g,r]of s.geometries)g.setIndex(r.original);s.geometries.clear();s.guideCuts.clear();const groom=a.h5Groom,mesh=groom?.group.children.find(m=>m.name==='Guided shoulder locks');if(mesh){const ids=[];for(let l=0;l<28;l++)for(let j=0;j<8;j++)for(let k=0;k<2;k++){const a=l*27+j*3+k,b=a+3;ids.push(a,b,a+1,a+1,b,b+1);}mesh.geometry.setIndex(ids);}}
