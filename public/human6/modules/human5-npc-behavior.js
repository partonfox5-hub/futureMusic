...
import {CITY_BLOCKS,ROUTES} from './human5-worldfield.js?v=20.3.0';
import {V,rng,wrapMethod,clamp} from './human5-common.js?v=20.3.0';
...
export function installNPCBehavior({mira,props,world,wardrobe,camera,combat,quest=true}={}){
 ...
 const api={brains,spawn,attach,command,consumePoint,pointFor,stop,setGoal,visible,maxActive,NPC_ROLES,NPC_MODES,FOLLOWER_ORDERS,
  ...
  tick(dt){time+=dt;const player=camera.getWorldPosition(V());for(const [a,b]of brains){if(!mira.actors.includes(a)){props.drop(a);b.restore?.();brains.delete(a);continue;}b.attack-=dt;b.frozen=Math.max(0,b.frozen-dt);if(a.dead){props.drop(a);continue;}if(a.held||b.frozen>0||a.h5Frozen){stop(a);continue;}if(world.h6Combat?.tickBrain(b,dt))continue;b.plan-=dt;if(b.plan>0)continue;b.plan=.20;if(!b.hostile&&!b.follower&&b.role==='pedestrian'&&!a.h5Ambient&&b.mode!=='still'){continue;}if(a.group.position.distanceToSquared(player)>180*180){stop(a);continue;}
    ...
    if(world.h6Combat){if(b.target&&!world.h6Combat.permitted(a,b.target))b.target=null;}const target=b.target?.group.position.clone().add(new T.Vector3(0,1.05,0))||player,distance=a.group.position.distanceTo(target),aggressive=!world.h6Combat&&(b.target||b.hostile&&b.mode!=='passive');
