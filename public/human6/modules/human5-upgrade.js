import {installActivities} from './human5-activities.js?v=20.3.0';
import {installPortableLights} from './human5-portable-lights.js?v=20.3.0';
import {installDrawers} from './human5-drawers.js?v=20.3.0';
import * as T from 'three';
import {tagMovable,placeFurniture} from '../mira-v2-furniture.js?v=20.3.0';
import {HouseDoors} from '../mira-v2-doors.js?v=20.3.0';
import {makeSurfaceMap} from '../mira-v2-walls.js?v=20.3.0';
import {SCENES} from '../mira-v2-world.js?v=20.3.0';
import {FixtureLights} from './human5-lighting.js?v=20.3.0';
import {upgradeHostFire} from './human5-fire-host.js?v=20.3.0';
import {FurnitureMaterials} from './human5-furniture.js?v=20.3.0';
import {TextileSystem} from './human5-textiles.js?v=20.3.0';
import {Neighborhood,NEIGHBORHOOD_NAME} from './human5-neighborhood.js?v=20.3.0';
import {installHumanDynamics,DYNAMICS_CONTROLS} from './human5-dynamics.js?v=20.3.0';
import {installSkinRefinement} from './human5-skin.js?v=20.3.0';
import {applyNPCProfile,exportNPCProfile} from './human5-npc-profile.js?v=20.3.0';
import {installVehicleRefinement} from './human5-vehicles.js?v=20.3.0';
import {installTissueSurface} from './human5-tissue-surface.js?v=20.3.0';
import {installReferenceIdentity,REFERENCE_EXPRESSIONS,REFERENCE_MIRA} from './human5-identity.js?v=20.3.0';
import {V,VERSION,wrapMethod,attachedTo} from './human5-common.js?v=20.3.0';
...
export function installHuman5Upgrade({scene,renderer,camera,world,mira,props,wardrobe,water,quest=true,daylight=null,newMap=true}={}){
  ...
  restores.push(wrapMethod(mira,'remove',old=>function(a){
    if(!mira.actors.includes(a))return;
    props.drop(a);props.releaseFurniture?.(a);
    for(const [key,d]of wardrobe.drags||[])if(d.cloth?.actor===a)wardrobe.cancel(key);
    for(const c of [...wardrobe.clothes])if(c.actor===a)wardrobe.remove(c);
    for(const link of [...(props.restraints?.links||[])])if(link.a.actor===a||link.b.actor===a)props.restraints.remove(link);
    if(a.seat?.occupant===a)a.seat.occupant=null;
    a.h5Identity?.dispose({restore:false});a.h5TissueSurface?.dispose();a.h5Dynamics?.dispose();a.h5Skin?.dispose();a.realism?.dispose();
