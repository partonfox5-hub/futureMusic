...
import {V,clamp,gravityOf,wrapMethod} from './human5-common.js?v=17.2.0';
...
export class AutomaticDrive {
  ...
  step(dt,{selector='P',throttle=0,brake=0,speed=0,driving=false}={}){
    ...
    const ratio=selector==='R'?3.2:RATIOS[this.gear-1],coupled=Math.abs(speed)/.34*ratio*3.42*60/(2*Math.PI);
    ...
    const wheelTorque=engaged?(selector==='R'?-1:1)*210*curve*load*(selector==='R'?3.2:RATIOS[this.gear-1])*3.42*.88*.5*(this.shift? .18:1):0;
...
export class VehicleDamage {
  ...
  impact(car,part,point,energy,kind,impulse){
    ...
    if(part.kind==='wheel'){
      ...
      d.alignment=clamp(d.alignment+part.side*severity*(kind==='bullet'?.018:.11),-.15,.15);
      if(part.z<0)this.steering=clamp(this.steering+severity*.6,0,1);
    }else if(!/glass|light|handle|mirror/i.test(part.name)){
      if(local.z<-.65){this.engine=clamp(this.engine+severity*(Math.abs(local.x)<.65?.7:.2),0,1);this.steering=clamp(this.steering+severity*.32,0,1);}
      const wheel=car.wheels.slice().sort((a,b)=>Math.hypot(local.x-a.side*.89,local.z-a.z)-Math.hypot(local.x-b.side*.89,local.z-b.z))[0];
      if(wheel&&Math.hypot(local.x-wheel.side*.89,local.z-wheel.z)<.9){const d=this.wheel(wheel);d.suspension=clamp(d.suspension+severity*.6,0,1);d.alignment=clamp(d.alignment+wheel.side*severity*.07,-.15,.15);}
    }
  }
...
export function sampleWheel(car,w){
  const p=car.group.localToWorld(new T.Vector3(w.side*.89,.70,w.z));
...
export function vehicleSubstep(car,dt,input){
  ...
  const drive=auto.step(dt,{selector:car.gear,throttle:input.throttle,brake,speed,driving:car.driving});car.engineRPM=drive.rpm;
  const force=gravity.clone().setY(0).multiplyScalar(car.mass);let up=0,pitch=0,roll=0,yawTorque=0,contacts=0,gripSum=0;
  const wheelbase=2.63,track=1.78;
  for(const w of car.wheels){
    const wd=damage.wheel(w),{p,floor,normal}=sampleWheel(car,w),radius=w.popped?.265:.34,raw=.411-(p.y-floor-radius),compression=clamp(raw,-.12,.14),rate=clamp((compression-(w.compression??compression))/dt,-3,3);
    w.compression=compression;
    const spring=60000*(1-wd.suspension*.55)*Math.max(0,compression)+5300*(1-wd.suspension*.70)*rate;
    w.load=w.broken||raw<-.12?0:clamp(spring,0,car.mass*9.81*.80)*(w.popped?.7:1);
    const N=w.load;up+=N*normal.y;force.addScaledVector(normal,N).addScaledVector(Y,-N*normal.y);
    pitch-=w.z*N;roll+=w.side*.89*N;
    const desired=car.group.worldToLocal(p.clone().setY(floor+radius)).y;
    w.group.position.y=clamp(desired,.15,.50);if(w.broken)continue;
    ...
    const torque=w.z>0&&!w.popped?drive.wheelTorque*damage.power:0,I=1.8;
  ...
  car.mu=gripSum/4;car.h5Vehicle.contacts=contacts;
  car.heaveVelocity+=(up/car.mass+gravity.y)*dt;car.heaveVelocity*=Math.exp(-.45*dt);car.group.position.y+=car.heaveVelocity*dt;
  car.pitchRate=(car.pitchRate+pitch/2200*dt)/(1+3.5*dt);car.rollRate=(car.rollRate+roll/850*dt)/(1+4.5*dt);
  ...
  if(contacts>=3&&brake>.95&&car.velocity.length()<.10&&Math.hypot(force.x,force.z)<car.mass*g*.8){car.velocity.set(0,0,0);car.yawRate*=Math.exp(-12*dt);}
  car.yawRate=(car.yawRate+yawTorque/2400*dt)/(1+1.8*dt);car.yawRate=clamp(car.yawRate,-1.6,1.6);
  car.group.rotation.y+=car.yawRate*dt;car.group.position.addScaledVector(car.velocity,dt);car.steeringWheel.rotation.z=car.steer*5.2;
...
export function installVehicleRefinement(car,{wetness=null}={}){
  if(car.h5Vehicle)return car.h5Vehicle;const restores=[],api={auto:new AutomaticDrive(),damage:new VehicleDamage(),contacts:0,wetness};car.h5Vehicle=api;car.group.userData.h5Vehicle=car;
  const detail=detailModel(car);
  restores.push(wrapMethod(car,'reset',old=>function(){api.auto.reset();api.damage.reset();const r=old.apply(this,arguments);this.wheels.forEach(w=>w.compression=.051);return r;}));
  restores.push(wrapMethod(car,'damage',old=>function(hit,energy,kind,dir,impulse){const part=hit?.object?.userData.carPart;if(part&&!part.broken){const point=hit.point?.clone()||part.mesh.getWorldPosition(V());api.damage.impact(this,part,point,energy,kind,impulse);}return old.apply(this,arguments);}));
  ...
  restores.push(wrapMethod(car,'step',()=>function(dt,input){const n=Math.max(1,Math.min(8,Math.ceil(dt*120)));for(let i=0;i<n;i++)vehicleSubstep(this,dt/n,input);detail.tick(input);}));
