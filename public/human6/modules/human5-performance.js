...
export const QUEST_PROFILES=Object.freeze([
  ...
  {name:'sustain',foveation:1.0,interiorRange:11,pile:400,mirrorHz:12,portalHz:12,shadowHz:18}
]);
...
export class FrameBudget {
  ...
  begin(time=performance.now()){
    this.started=performance.now();this.interval=this.last?time-this.last:0;this.last=time;this.sections={};this.drawSections={};this.lastCalls=0;this.renderer.info.reset();
  ...
  mark(name){const now=performance.now();this.sections[name]=(this.sections[name]||0)+now-(this.markAt??this.started);this.markAt=now;const calls=this.renderer.info.render.calls;this.drawSections[name]=(this.drawSections[name]||0)+calls-(this.lastCalls||0);this.lastCalls=calls;}
    ...
    const row={interval:this.interval,cpu:now-this.started,gpu:null,calls:this.renderer.info.render.calls,triangles:this.renderer.info.render.triangles,sections:{...this.sections},drawSections:{...this.drawSections},label:this.label||'Free roam',tier:this.tier};this.markAt=null;
  ...
  report(){const rows=this.samples,active=rows.filter(r=>r.interval>0),budget=1000/this.targetHz,names=new Set(rows.flatMap(r=>Object.keys(r.sections)));
    return {format:'human5-performance-1',recordedAt:new Date().toISOString(),userAgent:navigator.userAgent,xr:this.renderer.xr.isPresenting,targetHz:this.targetHz,budgetMs:budget,tier:QUEST_PROFILES[this.tier].name,samples:rows.length,
      ...
      drawCalls:summary(active.map(r=>r.calls)),triangles:summary(active.map(r=>r.triangles)),sectionsMs:Object.fromEntries([...names].map(n=>[n,summary(active.map(r=>r.sections[n]||0))])),drawCallsBySection:Object.fromEntries([...names].map(n=>[n,summary(active.map(r=>r.drawSections?.[n]||0))])),resources:{...this.renderer.info.memory,programs:this.renderer.info.programs?.length},raw:rows};
  }
...
export function installPerformance({renderer,scene,camera,world,mira,props,upgrade,quest=true}={}){
  const restores=[],visibility=new Map(),actors=new Map(),shadowAuthored=new WeakMap();let time=0,lodT=0,shadowT=0,lastRevision=-1;const p=V(),furnitureP=V();
  ...
  function actorLOD(a){if(actors.has(a)||a.version!=='v2')return;let acc=((a.h5Brain?.id||actors.size)%5)*.003,sleepCheck=0,support=null;const restore=wrapMethod(a,'tick',old=>function(dt,cam,t){this.h5SimulationDue=true;
...
const far=quest&&this.group.position.distanceTo(cam)>9&&!this.grabs.size&&!this.socialPair&&!this.seat&&!this.dead&&this.balance.state==='standing';
      if(far){acc+=dt;const hz=this.group.position.distanceToSquared(cam)>45*45?10:this.group.position.distanceToSquared(cam)>22*22?20:30;if(acc<1/hz){this.h5SimulationDue=false;return;}dt=Math.min(.10,acc);acc=0;}else acc=0;return old.call(this,dt,cam,t);});actors.set(a,restore);
  }
  const vehicleShadows=quest?installVehicleShadows({scene,props}):null;const api={budget,batches,vehicleShadows,
    beforeFrame(dt){time+=dt;lodT-=dt;shadowT+=dt;camera.getWorldPosition(p);world.h5Viewer=p;world.h5QuestBudget=quest;for(const [a,restore]of actors)if(!mira.actors.includes(a)){restore();actors.delete(a);}for(const [g]of visibility)if(!g.parent)visibility.delete(g);for(const a of mira.actors)actorLOD(a);
      ...
      if(lodT<=0||world.revision!==lastRevision){lodT=.15;if(world.revision!==lastRevision)batches.clear();lastRevision=world.revision;
        for(const g of world.movables||[])batches.add(g);if(props.wardrobe?.rack)batches.add(props.wardrobe.rack);
        for(const f of world.h5Birds?.feeders.values()||[])batches.add(f.root);
        ...
        for(const car of props.cars()){
          ...
          car.group.userData.h6BatchGlass=true;batches.add(car.group);for(const h of car.hinges){h.root.userData.h6BatchGlass=true;batches.add(h.root);}for(const w of car.wheels)batches.add(w.spin||w.group);
          if(car.steeringWheel)batches.add(car.steeringWheel);if(car.shifterLever)car.shifterLever.userData.h5DynamicPart=true;if(car.shifterBase)batches.add(car.shifterBase);
        }
for(const d of world.doors?.list||[])batches.add(d.root);for(const h of world.neighborhood?.houses||[])if(h.staticRoot){h.staticRoot.userData.h6BatchGlass=true;batches.add(h.staticRoot);}batches.tick();
        for(const h of world.neighborhood?.houses||[]){const inside=h.bounds.containsPoint(p),range=quest?Math.min(8,profile.interiorRange):35;
          for(const g of h.contents){if(!g.parent)continue;const f=g.userData.furniture,near=inside||g.getWorldPosition(furnitureP).distanceToSquared(p)<range*range||f?.held!=null||f?.holds?.size||(f?.velocity?.lengthSq()||0)>.01;
            if(!visibility.has(g))visibility.set(g,g.visible);g.visible=visibility.get(g)&&near;
          }
          const shadowNear=h.bounds.distanceToPoint(p)<16;for(const root of h.meshes){if(root===h.ceiling)continue;root.traverse(m=>{if(!m.isMesh)return;if(!shadowAuthored.has(m))shadowAuthored.set(m,m.castShadow);m.castShadow=shadowAuthored.get(m)&&shadowNear;});}
        }
