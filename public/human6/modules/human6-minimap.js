...
import {ROUTES} from './human5-worldfield.js?v=20.2.0';
...
export function installMinimap({world,mira,camera,scene,props,renderer,factions}){
  ...
  const panel=new T.Group();panel.name='Human6 folding field map';scene.add(panel);
  ...
  function build(){const field=world.h5OpenWorld?.field;if(!field)return;const data=bc.createImageData(128,128),half=field.half;for(let z=0;z<128;z++)for(let x=0;x<128;x++){const wx=(x/127*2-1)*half,wz=(z/127*2-1)*half,h=field.heightAt(wx,wz)||0,i=(z*128+x)*4;const wet=field.waterAt(wx,wz);data.data[i]=wet?33:Math.min(140,52+h*.5);data.data[i+1]=wet?84:Math.min(151,78+h*.3);data.data[i+2]=wet?114:Math.min(140,58+h*.4);data.data[i+3]=255;}bc.putImageData(data,0,0);built=true;}
  ...
  function draw(){camera.getWorldPosition(eye);c.fillStyle='#1a272a';c.fillRect(0,0,384,384);if(!built)build();if(built){const half=world.h5OpenWorld.field.half,scale=356/(radius*2);c.drawImage(base,px(-half),pz(-half),half*2*scale,half*2*scale);}
  ...
  const api={canvas,panel,texture:tex,open,get deployed(){return deployed;},get unfold(){return unfold;},gripsHand(i){return deployed&&panel.visible&&(i===leftIndex()||pulling&&mira.hands.handedness[i]==='right');},get enabled(){return enabled;},set enabled(v){enabled=!!v;},get radius(){return radius;},set radius(v){radius=T.MathUtils.clamp(Number(v)||180,60,2400);},waypoint,
    setWaypoint(t){waypoint.set(t.x,t.y||0,t.z);hasWaypoint=true;draw();},tick(dt){canvas.style.display=enabled&&!renderer.xr.isPresenting?'block':'none';handPose(dt);accumulated+=dt;if(enabled&&accumulated>=.2){accumulated=0;draw();}},dispose(){canvas.remove();panel.removeFromParent();undoGrip();undoRelease();removeEventListener('keydown',key);renderer.xr.removeEventListener('sessionend',end);for(const h of leaves)h.children[0].geometry.dispose();paper.dispose();tex.dispose();delete world.h6Minimap;}};world.h6Minimap=api;return api;
}
