...
import {V,clamp,smooth,gravityOf,wrapMethod,rng} from './human5-common.js?v=20.2.0';
...
export function installReferenceGroom(actor){
  ...
  const material=new T.MeshStandardMaterial({color:0x4a352a,map:texture,roughness:.56,metalness:0,side:T.DoubleSide,alphaTest:.36,alphaToCoverage:true,envMapIntensity:.8});
  ...
  // The rear cranium is wider than the old collision proxy. The same envelope
  // protects guide centers, card edges AND the continuous crown after hand pushes.
  const scalpCenter=V().set(0,1.525,-.004),scalpRadii=V().set(.099,.119,.118);
  function outsideScalp(p){
    if(p.y<1.438)return p;
    const x=(p.x-scalpCenter.x)/scalpRadii.x,y=(p.y-scalpCenter.y)/scalpRadii.y,z=(p.z-scalpCenter.z)/scalpRadii.z;
    const length=Math.hypot(x,y,z);if(length<1&&length>1e-8)p.set(scalpCenter.x+x/length*scalpRadii.x,scalpCenter.y+y/length*scalpRadii.y,scalpCenter.z+z/length*scalpRadii.z);
    return p;
  }
  function crown(theta,phi,out=V()){
    const part=.006*(1-smooth(theta/.7));
    return out.set(part+.102*Math.sin(theta)*Math.sin(phi),1.525+.122*Math.cos(theta),-.004+.121*Math.sin(theta)*Math.cos(phi));
  }
  for(let j=0;j<=thetaN;j++)for(let i=0;i<=phiN;i++){
    ...
    if(front>.25&&drop>.01){const blend=smooth(drop/.45)*smooth((front-.25)/.15);p.x=T.MathUtils.lerp(p.x,Math.sign(Math.sin(phi))*Math.max(Math.abs(p.x),.079+.020*drop),blend);p.z=T.MathUtils.lerp(p.z,Math.min(p.z,.033-.014*drop),blend);}
    // Continuous underlayer connects the crown to the hanging sheets.
    p.x-=Math.sin(phi)*.002;p.z-=Math.cos(phi)*.002;outsideScalp(p).applyMatrix4(bind);cp.push(p.x,p.y,p.z);cu.push(i/phiN*6,v*.88);
    const guide=clamp(Math.round((phi-.48)/(Math.PI*2-.96)*43),0,43),t=v<.55?clamp(((theta-.34)/1.19)*.46,0,.46):.46+(v-.55)/.45*.54;
    capLinks.push({guide,node:clamp(Math.round(t*8),0,8),t,weight:skirt*smooth((v-.58)/.42)*.85});
  ...
  const cap=new T.Mesh(cg,crownMat);cap.name='Fitted softly parted crown';cap.frustumCulled=false;group.add(cap);
  ...
  for(let l=0;l<lockCount;l++){
    ...
    for(let j=0;j<nodes;j++){
      ...
      if(Math.cos(phi)>.25&&t>.23){const blend=smooth((t-.23)/.28)*smooth((Math.cos(phi)-.25)/.15);p.x=T.MathUtils.lerp(p.x,Math.sign(Math.sin(phi))*Math.max(Math.abs(p.x),.078+.023*smooth((t-.23)/.64)),blend);p.z=T.MathUtils.lerp(p.z,Math.min(p.z,.036-.015*smooth(t)),blend);}
      const layer=-.0025+.0055*smooth((t-.35)/.25);p.x+=Math.sin(phi)*layer+Math.sin(t*4+phase)*.0012*smooth(t);p.z+=Math.cos(phi)*layer;
      rest.push(outsideScalp(p).applyMatrix4(bind));points.push(V());previous.push(V());targets.push(V());
  ...
  const tick=dt=>{
    ...
    const headPosition=head.getWorldPosition(p),hands=(actor.externalHands||[]).filter(c=>c.a&&c.b&&c.a.distanceToSquared(headPosition)<.36);
    let contacts=0;
    ...
    while(accumulator+1e-9>=h){
      const drag=Math.exp(-(4.0+wet*7)*h),shapeRate=1-Math.exp(-(8-motion*6.2)*h);
      for(const lock of locks){const {points,previous,targets}=lock;
        ...
        for(let it=0;it<4;it++)for(let j=2;j<nodes;j++){
          ...
          d.subVectors(points[j],targets[j]).clampLength(0,.003+motion*.100*smooth((j/(nodes-1)-.36)/.64));points[j].copy(targets[j]).add(d);
        }
        for(let j=2;j<nodes;j++){
          local.copy(points[j]).applyMatrix4(worldToModel);
          outsideScalp(local);points[j].copy(local).applyMatrix4(modelToWorld);
          for(const c of hands){
            ab.subVectors(c.b,c.a);d.subVectors(points[j],c.a);const t=clamp(d.dot(ab)/Math.max(ab.lengthSq(),1e-8),0,1);nearest.copy(c.a).addScaledVector(ab,t);d.subVectors(points[j],nearest);
            const distance=d.length(),radius=c.r+.011,penetration=radius-distance;
            if(penetration>0){
              if(distance<1e-6)d.subVectors(points[j],headPosition).normalize();else d.divideScalar(distance);
              points[j].copy(nearest).addScaledVector(d,radius);
              // Friction carries a sheet along an open palm. Bound impulse to
              // avoid explosive velocity when tracking resumes or passes the head.
              ab.copy(c.velocity||V()).clampLength(0,2.5).multiplyScalar(h*.45*motion);
              previous[j].copy(points[j]).sub(ab);contacts++;
              if(j===5)c.onContact?.('hair',c.velocity?.length()||0,Math.min(.025,penetration));
            }
          }
          local.copy(points[j]).applyMatrix4(worldToModel);outsideScalp(local);points[j].copy(local).applyMatrix4(modelToWorld);
    ...
    for(let l=0;l<locks.length;l++){const lock=locks[l];for(let j=0;j<nodes;j++){
      ...
      for(let k=0;k<3;k++){p.copy(local).addScaledVector(lock.tangent,(k-1)*half);p.applyMatrix4(headBind);outsideScalp(p).applyMatrix4(bind);p.toArray(attr.array,(l*nodes*3+j*3+k)*3);
    ...
    for(let i=0;i<capLinks.length;i++){const link=capLinks[i];p.fromArray(capRest,i*3);if(link.weight){const lock=locks[link.guide],f=link.t*8,j=Math.min(7,Math.floor(f)),u=f-j;d.copy(lock.points[j]).lerp(lock.points[j+1],u).applyMatrix4(invWorld);before.copy(lock.rest[j]).lerp(lock.rest[j+1],u);d.sub(before);p.addScaledVector(d,link.weight);}p.applyMatrix4(headBind);outsideScalp(p).applyMatrix4(bind);p.toArray(capPosition.array,i*3);}
