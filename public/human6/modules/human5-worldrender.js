import {seasons,crownGeometry,branchedTrunkGeometry,canopyMaterial,treeTint} from './human5-seasons.js?v=19.3.0';
import * as T from 'three';
import {WorldField,WORLD_SIZE,CELL_SIZE,hash2,LAKE,RIVER,OUTFLOW,cellKey} from './human5-worldfield.js?v=19.3.0';
...
export function createForestCell(field,cx,cz,state,{quest=true}={}){
 ...
 for(let i=0;i<count*3&&n<count;i++){const x=(cx+hash2(i,cx,cz*373+79))*CELL_SIZE,z=(cz+hash2(i,cz,cx*353+191))*CELL_SIZE,y=field.heightAt(x,z),route=field.routeAt(x,z);if(y===null||field.protected(x,z)||(x>52&&x<371&&z>-175&&z<145)||field.waterAt(x,z)||y>86||Math.hypot(x-665,z+565)<242||(route&&route.distance<route.route.width/2+3)||Math.max(Math.abs(x)/38,Math.abs(z)/44)<1.1)continue;
  ...
  const id=cx+'/'+cz+'/'+i,kind=hash2(i,cz,99)>.55?'pine':'broad',height=6+hash2(i,cz,19)*7,tree={id,index:n,kind,x,y,z,height,health:state.damage.get(id)??110,dead:state.felled.has(id),fuel:null};trees.push(tree);const canopyTrees=kind==='pine'?pineTrees:broadTrees;tree.canopyIndex=canopyTrees.length;canopyTrees.push(tree);
  ...
  for(let j=0;j<2;j++){dummy.position.set(x+(kind==='broad'?(j-.5)*1.8:0),y+height*(.70+j*.16),z);dummy.scale.set(tree.dead?0:kind==='pine'?2.7-j*.6:2.4,height*(kind==='pine'?.48:.32),tree.dead?0:kind==='pine'?2.7-j*.6:2.4);dummy.updateMatrix();(kind==='pine'?pine:broad).setMatrixAt(tree.canopyIndex*2+j,dummy.matrix);}
  n++;
 }
 trunk.count=n;broad.count=broadTrees.length*2;pine.count=pineTrees.length*2;for(const m of [trunk,broad,pine]){m.instanceMatrix.needsUpdate=true;m.computeBoundingBox();m.computeBoundingSphere();}broad.userData.h5Leaf=true;pine.userData.h5Leaf=true;
 const nearGeometry=[trunk.geometry,broad.geometry,pine.geometry],farGeometry=[branchedTrunkGeometry(true),crownGeometry(false,true),crownGeometry(true,true)],nearMaterial=[trunkMat,leafMat,pineMat],farMaterial=[trunkMat,canopyMaterial(false,true),canopyMaterial(true,true)];let detailed=true;
 const offSeason=seasons.subscribe(mode=>{broad.visible=mode!=='winter';for(const tree of trees){const seed=hash2(tree.index,cx,cz+378),color=treeTint(tree.kind==='pine',seed,hash2(Math.floor(tree.x/180),Math.floor(tree.z/180),443));for(let j=0;j<2;j++)(tree.kind==='pine'?pine:broad).setColorAt(tree.canopyIndex*2+j,color);}for(const m of [broad,pine])if(m.instanceColor)m.instanceColor.needsUpdate=true;});
 const api={root,trees,trunk,broad,pine,get detailed(){return detailed;},setDetail(value){value=!!value;if(value===detailed)return;detailed=value;[trunk,broad,pine].forEach((m,i)=>{m.geometry=(value?nearGeometry:farGeometry)[i];m.material=(value?nearMaterial:farMaterial)[i];m.computeBoundingSphere();});},remove(tree){tree.dead=true;state.felled.add(tree.id);dummy.scale.setScalar(0);dummy.position.set(0,0,0);dummy.updateMatrix();trunk.setMatrixAt(tree.index,dummy.matrix);for(let j=0;j<2;j++)(tree.kind==='pine'?pine:broad).setMatrixAt(tree.canopyIndex*2+j,dummy.matrix);for(const m of [trunk,broad,pine])m.instanceMatrix.needsUpdate=true;},dispose(){offSeason();root.removeFromParent();for(const g of [...nearGeometry,...farGeometry])g.dispose();for(const m of new Set([...nearMaterial,...farMaterial]))m.dispose();}};for(const t of trees)t.forest=api;return api;
}
...
export function createFarForest(field){const trees=[];for(let iz=0;iz<88;iz++)for(let ix=0;ix<88;ix++){
 ...
 const mask=keys=>{resident=keys;let b=0,p=0,n=0;for(const t of trees){if(keys.has(t.key))continue;const evergreen=t.color>.55,m=evergreen?pine:broad,index=evergreen?p++:b++;dummy.position.set(t.x,t.y+t.height*.71,t.z);dummy.rotation.set(0,t.color*6.28,0);dummy.scale.set(evergreen?3.1:3.6,t.height*.59,evergreen?3.1:3.2);dummy.updateMatrix();m.setMatrixAt(index,dummy.matrix);m.setColorAt(index,treeTint(evergreen,t.color,hash2(Math.floor(t.x/180),Math.floor(t.z/180),443)));dummy.position.y=t.y+t.height*.39;dummy.scale.set(1,t.height*.78,1);dummy.updateMatrix();trunks.setMatrixAt(n++,dummy.matrix);}broad.count=b;pine.count=p;trunks.count=n;broad.visible=seasons.mode!=='winter';for(const m of [broad,pine,trunks]){m.instanceMatrix.needsUpdate=true;if(m.instanceColor)m.instanceColor.needsUpdate=true;m.computeBoundingSphere();}};
 const off=seasons.subscribe(()=>mask(resident));
 return {mesh,mask,dispose(){off();mesh.removeFromParent();for(const m of [broad,pine,trunks]){m.geometry.dispose();m.material.dispose();}}};
}
