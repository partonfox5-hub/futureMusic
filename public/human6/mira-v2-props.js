...
import {rayMayHitActor} from './modules/human5-ray-budget.js?v=20.3.0';
import {EXTRA_WEAPONS,buildSpecialEquipment,refineEquipmentModel} from './modules/human5-equipment-models.js?v=20.3.0';
import {RoundedBoxGeometry} from 'three/addons/geometries/RoundedBoxGeometry.js';
import {playSfx,sfxForHit,unlockSfx} from './mira-v2-sfx.js?v=20.3.0';
import {furnitureRoot,syncFurniture} from './mira-v2-furniture.js?v=20.3.0';
import {ensureGrabbableWood} from './mira-v2-nature.js?v=20.3.0';
import {GUNS,MELEE,buildMarker,buildPortalGun} from './mira-v2-gadgets.js?v=20.3.0';
import {FIREARMS,buildFirearm,firearmSpread} from './mira-v2-firearms.js?v=20.3.0';
import {buildTorch,buildExtinguisher} from './mira-v2-fire.js?v=20.3.0';
import {installWeightPhysics,throwSpeed} from './mira-v2-weights.js?v=20.3.0';
...
export class Props {
 ...
 if(buildSpecialEquipment(id,add)){}
 else if(id==='marker'||id==='marker2')buildMarker(id,add);
 ...
 const item={id,data,group,holder:null,lastTip:null,lastPoint:null,handle:new T.Vector3(0,gunGrip?-.057:-.025,gunGrip?.016:0),velocity:V(),lastHit:new Map(),kick:0,swing:0,portalNext:0,mag:data.magSize||0,support:null};this.scene.add(group);this.gadgets?.decorate?.(item);refineEquipmentModel(item);return item;}
 ...
 applyHoldPose(item){
  ...
  const tmp=this.h6PoseScratch||(this.h6PoseScratch={e:new T.Euler(),p:V(),v:V(),dir:V(),q:Q(),parentQ:Q(),forward:new T.Vector3(0,0,-1)});
  ...
  if(npc){
   ...
   const k=item.kick||0;
   item.group.quaternion.setFromEuler(tmp.e.set((gun?-.18:0)-k*1.25,item.kickYaw||0,gun?0:Math.PI/2));
   item.group.position.set(0,-.012+k*.03,-.05+k*.08).sub(tmp.v.copy(item.handle).applyQuaternion(item.group.quaternion));
  }else{
  ...
  if(!npc&&item.support!=null&&typeof item.support==='number'){
   const palm=this.system.hands?.palmPos?.(item.support);
   if(palm){
    item.group.localToWorld(tmp.p.copy(item.handle));tmp.dir.copy(palm).sub(tmp.p);
    if(tmp.dir.lengthSq()>.025){
     tmp.q.setFromUnitVectors(tmp.forward,tmp.dir.normalize());parent.getWorldQuaternion(tmp.parentQ).invert();tmp.q.premultiply(tmp.parentQ);item.group.quaternion.slerp(tmp.q,.35);
     // Re-apply the grip offset after aiming so the primary palm stays attached.
     parent.worldToLocal(tmp.p);item.group.position.copy(tmp.p).sub(tmp.v.copy(item.handle).applyQuaternion(item.group.quaternion));item.group.updateWorldMatrix(true,true);
    }
   }
  }
 ...
 resolveHeldWall(item){
  ...
  const dir=span.clone().normalize(),ray=new T.Ray(handle,dir),hit=this.hit(ray,len+.02,false,{world:true,actors:false,ignoreObject:item.group});
  if(!hit)return;
  ...
  item.group.updateMatrixWorld(true);
  // Weapon contact may constrain the prop, never translate the player rig.

 }
 ignoresPlayerObstacle(o){
  const object=o.object||o.mesh||o.root;if(!object)return false;
  for(const [key,item]of this.held){if(key!=='desktop'&&typeof key!=='number')continue;for(let n=object;n;n=n.parent)if(n===item.group)return true;}return false;
 }
 ...
 applyFurnitureGravity(group,furn,dt,holds){
  ...
  if(contacting.length&&grav>0.5){
   ...
   const tangent=Math.hypot(furn.velocity.x,furn.velocity.z),mu=/Mattress|Couch|Ottoman/.test(furn.id)?.75:.55,friction=Math.max(0,1-mu*grav*dt/Math.max(tangent,1e-9));furn.velocity.x*=friction;furn.velocity.z*=friction;
  ...
  for(const group of this.world.movables||[]){
   ...
   if(furn.id==='Mattress'){
    ...
    if(frame?.parent&&!ff?.broken){
     if(frame.userData.h5MattressSupport){group.position.copy(frame.localToWorld(frame.userData.h5MattressSupport.clone()));group.quaternion.copy(frame.quaternion);furn.velocity.set(0,0,0);furn.omega.set(0,0,0);syncFurniture(this.world,group);continue;}
    ...
    if(!Number.isFinite(group.position.lengthSq())||group.position.y<-60){group.position.set(0,.5,2);furn.velocity.set(0,0,0);furn.omega.set(0,0,0);}
   }
 ...
 grip(i){if(this.furnHolds.has(i))return true;if(this.water?.grip(i))return true;if(this.world.doors?.grip?.(i,this))return true;
  ...
  if(this.held.has(i))return true;
  const palm=palm0||this.system.hands.palmPos(i);
  const other=this.held.get(i===0?1:0);
  if(palm&&other&&isGun(other.data.kind)&&['sniper','rifle','shotgun'].includes(other.id)&&this.alongGun(other,palm)<.13){other.support=i;this.status=other.data.name+' · supported';return true;}
 ...
 hit(ray,max=50,ropes=true,opts={}){
  ...
  const hit=hits.find(h=>{
   ...
   if(opts.ignoreObject){for(let o=h.object;o;o=o.parent)if(o===opts.ignoreObject)return false;}
 ...
 alongGun(item,palm){
  item.group.updateMatrixWorld(true);
  const a=item.group.localToWorld(new T.Vector3(0,0,.06)),b=item.group.localToWorld(new T.Vector3(0,0,-item.data.reach));
  const ab=b.clone().sub(a),t=T.MathUtils.clamp(palm.clone().sub(a).dot(ab)/Math.max(1e-8,ab.lengthSq()),0,1);
  return a.addScaledVector(ab,t).distanceTo(palm);
 }
 ensureReloadBar(item){
  if(item.reloadBar)return item.reloadBar;
  const g=new T.Group();g.position.set(0,.09,-.02);item.group.add(g);
  const bg=new T.Mesh(new T.PlaneGeometry(.14,.012),new T.MeshBasicMaterial({color:0x141414,depthTest:false}));
  const fg=new T.Mesh(new T.PlaneGeometry(.14,.012),new T.MeshBasicMaterial({color:0xb7e07a,depthTest:false}));
  fg.position.z=.001;g.add(bg,fg);g.visible=false;item.reloadBar=g;item.reloadFill=fg;return g;
 }
 startReload(item){
  if(!item.data.magSize||item.reloading)return;
  item.reloading=true;item.reloadT=0;item.reloadDur=item.data.reload||1.8;
  this.ensureReloadBar(item);item.reloadBar.visible=true;item.reloadFill.scale.set(0.02,1,1);
  unlockSfx();playSfx('reload',.9);this.status='Reloading '+item.data.name;
 }
 fire(item,aim){
  ...
  unlockSfx();if(this.h6Audio&&item.id!=='laser')this.h6Audio.play(item.id,item.group.getWorldPosition(V()));else playSfx(item.id==='laser'?'laser':'gun');
  ...
  const scope=item.id==='sniper'&&this.h5Equipment?.scope.item===item&&this.h5Equipment.scope.camera;const aimDir=scope?scope.getWorldPosition(V()).addScaledVector(scope.getWorldDirection(V()),100).sub(muzzle).normalize():aim?(this.hit(aim,160)?.point||aim.at(80,V())).clone().sub(muzzle).normalize():new T.Vector3(0,0,-1).applyQuaternion(item.group.getWorldQuaternion(Q()));
  const pellets=Math.max(1,item.data.pellets||1),spread0=item.data.spread||0,spread=item.support!=null?spread0*.22:spread0,energy=item.data.energy||32,speed=item.id==='sniper'?720:item.id==='rifle'?670:item.id==='musket'?330:item.id==='shotgun'?360:310;
 ...
 impact(hit,energy,dir,kind,sharpness=0){unlockSfx();if(!this.h6Audio)playSfx(sfxForHit(kind));if(hit.object.userData.restraint){if(kind==='cut'||kind==='laser')this.restraints.cut(hit.object.userData.restraint);return;}
...
void main(){
 ...
 tickHeldGuns(dt){
  ...
  for(const item of this.items){
   ...
   if(item.reloading){
    this.ensureReloadBar(item);item.reloadT=(item.reloadT||0)+dt;
    const k=T.MathUtils.clamp(item.reloadT/Math.max(.2,item.reloadDur||1.8),0,1);
    item.reloadBar.visible=true;item.reloadFill.scale.set(Math.max(.02,k),1,1);item.reloadFill.position.x=(k-1)*.07;
    if(k>=1){item.reloading=false;item.mag=item.data.magSize;item.reloadBar.visible=false;this.status=item.data.name+' loaded';}
   }else if(item.reloadBar)item.reloadBar.visible=false;
  }
  this.tickSniperScope(dt);
 }
 tickSniperScope(dt){this.h5Equipment?.scope.tick(dt||1/72);}
 ...
 if(item.lastSamples&&item.holder!=='desktop'&&!item.holder?.h5Mouth&&!item.holder?.h5Brain&&['cut','blunt'].includes(item.data.kind)&&this.time>=(item.armedAt||0))for(let j=0;j<samples.length;j++){
  const point=samples[j],old=item.lastSamples[j],dist=point.distanceTo(old),speed=Math.min(12,dist/Math.max(.001,dt));
  if(dist>.004&&dist<.75){const ray=new T.Ray(old.clone(),point.clone().sub(old).normalize()),hit=this.hit(ray,dist+.03,false,{world:true,kind:item.data.kind});if(!hit)continue;
