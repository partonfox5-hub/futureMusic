...
import {ProceduralTrees} from './src/trees/RealisticTrees.js?v=17.2.0';
import {inCastleClearing} from './mira-v2-castle.js?v=17.2.0';
...
export function installTerrainFeatures({world,props,WEAPONS,GUNS,weaponCatalogs=[],map:mapOverrides={},autoTick=true}={}){
  ...
  function exclude(x,z,y){
    ...
    if(world.name==='Cul-de-sac'&&Math.abs(x)<30&&z>-36&&z<40)return true;
  ...
  function populate(){
    ...
    if(world.h5OpenWorld?.enabled&&world.name==='Cul-de-sac')return;
    ...
    if(world.name==='Cul-de-sac')Object.assign(options,{size:256,segments:128,pad:44,padBlend:12,mountainHeight:8,minHeight:-4,maxHeight:14,thermalPasses:1});
    ...
    world.extent=world.name==='Cul-de-sac'?90:config.playableExtent;
    ...
    for(const child of world.name==='Cul-de-sac'?[]:world.root.children){
    ...
    const trees=new ProceduralTrees(field,{root:mesh.root,world,count:world.name==='Cul-de-sac'?24:world.name==='Beach'?Math.round(config.treeCount*.35):config.treeCount,clearance:world.name==='Living room'?config.treeClearance:8,exclude,preset:/Quest|OculusBrowser/i.test(globalThis.navigator?.userAgent||'')?'quest':'quality'});
  ...
  const gun=new TerrainGun({props,getTerrain:()=>world.h5OpenWorld?.active?world.h5OpenWorld.terrainAdapter:current,...config.brush});
  ...
  const api={
    suspend(){const old=detach();old?.removeFromParent();lastRevision=world.revision;world.terrain=null;},
    ...
    tick(dt){
      if(disposed)return;if(world.h5OpenWorld?.active){gun.tick(dt);return;}if(world.revision!==lastRevision){const legacy=detach();legacy?.geometry?.dispose();legacy?.material?.dispose();populate();}
