import {installHuman6Expansion} from './modules/human6-expansion.js?v=20.3.0';
import {installPhotoNPCImport} from './modules/human5-photo-import.js?v=20.3.0';
import {installCash} from './modules/human5-cash.js?v=20.3.0';
import {Builder,FURNITURE,SURFACES} from './mira-v2-builder.js?v=20.3.0';
import {SprintBurst,PLAYER_SPEED_GAIN} from './modules/human5-sprint.js?v=20.3.0';
import {SmoothLocomotion} from './mira-v2-locomotion.js?v=20.3.0';
import {Car} from './mira-v2-car.js?v=20.3.0';
import {Restraints} from './mira-v2-restraints.js?v=20.3.0';
import { createDogSystem } from './mira-v2-dog.js?v=20.3.0';
import { installGadgets } from './mira-v2-gadgets.js?v=20.3.0';
import {Injuries} from './mira-v2-injuries.js?v=20.3.0';
import {Props,WEAPONS,GUNS} from './mira-v2-props.js?v=20.3.0';
import {installFire} from './mira-v2-fire.js?v=20.3.0';
import {installWeather} from './mira-v2-weather.js?v=20.3.0';
import {installTerrainFeatures} from './mira-v2-terrain-features.js?v=20.3.0';
import {syncFurniture} from './mira-v2-furniture.js?v=20.3.0';
import {installWater} from './mira-v2-water.js?v=20.3.0';
import {createFloraSystem} from './mira-v2-flora.js?v=20.3.0';
import {EstimatedRoomLight as RoomLight,createQuestLighting} from './modules/human5-lighting.js?v=20.3.0';
import {installHuman5Upgrade} from './modules/human5-upgrade.js?v=20.3.0';
import {installPerformance} from './modules/human5-performance.js?v=20.3.0';
import {installOpenWorld} from './modules/human5-openworld.js?v=20.3.0';
import {installGameplay} from './modules/human5-gameplay.js?v=20.3.0';
import {installVehicleSpawns} from './modules/human5-vehicle-spawns.js?v=20.3.0';
import {createStartupWarmup} from './modules/human5-startup-warmup.js?v=20.3.0';
import {installHome} from './modules/human5-home.js?v=20.3.0';
import {installEquipment} from './modules/human5-equipment.js?v=20.3.0';
import {installEcology} from './modules/human5-ecology.js?v=20.3.0';
import {createVoiceBridge} from './modules/human5-voice.js?v=20.3.0';
import {localTranscribe,warmLocalSpeech,stopLocalSpeech} from './mira-local-speech.js?v=20.3.0';
...
import {draft,saveDraft,spawnOptions,OUTFITS,clothingItems} from './mira-v2-catalog.js?v=20.3.0';
import {MiraWorld,SCENES} from './mira-v2-world.js?v=20.3.0';
import {Wardrobe,GARMENTS} from './mira-v2-wardrobe.js?v=20.3.0';
...
function banner(msg) {
  if (!boot && loadEl) loadEl.textContent = msg;
  if (hintEl) hintEl.textContent = msg;
}

await boot?.step('Starting graphics');

// Quest: skip MSAA (tiled Adreno pays a full extra render pass). Start at scale 1.0; profile draw calls before increasing resolution.
...
const xrEye=new THREE.Vector3(),xrBefore=new THREE.Vector3();
...
if (QUEST) renderer.xr.setFramebufferScaleFactor(1.0);
...
addEventListener('pointerdown',e=>{if(e.button===2)keys.MouseRight=true;});
addEventListener('pointerup',e=>{if(e.button===2)keys.MouseRight=false;});
addEventListener('pointercancel',()=>{keys.MouseRight=false;});
addEventListener('contextmenu',e=>{if(props?.held?.get?.('desktop')?.id==='sniper')e.preventDefault();});
...
orbit.update();
document.getElementById("desk").onclick = () => { unlockSfx(); if(ui)ui.style.display="none"; const forward=camera.getWorldDirection(new THREE.Vector3());orbit.target.copy(camera.position).addScaledVector(forward,2);orbit.enabled=true;orbit.update(); };
document.getElementById("firstPerson").onclick=()=>{unlockSfx();orbit.enabled=false;if(ui)ui.style.display="none";controls?.lock();};
// Leaving first person keeps the current view; an old orbit target can pull the camera across the map.
controls?.addEventListener('unlock',()=>{orbit.enabled=false;});
...
await boot?.step('Building the home');
const mira = createMiraSystem({ scene, renderer, camera, xrOn: XR_ON, rig });
const world=new MiraWorld(scene,mira,{deferScene:true});mira.setEnvironment(world);floor.visible=false;const wardrobe=new Wardrobe(scene,mira,mira.contacts,world);mira.setWardrobe(wardrobe);
...
await boot?.step('Preparing vehicles');
...
await boot?.step('Preparing interactions');
...
props.flora=flora;
const terrainFeatures=installTerrainFeatures({world,props,WEAPONS,GUNS,deferInitial:true});
...
const vehicleSpawns=installVehicleSpawns({world,props,camera,renderer,quest:QUEST});
const cash=installCash({world,props,camera,renderer,scene,water,quest:QUEST});
const equipment=installEquipment({world,mira,props,camera,rig,renderer,scene,quest:QUEST});
await boot?.step('Preparing home interactions');
const home=installHome({world,scene,camera,renderer,rig,props,mira,wardrobe,weather,upgrade,keys,quest:QUEST});
const performanceController=installPerformance({renderer,scene,camera,world,mira,props,upgrade,quest:QUEST});
await boot?.step('Preparing groups and the map');
const human6=installHuman6Expansion({world,mira,props,camera,rig,renderer,scene,gameplay,keys,quest:QUEST,weather});
const startup=createStartupWarmup({renderer,scene,camera,props,prepare(){props.syncWorld();upgrade.beforeFrame(.2);for(const c of props.cars())if(c.revision!==world.revision){c.revision=world.revision;c.reset();}home.wetness.tick(0,home.showers);performanceController.beforeFrame(.2);},status(text){boot?.stage(text);}});
let voiceSessionStart=()=>{},voiceSessionEnd=()=>{};
renderer.domElement.addEventListener('webglcontextlost',()=>{boot?.fail(new Error('The graphics context was lost. Reload to restore it.'));});
mira.load(
  (p) => { boot?.stage('Downloading Mira'); },
  async () => {
    await boot?.step('Preparing pets');
    dogs.spawnDefault();await boot?.step('Preparing the cat');dogs.spawnDefaultCat?.();stockPantry();
...
document.addEventListener('keydown',e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName))return;if(e.code==='KeyX'&&!e.repeat&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&!e.target.isContentEditable){deleteSelectedNPC();return;}if(e.code==='KeyQ'){if(props.furnHolds.has('desktop')&&props.weights?.throwHeld('desktop'))return;props.drop('desktop');}if(e.code==='Escape')builder.stop();if(e.code==='KeyE'&&!e.repeat){if(world.h6Fauna?.mounted){world.h6Fauna.dismount();return;}if(world.h6Fauna?.toggleMount())return;const c=activeCar();c.driving?c.exit():nearestCar().enter();}});
...
function deleteSelectedNPC(){const a=selected();if(!a||props.driving()||home.arcade?.focus)return false;releaseDesktop();mira.remove(a);syncHud();props.status="Removed "+(a.displayName||"NPC");return true;}

function bindHud() {
 document.getElementById('enterCar').onclick=()=>{if(world.h6Fauna?.mounted){world.h6Fauna.dismount();return;}if(world.h6Fauna?.toggleMount())return;const c=activeCar().driving?activeCar():nearestCar();c.driving?c.exit():c.enter();};document.getElementById('exitCar').onclick=()=>activeCar().exit();document.getElementById('jumpBtn')?.addEventListener('click',()=>{keys.Space=true;locomotion.jump?.();setTimeout(()=>keys.Space=false,120);});document.getElementById('carGear')?.addEventListener('click',()=>activeCar().cycleGear());document.getElementById('carHeadlights')?.addEventListener('click',()=>activeCar().toggleHeadlights?.());document.getElementById('carInterior')?.addEventListener('click',()=>activeCar().toggleInteriorLight?.());document.getElementById('repairCar').onclick=()=>{for(const c of props.cars())c.reset();};document.getElementById('mirrorOn').onchange=e=>{for(const c of props.cars())c.mirrorEnabled=e.target.checked;};
 ...
 const groups={melee:'Melee',firearm:'Firearms',tool:'Tools'};
 weaponSelect.replaceChildren();
 for(const [cat,label] of Object.entries(groups)){
  const og=document.createElement('optgroup');og.label=label;
  for(const [id,w] of Object.entries(WEAPONS))if((w.category||'tool')===cat)og.append(new Option(w.name,id));
  if(og.childElementCount)weaponSelect.append(og);
 }
...
function tickLocomotion(dt) {
 if(XR_ON()&&!world.h6Powers?.flying&&!world.h6Fauna?.mounted)locomotion.tick(dt,renderer.xr.getSession()?.inputSources||[],vrMenu.isOpen||props.driving());
}

let deskJumpVel=0,deskJumping=false,deskPace=0,deskHeading=new THREE.Vector3(0,0,-1);
const desktopSprint=new SprintBurst();
const ignoreHeldPlayerObstacle=o=>props.ignoresPlayerObstacle(o);
function desktopMove(dt) {
  const burst=desktopSprint.tick(dt,false,XR_ON()||vrMenu.isOpen||props.driving());
  if(world.h5Home?.arcade?.focus){deskPace=0;return;}
  if(world.h6Powers?.flying||world.h6Fauna?.mounted||props.driving()||XR_ON()||!controls||!controls.isLocked){deskPace=0;return;}
  ...
  const max=1.4*1.25*1.15*1.30*PLAYER_SPEED_GAIN*burst*(world.h5Combat?.speedScale()||1);
  ...
  world.project(obj.position,.32,-1.55,1.7,ignoreHeldPlayerObstacle);
...
document.addEventListener('visibilitychange',()=>{clock.getDelta();if(document.hidden)for(const key of Object.keys(keys))keys[key]=false;});
let fpsFrames = 0, fpsLast = performance.now(),floraAccum=0,hudAccum=0;
function tick(time,frame) {
 if(!startup.complete||boot?.failed){clock.getDelta();startup.render();return;}
 ...
 roomLight.tick(frame);
  ...
  if(rawDt>.22)world.h5LongFrames=(world.h5LongFrames||0)+1; // Record stalls; never reset live vehicles/NPCs because rendering was slow.
  const capped=Math.min(rawDt,0.05);
  const slowMo=!!equipment?.scope?.wantsSlowMo?.(keys);
  world.h5TimeScale=slowMo?0.5:1;
  const dt=capped*world.h5TimeScale;
  human6.farDetail.restore();
  ...
  tickLocomotion(dt);
  if(XR_ON()&&!props.driving()&&!world.h6Powers?.flying&&!world.h6Fauna?.mounted){const eye=camera.getWorldPosition(xrEye),before=xrBefore.copy(eye);world.project(eye,.32,-1.5,1.7,ignoreHeldPlayerObstacle);eye.x=THREE.MathUtils.clamp(eye.x,-world.extent,world.extent);eye.z=THREE.MathUtils.clamp(eye.z,-world.extent,world.extent);rig.position.add(eye.sub(before));}
  if (mira.ready) {
    ensureYardPond();
    stockPantry();
    ...
    water?.tick(dt);
    props.tick(dt);
    equipment.tick(dt);vehicleSpawns.tick(dt);home.tick(dt);cash.tick(dt);
  ...
  hudAccum+=dt;if(hudAccum>=.15){hudAccum=0;
  ...
  // Distance visibility is prepared per camera by the scene render hook.
  human6.layers.tick(frame);
...
let sharpMode=false, xrStatus='', xrHud={mode:'',scale:1.0,ffr:0.8,hz:null};
function xrQuality(){const v=Number(document.getElementById("quality")?.value);return Number.isFinite(v)?v:1.0;}
function sessionFoveation(){return .8;}
...
async function enterXr(opts={}) {
  if(!startup.complete||boot?.failed){banner('Please wait for loading to finish.');return;}
  ...
  try {
    ...
    const req=(mode,optional)=>navigator.xr.requestSession(mode,{requiredFeatures:["local-floor"],optionalFeatures:[...new Set([...optional,"layers"])]});
