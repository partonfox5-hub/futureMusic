...
export class LivingEyes {
 constructor(actor,loadMap,mode='advanced'){
  ...
  actor.root.traverse(mesh=>{if(!mesh.isSkinnedMesh)return;
   ...
   for(let index=0;index<materials.length;index++){
    ...
    if(cornea){
     const side=name.endsWith('_L')?'l':'r',m=new T.MeshPhysicalMaterial({name,color:0xffffff,map:loadMap('eye_'+side+'.jpg',true),normalMap:loadMap('eye_'+side+'_n.jpg',false),normalScale:new T.Vector2(.09,.09),roughness:.40,metalness:0,ior:1.376,specularIntensity:.7,clearcoat:.35,clearcoatRoughness:.055,envMapIntensity:.75});
     m.userData.livingEye=true;m.customProgramCacheKey=()=> 'mira-living-eye-r19';m.onBeforeCompile=shader=>this.shader(shader);entry.advanced=m;this.shells.push(mesh);
    }
 ...
 sampleLight(){
  ...
  const now=performance.now()/1000;let cached=sceneLights.get(scene);
  if(!cached||now-cached.time>.75){const lights=[];scene.traverseVisible(o=>{if(o.isLight)lights.push(o);});cached={time:now,lights};sceneLights.set(scene,cached);}
  for(const o of cached.lights){if(!o.parent||!(o.intensity>0))continue;const luminance=o.color?o.color.r*.2126+o.color.g*.7152+o.color.b*.0722:1;
 ...
 shader(s){s.uniforms.miraPupil=this.pupil;s.fragmentShader='uniform float miraPupil;\n'+s.fragmentShader;s.fragmentShader=s.fragmentShader.replace('#include <map_fragment>',`#ifdef USE_MAP
  ...
  if(abs(determinant)>1e-10){vec3 tu=(dp*dv.y-dq*du.y)/determinant,tv=(dq*du.x-dp*dv.x)/determinant,viewDir=normalize(vViewPosition);float depth=.00115/max(.42,abs(dot(viewDir,normalize(vNormal))));shift=-depth*vec2(dot(viewDir,tu)/max(dot(tu,tu),1e-7),dot(viewDir,tv)/max(dot(tv,tv),1e-7));shift=clamp(shift,vec2(-.012),vec2(.012));}
  ...
  vec4 eyeTexel=texture2D(map,.5+radial*sampleRadius/max(radius,1e-6));float limbus=smoothstep(.106,.124,radius)*(1.0-smoothstep(.129,.146,radius));eyeTexel.rgb*=1.0-.15*limbus;
  // Warm translucent sclera, a soft limbus, and a dark pupil without painted glare.
  float iris=1.-smoothstep(.125,.142,radius),pupil=1.-smoothstep(miraPupil-.001,miraPupil+.0015,radius);
  eyeTexel.rgb=mix(eyeTexel.rgb,eyeTexel.rgb*vec3(.91,.88,.84),1.-iris);eyeTexel.rgb=mix(eyeTexel.rgb,vec3(.004,.003,.002),pupil);diffuseColor*=eyeTexel;
 #endif`);}
 tick(dt,time){
  ...
  this.pupil.value=T.MathUtils.damp(this.pupil.value,target,target<this.pupil.value?2.6:1.1,Math.min(.05,dt));
 }
}
