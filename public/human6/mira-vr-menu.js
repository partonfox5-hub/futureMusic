import {FURNITURE,SURFACES} from './mira-v2-builder.js?v=20.3.0';
import {WEAPONS} from './mira-v2-props.js?v=20.3.0';
import {draft,saveDraft,OUTFITS,PERSONAS,clothingItems,setDraftGarment} from './mira-v2-catalog.js?v=20.3.0';
import {SCENES} from './mira-v2-world.js?v=20.3.0';
import {GARMENTS} from './mira-v2-wardrobe.js?v=20.3.0';
import * as THREE from 'three';
import {SLIDERS,FACE_TYPES} from './mira-v2.js?v=20.3.0';
import {shapeSliders,FACE_PRESETS,HAIR_STYLES,ACTIVITY_MODES,ACTION_LABELS,POSE_LABELS,ATTENTION_MODES,ATTENTION_LABELS} from './mira-v2-controls.js?v=20.3.0';
import {HAIR_COLORS} from './mira-v2.js?v=20.3.0';
import {EMOTION_NAMES,IDLE_NAMES,WALK_NAMES} from './mira-v2-features.js?v=20.3.0';
export function createVRMenu({scene,renderer,camera,system,spawn,onSync,world,wardrobe,spawnConfigured,copyConfiguration,props,saveScene,loadScene}){
 ...
 function draw(){
  ...
  if(page>=17&&world.h6UI?.vr(page,{ctx,button,cycle,draw})){}else if(page===16){
   ...
   cycle('Destination',475,places,()=>destination,v=>{if(region)region.menuDestination=v;});
  ...
  }else if(page===8){const car=props.cars?.().find(c=>c.driving)||props.vehicle;button(car.driving?'EXIT CAR':'ENTER / EXIT',40,325,455,80,()=>{car.enter();draw();});button('GEAR '+(car.gear||'P'),515,325,467,80,()=>{car.cycleGear?.();draw();});button('JUMP',40,445,455,80,()=>{document.getElementById('jumpBtn')?.click();draw();});button('REPAIR / RESET CAR',515,445,467,80,()=>{for(const c of props.cars?.()||[car])c.reset();draw();});button('REAR VIEW MIRROR '+(car.mirrorEnabled?'ON':'OFF'),40,565,942,80,()=>{car.mirrorEnabled=!car.mirrorEnabled;draw();});ctx.fillStyle='#dce8f2';ctx.font='27px sans-serif';ctx.fillText(car.telemetry||car.status||'',40,690);ctx.font='29px sans-serif';for(const [i,text] of ['Left door: drive. Right door: passenger.','Select Mira, trigger the car to seat her.','Left trigger drives in D/R. Grip stalk P/R/N/D.','Grip wheel to steer. X / Space brakes.'].entries())ctx.fillText(text,40,750+i*57);button('HEADLIGHTS: '+(car.lightMode||'auto').toUpperCase(),40,1020,455,70,()=>{car.toggleHeadlights?.();draw();});button('INTERIOR: '+(car.cabinLightOn?'ON':'OFF'),515,1020,467,70,()=>{car.toggleInteriorLight?.();draw();});ctx.fillText('Left stick click: sprint 10s · cooldown 10s',40,1170);ctx.fillText('Right stick click: restraint anchors',40,1220);
  }else if(page===7){
   ...
   button('REBUILD HOUSE',40,1114,942,62,()=>{world.setScene(world.name);notice='Map rebuilt';draw();});
   ...
   if(a.version==='v2'){
    ...
    button('PREVIEW WIDE SMILE',40,800,942,72,()=>{if(a.h5Identity)a.h5Identity.express('wideSmile');else a.playFaceReference?.('smile');});
    button('PREVIEW FROWN',40,890,942,72,()=>a.h5Identity?.express('frown'));
    button('RETRY VOICE',40,990,942,72,()=>document.getElementById('micBtn')?.click());
