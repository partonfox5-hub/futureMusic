...
export class DogFur {
  ...
  installSoftCoat(model){
    ...
    // close-up shell and guard-hair detail is managed by distance LOD.
    softOwners++;this.softOwner=true;if(!softTexture&&typeof document!=='undefined'){softTexture=new THREE.TextureLoader().load(new URL('fur-neutral-20.1.png',ASSET_ROOT).href);softTexture.wrapS=softTexture.wrapT=THREE.RepeatWrapping;softTexture.colorSpace=THREE.NoColorSpace;softTexture.anisotropy=4;}
    const mat=model.coat;mat.roughness=.91;mat.normalMap=null;mat.normalScale.set(0,0);mat.onBeforeCompile=s=>{
      ...
      s.fragmentShader=s.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
        ...
        float torsoGroom=smoothstep(.23,.36,h5CoatP.y)*(1.-smoothstep(.52,.63,h5CoatP.y))*(1.-smoothstep(.20,.34,h5CoatP.z));vec2 coatUV=mix(vec2(h5CoatP.x*2.8+h5CoatP.z*1.2,h5CoatP.y*3.5),vec2(h5CoatP.x*3.+h5CoatP.y*2.,-h5CoatP.z*2.6),torsoGroom);vec3 coatSample=texture2D(h5CoatMap,coatUV).rgb;float softFiber=dot(coatSample,vec3(.333));diffuseColor.rgb*=.77+softFiber*.46;diffuseColor.rgb*=mix(1.0,.73,h5CoatWet)*(1.0+fiber*fiberLod*.09);`);
...
#include <opaque_fragment>`);
    };mat.customProgramCacheKey=()=> 'h6-soft-coat-20.1';
    // Current breeds use one tapered, area-distributed guard-hair draw at close range.
    // Legacy shell implementation retained for older coats; current breeds use fins.
    model.h6CoatTexture=softTexture;this.guardHair=buildGuardHair(model);
    this.shellCount=0;
  }
  installGuardCoat(model){
    const source=model.body;if(!source)return;const long=model.breed.fur>1.1,levels=3;
    for(let layer=1;layer<=levels;layer++){
      const material=new THREE.MeshStandardMaterial({color:0xffffff,vertexColors:true,roughness:.95,side:THREE.FrontSide,alphaTest:.55,alphaToCoverage:false,depthWrite:true});
      material.name='H5_GuardCoat_'+layer;const length=(long?.010:.006)*layer*(model.species==='cat'?1.25:1),uniforms={time:{value:0},wet:{value:0}};
      material.userData.h5GuardUniforms=uniforms;material.onBeforeCompile=s=>{
        s.uniforms.h5GuardWet=uniforms.wet;s.uniforms.h6GuardColor={value:softTexture};
        ...
        s.fragmentShader=s.fragmentShader.replace('#include <common>','#include <common>\nvarying vec3 h5GuardRest;varying vec3 h5GuardN;uniform sampler2D h6GuardColor;');
        s.fragmentShader=s.fragmentShader.replace('#include <alphatest_fragment>',`float facing=abs(dot(normalize(vNormal),normalize(vViewPosition)));if(facing>.55)discard;vec3 sampleCoat=texture2D(h6GuardColor,vec2(h5GuardRest.z*2.2+h5GuardRest.x*.8,h5GuardRest.y*2.8)).rgb;diffuseColor.rgb*=.67+dot(sampleCoat,vec3(.333))*.60;vec3 flow=vec3(h5GuardRest.x*410.,h5GuardRest.y*110.,h5GuardRest.z*350.);vec3 w=pow(abs(normalize(h5GuardN)),vec3(6.));w/=max(dot(w,vec3(1.)),.001);float strand=sin(flow.x+sin(flow.z*.17)*.65)*w.z+sin(flow.z+sin(flow.y)*.5)*w.x+sin(flow.x+sin(flow.z*.17)*.65)*w.y;float fade=1.-smoothstep(.6,2.,max(fwidth(flow.x),fwidth(flow.z)));if(strand<${layer===1?'-.55':layer===2?'-.12':'.4'}||fade<.2)discard;diffuseColor.rgb*=.94;`);
      };material.customProgramCacheKey=()=> 'h6-guard-coat-20-'+length;model.materials.add(material);
  ...
  rebindLayers(){this.layers=[];this.guardHair=this.model.meshes.find(m=>m.material.name==='H6_GuardHair');this.model.root.traverse(mesh=>{if(mesh.isMesh&&/^(Dog_FurShell_|H5_GuardCoat_)/.test(mesh.material.name))this.layers.push({mesh,layer:Number(mesh.material.name.split('_').at(-1)),uniforms:mesh.material.userData.h5GuardUniforms});});}
  setLayers(n){this.shellCount=Math.max(0,Math.min(3,n));if(this.guardHair)this.guardHair.visible=this.shellCount>=3;for(const {mesh,layer} of this.layers)mesh.visible=layer<=this.shellCount;}
