...
export class Props {
 ...
 hold(item,key){if(!item||item.holder!==null)return false;this.drop(key);item.holder=key;item.lastTip=null;item.velocity.set(0,0,0);this.held.set(key,item);this.status=item.data.name+' held · '+(key==='desktop'?'click to use, Q to drop':'trigger fires, release grip to drop');return true;}
 equip(id){return this.hold(this.items.find(i=>i.id===id),'desktop');}
 drop(key){const item=this.held.get(key);if(!item)return;item.holder=null;item.lastTip=null;this.held.delete(key);this.status='Dropped '+item.data.name;}
 grip(i){if(this.vehicle?.grip(i))return true;const p=this.system.hands.grip[i].getWorldPosition(V()),item=this.items.filter(x=>x.holder===null).sort((a,b)=>a.group.position.distanceToSquared(p)-b.group.position.distanceToSquared(p))[0];return item&&item.group.position.distanceTo(p)<.28?this.hold(item,i):false;}
 release(i){this.vehicle?.release(i);this.drop(i);}
 trigger(i){if(this.restraints?.placing)return this.restraints.place(this.ray(i));if(this.vehicle?.driving)return true;const held=this.held.get(i);if(held){if(['bullet','laser'].includes(held.data.kind))this.fire(held);return true;}return false;}
 ...
 tick(dt){this.time+=dt;this.syncWorld();for(const item of this.items)item.group.visible=this.world.root.visible;if(!this.world.root.visible){this.vehicle?.tick(dt);return;}this.vehicle?.tick(dt);this.restraints?.tick(dt);this.injuries?.tick(dt);for(const item of this.items){if(item.holder!==null){const vr=item.holder!=='desktop',c=vr?this.system.hands.grip[item.holder]:this.camera;if(!c){this.drop(item.holder);continue;}const gun=item.data.kind==='bullet'||item.data.kind==='laser';const local=vr?(gun?new T.Vector3(0,.03,-.05):new T.Vector3(0,.02,-.06)):(gun?new T.Vector3(.18,-.12,-.42):new T.Vector3(.22,-.16,-.45));item.group.position.copy(c.localToWorld(local));item.group.quaternion.copy(c.getWorldQuaternion(Q()));if(vr&&gun)item.group.rotateX(-.42);else if(!vr&&gun)item.group.rotateX(.18);item.kick*=Math.exp(-dt*14);if(item.kick)item.group.rotateX(-item.kick*2.8);if(item.swing>0){item.swing=Math.max(0,item.swing-dt);item.group.rotateY(Math.sin(item.swing/.30*Math.PI)*1.1);}item.group.updateMatrixWorld(true);
 const tip=item.group.localToWorld(new T.Vector3(0,0,-item.data.reach));if(item.lastTip&&item.holder!=='desktop'&&['cut','blunt'].includes(item.data.kind)){const dist=tip.distanceTo(item.lastTip),speed=Math.min(12,dist/Math.max(.001,dt));if(dist>.004&&dist<.75){const ray=new T.Ray(item.lastTip.clone(),tip.clone().sub(item.lastTip).normalize()),hit=this.hit(ray,dist+.025),key=hit&&(this.actorFor(hit.object)?.group.uuid||hit.object.uuid);if(hit&&this.time-(item.lastHit.get(key)??-2)>.24){item.lastHit.set(key,this.time);this.impact(hit,.5*item.data.mass*speed*speed,ray.direction,item.data.kind,item.data.sharpness);this.system.hands.haptics.contact(item.holder,'prop',speed,.01);}}}item.lastTip=tip;const p=item.group.position;if(item.lastPoint)item.velocity.copy(p).sub(item.lastPoint).divideScalar(Math.max(.001,dt)).clampLength(0,8);item.lastPoint=p.clone();
