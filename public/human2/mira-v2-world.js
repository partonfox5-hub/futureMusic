...
export class MiraWorld {
 constructor(scene,system){this.scene=scene;this.system=system;this.root=new T.Group();scene.add(this.root);this.obstacles=[];this.seats=[];this.pickables=[];this.routes=new WeakMap();this.seated=new WeakMap();this.time=0;this.setScene('Living room');}
 ...
 setScene(name){
  ...
  this.root.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();}});this.root.clear();this.obstacles=[];this.seats=[];this.pickables=[];this.name=name;
  ...
  if(!jungle&&!beach){
   this.box(0,1.65,-4.45,9,3.3,.15,0xd4cfbf);this.box(-4.45,1.65,0,.15,3.3,9,0xc7c2b3);this.box(4.45,1.65,0,.15,3.3,9,0xd5d0c1);
   this.box(-4.34,1.95,-.7,.03,1.5,2.2,0xa8c9ce);for(const z of [-1.82,.42])this.box(-4.3,1.95,z,.07,1.6,.07,0x715741);
   const rug=this.box(.1,.006,0,3.3,.014,2.75,0x9d8d79);rug.receiveShadow=true;
   this.chair(0,-2.6,0,true);this.chair(2,-.9,-Math.PI/2);this.chair(-1.9,-.6,Math.PI/2);
   const table=this.box(.15,.61,-.5,1.12,.09,.65,0x6f4c35);this.obstacle(.15,-.5,1.12,.65,0,.655,table);for(const x of [-.32,.62])for(const z of [-.73,-.27])this.box(x,.30,z,.055,.60,.055,0x4f3e32);
   this.mesh(new T.CylinderGeometry(.18,.23,.32,16),this.mat(0xb08b69),3.6,.16,-3.4);this.tree(3.6,-3.4,1.65,false);
   const lamp=this.mesh(new T.ConeGeometry(.26,.34,20,1,true),this.mat(0xeaddbc),-3.2,1.64,-2.9);lamp.material.side=T.DoubleSide;this.box(-3.2,.72,-2.9,.035,1.45,.035,0x484844);
  }else{
  ...
  for(const [x,z,w,d] of [[-4.4,0,.2,9],[4.4,0,.2,9],[0,-4.4,9,.2],[0,4.4,9,.2]])this.obstacle(x,z,w,d,0,4);
 }
 ...
 blocked(p,r=.25,ignore=null){return this.obstacles.some(o=>o!==ignore&&Math.abs(p.x-o.x)<o.w/2+r&&Math.abs(p.z-o.z)<o.d/2+r);}
 path(start,goal,r=.25){
  const step=.22,n=39,toCell=p=>[clamp(Math.round((p.x+4.18)/step),0,n-1),clamp(Math.round((p.z+4.18)/step),0,n-1)],at=(x,z)=>new T.Vector3(x*step-4.18,0,z*step-4.18),id=(x,z)=>z*n+x,[sx,sz]=toCell(start),[gx,gz]=toCell(goal),end=id(gx,gz),begin=id(sx,sz);
 ...
 command(ray,actor){const rc=new T.Raycaster();rc.ray.copy(ray);const hit=rc.intersectObjects(this.pickables,false)[0];if(!hit)return null;const seat=hit.object.userData.seat;if(seat)return this.walk(actor,seat.approach,seat)?'seating':'blocked';return 'blocked';}
 ...
 project(p,r,y=0,height=.1){let hit=false;for(const o of this.obstacles){if(y>o.y+o.h||y+height<o.y)continue;const dx=p.x-o.x,dz=p.z-o.z,ex=o.w/2+r-Math.abs(dx),ez=o.d/2+r-Math.abs(dz);if(ex>0&&ez>0){if(ex<ez)p.x+=(dx>=0?1:-1)*ex;else p.z+=(dz>=0?1:-1)*ez;hit=true;}}return hit;}
 projectSphere(p,r){let hit=false;for(const o of this.obstacles){const q=new T.Vector3(T.MathUtils.clamp(p.x,o.x-o.w/2,o.x+o.w/2),T.MathUtils.clamp(p.y,o.y,o.y+o.h),T.MathUtils.clamp(p.z,o.z-o.d/2,o.z+o.d/2)),d=p.clone().sub(q),l=d.length();if(l>=r)continue;if(l>.000001)p.copy(q).addScaledVector(d,r/l);else{const choices=[[o.x+o.w/2+r-p.x,'x',1],[p.x-(o.x-o.w/2-r),'x',-1],[o.y+o.h+r-p.y,'y',1],[p.y-o.y+r,'y',-1],[o.z+o.d/2+r-p.z,'z',1],[p.z-(o.z-o.d/2-r),'z',-1]].sort((a,b)=>a[0]-b[0]);p[choices[0][1]]+=choices[0][0]*choices[0][2];}hit=true;}return hit;}
 tick(dt){this.time+=dt;for(const seat of this.seats)if(seat.occupant&&!this.system.actors.includes(seat.occupant))seat.occupant=null;for(const b of this.system.balls){if(b.held)continue;const before=b.mesh.position.clone();if(this.projectSphere(b.mesh.position,b.rad)){const n=b.mesh.position.clone().sub(before).normalize(),v=b.vel.dot(n);if(v<0)b.vel.addScaledVector(n,-1.4*v);}}}
}
