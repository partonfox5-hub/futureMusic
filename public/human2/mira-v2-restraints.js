...
import {bodyVolumes,projectVolume} from './mira-v2-contact.js?v=11.0';
import {furnitureRoot,syncFurniture} from './mira-v2-furniture.js?v=12.3';
...
export class Restraints {
 ...
 move(a,delta,hard=false){
  ...
  const step=delta.clone();if(!hard&&step.length()>.22)step.setLength(.22);
  ...
  actor.group.position.y=Math.max(actor.baseY||0,actor.group.position.y);actor.group.updateMatrixWorld(true);actor.contactSoft?.(a.region,delta.clone().normalize(),.002,.03);
 }
 clampLink(link){
  const A=this.position(link.a),B=this.position(link.b),n=A.distanceTo(B),extra=n-link.length;
 ...
 solve(dt){
  for(const link of this.links){
   ...
   for(let j=0;j<8;j++){
    const A=this.position(link.a),B=this.position(link.b),n=A.distanceTo(B),extra=n-link.length;
    if(extra<=.001||n<1e-6)break;
    let wa=this.weight(link.a),wb=this.weight(link.b),total=wa+wb;if(!total)break;
    const dir=B.clone().sub(A).multiplyScalar(1/n);
    this.move(link.a,dir.clone().multiplyScalar(extra*wa/total));
    this.move(link.b,dir.clone().multiplyScalar(-extra*wb/total));
   }
   this.clampLink(link);
  }
 }
 constrainActor(actor){if(!actor)return;for(const link of this.links){if(link.broken)continue;const mine=link.a.actor===actor?link.a:link.b.actor===actor?link.b:null;if(!mine)continue;const other=mine===link.a?link.b:link.a,P=this.position(mine),Q=this.position(other),n=P.distanceTo(Q);if(n<=link.length+.002)continue;const target=Q.clone().add(P.sub(Q).setLength(link.length));actor.group.position.add(target.sub(this.position(mine)));actor.group.updateMatrixWorld(true);}}
 constrainObject(object){if(!object)return;for(const link of this.links){if(link.broken)continue;const mine=link.a.object===object?link.a:link.b.object===object?link.b:null;if(!mine)continue;const other=mine===link.a?link.b:link.a,P=this.position(mine),Q=this.position(other),n=P.distanceTo(Q);if(n<=link.length+.002)continue;const target=Q.clone().add(P.clone().sub(Q).setLength(link.length));object.position.add(target.sub(P));object.updateWorldMatrix(true,true);if(mine.furniture)syncFurniture(this.world,object);}}
 ensurePanel(){
 ...
 drawPanel(){
  ...
  const ctx=p.canvas.getContext('2d');ctx.clearRect(0,0,512,280);
  ctx.fillStyle='rgba(16,12,10,.94)';ctx.fillRect(0,0,512,280);
  ctx.strokeStyle='#7eb6ff';ctx.lineWidth=4;ctx.strokeRect(2,2,508,276);
  ctx.fillStyle='#9bd6ff';ctx.font='700 28px Segoe UI,sans-serif';ctx.fillText('LINK '+link.id,24,44);
  ctx.fillStyle='#f4efe8';ctx.font='22px Segoe UI,sans-serif';ctx.fillText(link.length.toFixed(2)+' m',330,44);
  ctx.fillStyle='#d8d0c6';ctx.font='16px Segoe UI,sans-serif';ctx.fillText('Trigger and drag the bar to set length',24,78);
  ctx.fillStyle='#526c80';ctx.fillRect(28,112,456,36);
  ...
  const btn=(x,y,w,h,label)=>{ctx.fillStyle='#7eb6ff';ctx.fillRect(x,y,w,h);ctx.fillStyle='#111';ctx.font='700 18px Segoe UI,sans-serif';ctx.fillText(label,x+16,y+28);};
  btn(28,176,140,48,'SHORTER');btn(186,176,140,48,'LONGER');btn(344,176,140,48,'CUT');
  p.tex.needsUpdate=true;
 }
 panelAction(uv){
  if(!this.selected||this.selected.broken||!uv)return false;
  if(uv.y>.46&&uv.y<.64){this.setLength(T.MathUtils.clamp((uv.x-.055)/.89*5,.03,5));return true;}
  if(uv.y>.18&&uv.y<.40){
   if(uv.x<.36)this.setLength(this.selected.length-.08);
   else if(uv.x<.68)this.setLength(this.selected.length+.08);
   else this.cut();
   return true;
  }
  return true;
 ...
 tickPopup(){
  ...
  this.ensurePanel();this.drawPanel();
  ...
  if(xr){
   ...
   if(session)for(const src of session.inputSources){
    ...
    if(hit?.object===p.mesh&&down)this.panelAction(hit.uv);
   }
  }else{
