...
export class ContactHaptics {
 constructor(renderer){this.renderer=renderer;this.gain=1;this.time=0;this.last=[-1,-1];this.pending=[0,0];this.duration=[0,0];}
 ...
 contact(i,kind,speed=0,depth=0){
  ...
  const strength=Math.min(.13,this.gain*(base+Math.min(2,Math.max(0,speed))*.015+Math.min(.02,Math.max(0,depth))*.45));
