...
export function createVRMenu({scene,renderer,camera,system,spawn,onSync}){
 ...
 let page=0,shapePage=0,items=[],open=false,drag=[null,null],stamp='',lastDraw=0,notice='';
 ...
 function toggle(){open=!open;panel.visible=open&&renderer.xr.isPresenting;drag=[null,null];if(open){const c=renderer.xr.getCamera();const eye=c.getWorldPosition(new THREE.Vector3());const forward=c.getWorldDirection(new THREE.Vector3());forward.y=0;if(forward.lengthSq()<.001)forward.set(0,0,-1);forward.normalize();panel.position.copy(eye).addScaledVector(forward,1.1);panel.position.y-=.10;panel.lookAt(eye.x,panel.position.y,eye.z);panel.updateMatrixWorld(true);draw();}}
 ...
 function tick(){
  ...
  rays.forEach((r,i)=>r.visible=open||(renderer.xr.isPresenting&&hitWrist(i)));if(!open)return;
