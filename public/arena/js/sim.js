...
export class Simulation {
 ...
 populate(){
  ...
  for(const n of this.map.nests)this.add('kennel',n.pos.clone().addScaledVector(n.dir,2.6),{hp:240,maxHp:240,r:3,nest:n.id,q:new T.Quaternion().setFromUnitVectors(V(0,0,1),n.dir),scale:1.7});
 ...
 tickProjectiles(dt,input){for(let i=this.projectiles.length-1;i>=0;i--){const b=this.projectiles[i];if(!b.alive){this.projectiles[i]=this.projectiles[this.projectiles.length-1];this.projectiles.pop();this.projectileFree.push(b);continue;}b.age+=dt;b.life-=dt;b.prev.copy(b.p);
  ...
  if(b.owner==='enemy'){
   ...
   if(w.tip&&segmentDistance(next,w.base,w.tip)<.35){blocked=true;this.emit('shield',next);}
