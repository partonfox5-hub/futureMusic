...
import {G,M,material,buildModel,part,mergeParts,tickTicker} from './models.js';
...
function animate(bone,e,time,distance){offset.copy(bone.pos);q.copy(bone.q);s.copy(bone.scale);if(distance>85)return;const n=bone.name,phase=e.phase||0,walk=Math.sin(time*(e.type==='trilo'?5.5:3.5)+phase),side=n.includes('-1')?-1:1;
 if(n==='blade')s.z=Math.max(.06,e.morph||0);else if(n==='gun')s.z=1-(e.morph||0)*.65;else if(n==='jaw')q.multiply(q2.setFromAxisAngle(V(1,0,0),(e.jaw||0)*.78));
 ...
 else if(n.startsWith('arm'))q.multiply(q2.setFromAxisAngle(V(1,0,0),e.attack>0?-1.0:Math.sin(time*2+phase+side)*.2));
 else if(n==='fore'){offset.z+=(e.mode===1?.23:0);q.multiply(q2.setFromAxisAngle(V(1,0,0),-.18));}
...
class ModelBatch {
 ...
 resize(n){if(n<=this.capacity)return;this.capacity=2**Math.ceil(Math.log2(n));for(const b of this.bones)for(const p of b.parts){if(p.mesh){this.scene.remove(p.mesh);p.mesh.dispose();}p.mesh=new T.InstancedMesh(p.geometry,p.material,this.capacity);p.mesh.instanceMatrix.setUsage(T.DynamicDrawUsage);p.mesh.frustumCulled=false;p.mesh.count=0;this.scene.add(p.mesh);}}
 begin(){this.count=0;}
 add(e,time,distance){this.resize(this.count+1);let scale=e.scale||1;if(e.type==='knight'&&e.white)scale*=1.85;if(e.type==='hatch')scale=e.r;if(e.type==='cage'&&e.shake>0)scale*=1+Math.sin(time*48)*.045*e.shake/.38;if(e.type==='hornet')scale*=1.25;
...
export class EntityView {
 ...
 update(g){tickTicker(g.time);for(const b of this.batches.values())b.begin();let neckN=0,jointN=0,spineN=0;this.visible.length=0;const far=this.quality==='high'?150:this.quality==='performance'?85:110;
  for(const e of g.entities){if(!e.alive||['shard','coin','gold','well'].includes(e.type))continue;const distance=e.p.distanceTo(g.player.p),max=e.enemy?far:['window','marquee'].includes(e.type)?60:e.type==='blimp'?140:85;if(distance>max)continue;
   ...
   if(e.type==='hydra'){const nest=g.map.nests[e.nest],a=nest.pos.clone().addScaledVector(nest.dir,-2.2),delta=e.p.clone().sub(a),side=nest.dir.clone().cross(V(0,1,0)).normalize(),up=side.clone().cross(nest.dir).normalize();const n=this.quality==='high'?20:16;let previous=null;
    for(let i=0;i<=n&&neckN<512;i++){const u=i/n,phase=u*Math.PI*(3.4-(e.coil||.5)*1.2)+g.time*.9+e.phase,rad=Math.sin(Math.PI*u)*(1.5-(e.coil||.5)*.8),pos=a.clone().addScaledVector(delta,u).addScaledVector(side,Math.sin(phase)*rad).addScaledVector(up,Math.cos(phase)*rad),thickness=.48-u*.22;
     if(previous){const vector=pos.clone().sub(previous),length=vector.length(),q=new T.Quaternion().setFromUnitVectors(V(0,1,0),vector.normalize());m.compose(pos.clone().add(previous).multiplyScalar(.5),q,V(thickness,length+.035,thickness));this.neck.setMatrixAt(neckN++,m);m.compose(pos,q,V(thickness,thickness,thickness));this.joints.setMatrixAt(jointN++,m);
      if(i%2===0&&spineN<256){const dorsal=up.clone().multiplyScalar(thickness),spine=pos.clone().add(dorsal);m.compose(spine,new T.Quaternion().setFromUnitVectors(V(0,1,0),up),V(.10,.40,.12));this.spines.setMatrixAt(spineN++,m);}}
     previous=pos;
    }
   }
