import {T,V,clamp,unit,raySphere,segmentDistance} from './math.js?v=4.0.0';
import {SPHERES,LINKS,BRANCHES,RULES} from './data.js?v=4.0.0';
import {fractureProfile,inSphereHole,inTubeHole} from './fracture.js?v=4.0.0';
export class ArenaMap {
 constructor(random){
  ...
  for(const [id,delay] of [[0,18],[1,40],[2,72]]){const s=this.spheres[id];let dir=unit(random);for(let i=0;i<16;i++){dir=unit(random);if(Math.abs(dir.y)<.7&&!this.spheres.some(o=>o!==s&&o.c.clone().sub(s.c).normalize().dot(dir)>.8))break;}this.nests.push({id:this.nests.length,sphere:id,dir,pos:s.c.clone().addScaledVector(dir,s.r*.992),timer:delay,open:false,openT:0,headsNext:1,headHealth:60,severed:0,headSerial:0,regrowth:[],fallen:false,disabled:false,hp:240,maxHp:240});}
 ...
 scorch(hit,radius=3){if(!hit||!['sphere','tube'].includes(hit.type))return false;let b=this.burns.find(b=>b.type===hit.type&&b.id===hit.id&&b.pos.distanceToSquared(hit.pos)<5);if(!b){b={type:hit.type,id:hit.id,pos:hit.pos.clone(),hits:0,radius};this.burns.push(b);}b.radius=Math.max(b.radius||3,radius);if(++b.hits<4)return false;
