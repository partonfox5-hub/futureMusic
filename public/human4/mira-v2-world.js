...
export class MiraWorld {
 ...
 before(a){if(a.autonomy&&a.mode==='wander'&&a.dest&&!a.navigation&&!a.socialPair&&!a.seat){const points=this.path(a.group.position,a.dest,.22*Math.sqrt(a.shape.hips));if(points?.length){a.navigation={points,index:0,goal:a.dest.clone(),auto:true};if(points[0].distanceTo(a.group.position)>.2)a.dest=points[0].clone();}else a.dest=null;}if(a.navigation&&a.balance.state!=='standing'){if(a.navigation.seat)a.navigation.seat.occupant=null;a.navigation=null;}const n=a.navigation;if(n){const p=n.points[n.index];if(a.group.position.clone().setY(0).distanceTo(p)<.16||!a.dest){n.index++;if(n.index<n.points.length){a.dest=n.points[n.index].clone();a.directedWalk=n.goal.clone();a.mode='wander';a.autoWander=true;}else{a.navigation=null;a.dest=null;a.directedWalk=null;if(n.auto)a.setMode('auto');else{a.autoWander=false;a.setMode('idle');}if(n.seat){a.seat=n.seat;n.seat.occupant=a;a.seatBlend=0;}}}}}
 after(a,dt){
