import {LivingEyes} from './mira-v2-eyes.js?v=h4.1';
import {EMOTION_NAMES,IDLE_NAMES,WALK_NAMES} from './mira-v2-controls.js?v=h4.1';
export {EMOTION_NAMES,IDLE_NAMES,WALK_NAMES} from './mira-v2-controls.js?v=h4.1';
import * as THREE from 'three';
import {restoreSurfaceUV} from './mira-v2-uv.js?v=h4.1';
import {V2_EXTRA_SLIDERS,FACE_PRESETS,EXERCISE_MODES} from './mira-v2-controls.js?v=h4.1';
import {HairGuides} from './mira-v2-hair.js?v=h4.1';
import {SurfaceFlesh} from './mira-v2-tissue.js?v=h4.1';
...
export function createV2Class(Base,{loadMap,MORPH,BODY_HIT,installSkinShader,HAIR_COLORS,SLIDERS}){
 return class MiraV2 extends Base {
  constructor(root,scale,opts={}){
   ...
   this.headTouch=new THREE.Vector3();this.headTouchGoal=new THREE.Vector3();this.attentionT=0;this.attention=V();this.externalHands=[];this.painFace=0;
  ...
  tickExpr(dt){
   ...
   if(this.painFace>0)this.painFace=Math.max(0,this.painFace-dt);
   ...
   if(this.painFace>0||this.woundReact){
    const k=this.painFace>0?clamp(this.painFace/2.2,0,1):.55;
    this.want.Mouth_Frown=Math.max(this.want.Mouth_Frown||0,.62*k);
    this.want.Brow_Raise_Inner_L=Math.max(this.want.Brow_Raise_Inner_L||0,.48*k);
    this.want.Brow_Raise_Inner_R=Math.max(this.want.Brow_Raise_Inner_R||0,.48*k);
    this.want.Brow_Drop_L=Math.max(this.want.Brow_Drop_L||0,.28*k);
    this.want.Brow_Drop_R=Math.max(this.want.Brow_Drop_R||0,.28*k);
    this.want.Mouth_Smile_L*=1-k;this.want.Mouth_Smile_R*=1-k;
   }
  ...
  reactToHit(hit,dir,energy,kind,point){
   ...
   if((this.time||0)-(this.painAt||-10)>0.42){
    ...
    this.painFace=2.8;
    this.cryOut(pick[0],pick[1]==='angry'?'angry':'sad');
   }
  }
  poseWoundReact(dt){
