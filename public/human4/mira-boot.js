...
import {miraSpeak,unlockVoice} from './mira-voice-v2.js?v=h4.1';
import {retryNeuralVoice} from './mira-neural-voice.js?v=h4.1';
import {FACE_PRESETS,HAIR_STYLES,EMOTION_NAMES,IDLE_NAMES,WALK_NAMES,ACTIVITY_MODES,ACTION_LABELS} from './mira-v2-controls.js?v=h4.1';
...
if(hud){
  ...
  hud.addEventListener('mousedown',e=>{if(e.target.tagName==='SELECT')openSel();});
...
const fields={npcName:'name',npcBody:'bodyType',npcFace:'faceType',npcHair:'hairStyle',npcColor:'hairColor',npcOutfit:'outfit',npcPersona:'persona',npcPrompt:'prompt'};
...
function sync(){for(const slot of slots)$('spawn_'+slot).value=clothingItems().find(id=>GARMENTS.find(g=>g.id===id)?.slot===slot)||'';for(const [id,key] of Object.entries(fields))$(id).value=String(draft[key]);}
