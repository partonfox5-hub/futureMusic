...
export function garmentPattern(actor,style,surface){
 ...
 const accepts=(x,y,z)=>{if(y<bottom||y>style.top||Math.abs(x)>.235)return false;
  ...
  return true;};
 const add=(mesh,id,tri)=>{const g=mesh.geometry,raw=V().fromBufferAttribute(g.attributes.position,id),key=(raw.y<.75?Math.sign(raw.x)+'/':'')+[raw.x,raw.y,raw.z].map(x=>Math.round(x/.023)).join('/');if(weld.has(key))return weld.get(key);const i=p.length;weld.set(key,i);const position=mesh.getVertexPosition(id,V()).applyMatrix4(mesh.matrixWorld),normal=V().fromBufferAttribute(g.attributes.normal,id).transformDirection(mesh.matrixWorld);position.addScaledVector(normal,.022);p.push(position);const k=tri.indexOf(id),bary=V();bary.setComponent(k,1);let bi=0,weight=-1;for(let j=0;j<4;j++)if(g.attributes.skinWeight.array[id*4+j]>weight){weight=g.attributes.skinWeight.array[id*4+j];bi=g.attributes.skinIndex.array[id*4+j];}refs.push({cache:surface.stores.get(g.attributes.position),ids:tri,bary,bone:mesh.skeleton.bones[bi]});pins.push(raw.y>style.top-.035||(style.id==='bikiniTop'&&raw.y<1.145));uv.push(Math.atan2(raw.x,raw.z)/(2*Math.PI)+.5,(style.top-raw.y)/Math.max(.01,style.top-style.bottom));return i;};
...
export function fabricMaterial(style){const satin=style.fabric==='satin',m=new T.MeshPhysicalMaterial({color:style.color,roughness:satin?.48:.9,metalness:0,side:T.DoubleSide,sheen:satin?.6:.22,sheenColor:new T.Color(style.color),sheenRoughness:.75,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2,depthWrite:true});m.onBeforeCompile=s=>{s.vertexShader='varying vec2 fabricUV;\n'+s.vertexShader;s.fragmentShader='varying vec2 fabricUV;\n'+s.fragmentShader;s.vertexShader=s.vertexShader.replace('#include <uv_vertex>','#include <uv_vertex>\nfabricUV=uv;');s.fragmentShader=s.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
 vec2 weaveUv=fabricUV*vec2(${style.fabric==='rib'?'110.0,18.0':'190.0,145.0'});float lod=1.0-smoothstep(.35,1.5,max(fwidth(weaveUv.x),fwidth(weaveUv.y)));float weave=sin(weaveUv.x*6.28318)*sin(weaveUv.y*6.28318);float hem=1.0-smoothstep(.004,.018,min(fabricUV.y,1.0-fabricUV.y));diffuseColor.rgb*=1.0+weave*lod*.035-hem*.11;`);};m.customProgramCacheKey=()=> 'mira-fabric-r10-'+style.fabric;return m;}
